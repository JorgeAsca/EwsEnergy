...
class RunScriptPlugin {
    ...
    async _runScriptAsync(heftTaskSession, heftConfiguration, pluginOptions, runOptions) {
        const resolvedModulePath = path.resolve(heftConfiguration.buildFolderPath, pluginOptions.scriptPath);
        const runScript = await Promise.resolve(`${resolvedModulePath}`).then(s => __importStar(require(s)));
