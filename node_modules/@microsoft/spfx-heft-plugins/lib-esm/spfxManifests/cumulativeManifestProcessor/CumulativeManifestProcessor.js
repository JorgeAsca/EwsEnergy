...
import { buildManifestsJsAsync } from '../manifestFileBuilder/ManifestsFileBuilder';
import { ManifestUrlProcessor } from './ManifestUrlProcessor';
import { DEFAULT_TEMP_FOLDER } from '../../utilities/constants';
...
export var DependencyDiscoveryMode;
...
/**
 * @beta
 */ export class CumulativeManifestProcessor {
    /**
   * Ths function:
   ...
   */ async generateCumulativeManifestAsync(debugManifests, manifestsJsFileMetadata, discoveryMode) {
        ...
        if (debugManifests.length > 0 && debugManifests.length <= 2) {
            try {
                const packageJson = PackageJsonLookup.instance.loadPackageJson(packageJsonPath);
        ...
        for (const [id, referencedProject] of Object.entries(referencedProjectManifests)){
            ...
            for (const [version, reference] of Object.entries(referencedProject)){
                ...
                flattenedReferences.set(key, {
                    ...
                    relativeManifestPath: Path.convertToSlashes(path.relative(this._options.rootPath, reference.manifestPath)),
        ...
        if (discoveryMode === 1 || discoveryMode === 0) {
            ...
            try {
                existingCacheFileBuffer = await FileSystem.readFileToBufferAsync(cacheFilePath);
            } catch (e) {
            ...
            if (!existingCacheFileBuffer?.equals(newCacheFileBuffer)) {
                await FileSystem.writeFileAsync(cacheFilePath, newCacheFileBuffer, {
        ...
        const manifestsArray = Array.from(flattenedReferences.values(), (reference)=>{
            ...
            ManifestUrlProcessor.processInternalModuleBaseUrls(manifest, relativePath, baseUrl);
            return manifest;
        });
        await buildManifestsJsAsync(manifestsArray, `${this._options.rootPath}/${this._options.tempFolderName}/${CUMULATIVE_MANIFEST_FILENAME}.js`, undefined, manifestsJsFileMetadata);
    }
    reset() {
    ...
    _discoverManifestsInner(packagePath, forceSearchMode, options, canSkipPackage) {
        ...
        const resolvedPackagePath = FileSystem.getRealPath(packagePath);
        ...
        const resolvedTempPath = `${resolvedPackagePath}/${this._options.tempFolderName || DEFAULT_TEMP_FOLDER}`;
        const rawTargetFolderPath = `${packagePath}/${projectTargetFolder}`;
        const resolvedTargetFolderPath = `${resolvedPackagePath}/${projectTargetFolder}`;
        const packageJsonPath = `${resolvedPackagePath}/package.json`;
        let linkedProjectManifests = undefined;
        let foundCumulativeManifestsFile = false;
        if (forceSearchMode === 0) {
            try {
                // Ensure that the actual reads happen using the fully resolved path for benefit of access tracers.
                const manifestMapPath = `${resolvedTempPath}/${CUMULATIVE_MANIFEST_FILENAME}.json`;
                const manifestsJson = FileSystem.readFile(manifestMapPath);
                ...
                for (const reference of manifestReferences){
                    ...
                    if (!isAssembly || this._options.includeAssemblies || this._options.explicitInclude.includes(manifestData.id)) {
                        ...
                        this._insertManifestIntoMap(referencedProjectManifests, path.join(preferredPackagePath, relativeManifestPath), reference.packageName ?? '', manifestCreationTime, manifestData, isAssembly);
                    }
        ...
        if (!foundCumulativeManifestsFile) {
            ...
            try {
                for (const targetFilename of FileSystem.readFolderItemNames(resolvedTargetFolderPath)){
                    if (targetFilename.match(manifestFileRegex)) {
                        const resolvedTargetFilePath = `${resolvedTargetFolderPath}/${targetFilename}`;
                        try {
                            const fileStats = FileSystem.getStatistics(resolvedTargetFilePath);
                            ...
                            const manifestData = cloneDeep(JsonFile.load(resolvedTargetFilePath));
                            ...
                            linkedProjectManifests.push({
                                ...
                                manifestData,
                                // Using the unresolved path here intentionally
                                manifestPath: `${rawTargetFolderPath}/${targetFilename}`
                            });
                        } catch (e) {
                            this._options.terminal.writeErrorLine(`Unable to process manifest file at "${resolvedTargetFilePath}", ${e}`);
                        }
                    }
                }
            } catch (e) {
            ...
            if (forceSearchMode === 2 || forceSearchMode === 4 || forceSearchMode === 1 || linkedProjectManifests.length > 0) {
                ...
                const packageJson = PackageJsonLookup.instance.loadPackageJson(`${packagePath}/package.json`);
    ...
    _resolvePackage(packageName, rootPath, resolveSymlinks) {
        ...
        let normalizedRootPath = resolveSymlinks ? FileSystem.getRealPath(rootPath) : rootPath;
        normalizedRootPath = PackageJsonLookup.instance.tryGetPackageFolderFor(normalizedRootPath) || normalizedRootPath;
        ...
        try {
            ...
            try {
                ...
                const currentPackageJson = PackageJsonLookup.instance.loadPackageJson(`${normalizedRootPath}/package.json`);
