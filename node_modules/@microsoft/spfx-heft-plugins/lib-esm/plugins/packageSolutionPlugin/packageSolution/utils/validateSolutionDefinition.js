 ...
 */ export function validateSolutionDefinition(terminal, options, customFeatures, manifests) {
    ...
    if (options.solution.metadata && options.solution.metadata.categories && options.solution.metadata.categories.length > MAX_SOLUTION_CATEGORIES_COUNT) {
        hasWarning = true;
        terminal.writeWarningLine(`You can add up to ${MAX_SOLUTION_CATEGORIES_COUNT} categories for the solution. All others will be ignored.`);
    }
    return !hasWarning;
}
