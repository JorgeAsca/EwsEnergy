...
import { MAX_SOLUTION_CATEGORIES_COUNT } from '../models/packageDefinition/ISolutionMetadata';
...
export default function createSolutionXml(terminal, xmlSerializationOptions, solution, customFeatureFilenames, resources, contentTypes) {
    solution = merge({
        version: '1.0.0.0'
    }, solution);
    // Create the top-level .rels file which points at the AppManifest
    const relationshipsXml = createRelationshipsXml(xmlSerializationOptions, '', [
        {
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const featuresXml = solution.features.map((feature)=>createFeatureXml(terminal, xmlSerializationOptions, feature, resources));
    ...
    if (includeClientAssets) {
        ...
        const clientSideFeatureXml = createFeatureXml(terminal, xmlSerializationOptions, clientSideAssetsFeature, {
    ...
    const icon = solution.iconPath ? createIcon(xmlSerializationOptions, solution.iconPath) : undefined;
    ...
    manifestXml.relationships = createRelationshipsXml(xmlSerializationOptions, constants.appManifestFileName, relsList);
    return {
...
function buildPropertiesJson(solution) {
    ...
    if (metadata) {
        ...
        if (metadata.categories && metadata.categories.length) {
            properties.push({
                CategoryID: metadata.categories.slice(0, MAX_SOLUTION_CATEGORIES_COUNT).join(',')
            });
        }
