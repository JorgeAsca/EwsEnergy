...
const bundles = REPLACE_WITH_BUNDLE_CONFIG_TOKEN;
const localization = REPLACE_WITH_LOCALIZATION_TOKEN;
/**
...
/**
 * Discovers and configures linked SharePoint Framework external dependencies.
 *
 * This function scans the project structure to find linked SPFx projects and their manifests,
 * then creates the necessary configuration for webpack externals. It processes referenced
 * projects to extract external dependencies that should not be bundled but loaded at runtime.
 *
 * @param terminal - Terminal instance for logging output during manifest discovery
 * @param rootPath - Root path of the project to scan for manifests
 *
 * @returns An object containing:
 * - `referencedProjects`: Map of discovered project manifests organized by ID and version
 * - `cumulativeManifestProcessor`: Processor instance for handling manifest operations
 * - `linkedExternals`: Map of external package names to their project metadata
 * - `externals`: Set of external package names for webpack configuration
 *
 * @example
 * ```typescript
 * const { linkedExternals, externals } = getLinkedSPFxExternals(terminal, __dirname);
 * // linkedExternals: Map { '@my/package' => { id: 'abc123', name: '@my/package', version: '1.0.0' } }
 * // externals: Set { '@my/package', '@another/package' }
 * ```
 */ export function getLinkedSPFxExternals(terminal, rootPath) {
...
export function getSPFxWebpackConfig({ production }) {
