...
import { SpfxWebpackConfigurationPlugin as PLUGIN_NAME } from '../PluginNames';
import { WebpackConfigurationGenerator } from './WebpackConfigurationGenerator';
import { readConfigFileAsync } from '../../spfxConfig/configJsonReader';
import { migrateToLatestVersion } from '../../spfxConfig/configJsonMigrator';
...
class WebpackConfigurationPlugin {
    ...
    static getSpfxWebpackFolders(buildFolder) {
        return {
            ...
            tempFolder: `${buildFolder}/${DEFAULT_TEMP_FOLDER}`,
   ...
   */ apply(taskSession, heftConfiguration) {
        ...
        taskSession.requestAccessToPluginByName('@rushstack/heft-webpack5-plugin', WEBPACK5_PLUGIN_NAME, (accessor)=>{
            accessor.hooks.onLoadConfiguration.tapPromise(PLUGIN_NAME, async ()=>{
    ...
    async _generateSpfxWebpackConfigurationAsync(taskSession, heftConfiguration, isServe, emitStats) {
        ...
        if (configJson) {
            ...
            return await WebpackConfigurationGenerator.generateWebpackConfigurationAsync({
                ...
                emitStats,
                _moduleMinifier: accessorProperties._moduleMinifier,
                _flightedDependencyConfig: accessorProperties._flightedDependencyConfig
            });
        } else {
            return undefined;
        }
    ...
    async _loadConfigJsonAsync(heftConfiguration, scopedLogger, allowMissingConfigJson) {
        ...
        const configResult = await readConfigFileAsync(scopedLogger.terminal.writeVerboseLine.bind(scopedLogger.terminal), configFilePath);
        ...
        if (!configResult.isLatestVersion && !configResult.missingFile) {
            migrateToLatestVersion(scopedLogger.terminal.writeWarningLine.bind(scopedLogger.terminal), // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            configResult.configData);
            await JsonFile.saveAsync(configResult.configData, configFilePath, {
...
export { WebpackConfigurationPlugin as default };
