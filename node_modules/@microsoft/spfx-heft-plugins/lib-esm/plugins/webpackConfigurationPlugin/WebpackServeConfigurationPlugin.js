...
import { SPFxDebugPageUrlUtilities } from './SPFxDebugPageUrlUtilities';
        ...
        taskSession.requestAccessToPluginByName('@rushstack/heft-webpack5-plugin', WEBPACK5_PLUGIN_NAME, (accessor)=>{
            if (taskSession.parameters.watch && accessor.parameters.isServeMode) {
                accessor.hooks.onConfigure.tapPromise(PLUGIN_NAME, async (config)=>{
                    ...
                    const noBrowser = taskSession.parameters.getFlagParameter(SPFxDebugPageUrlUtilities.NO_BROWSER_CLI_PARAMETER).value;
                    const serveConfigName = taskSession.parameters.getStringParameter(SPFxDebugPageUrlUtilities.CONFIG_CLI_HEFT_PARAMETER).value;
                    return updateServeConfigAsync(taskSession, heftConfiguration, additionalServePaths, config, noBrowser, serveConfigName);
                });
            }
        });
    }
