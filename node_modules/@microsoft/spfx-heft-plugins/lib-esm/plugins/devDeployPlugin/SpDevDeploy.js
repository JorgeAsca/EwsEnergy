...
import { FileSystem, PackageJsonLookup, Async } from '@rushstack/node-core-library';
import { ConfigurationFile } from '@rushstack/heft-config-file';
import { printDevDeployLink } from './DevDeployLinkPrinter';
import { UrlUtilities } from '../../utilities/UrlUtilities';
import { buildManifestsJsAsync } from '../../spfxManifests/manifestFileBuilder/ManifestsFileBuilder';
...
import { DEFAULT_LOCALE, DEFAULT_TEMP_FOLDER, SP_LOADER_ASSEMBLY_ID } from '../../utilities/constants';
import { IconHandler, IconPathType } from '../../spfxManifests/utilities/IconHandler';
import { getDevDeployContainerDirectoryName } from '../../utilities/devDeployUtilities';
import schema from './sp-dev-deploy.schema.json';
const MANIFESTS_JS_FILENAME = 'manifests.js';
/**
 * @public
 */ export class SpDevDeploy {
    static get _spDevDeployConfigFile() {
        if (!SpDevDeploy.__spDevDeployConfigFile) {
            SpDevDeploy.__spDevDeployConfigFile = new ConfigurationFile({
    ...
    async getFilesAsync() {
            ...
            if (manifests.hasOwnProperty(id)) {
                for(const version in manifests[id]){
                    if (manifests[id].hasOwnProperty(version)) {
                        ...
                        const isBaseUrlRelative = loaderConfig.internalModuleBaseUrls.length > 0 && !Url.parse(loaderConfig.internalModuleBaseUrls[0]).protocol;
                        ...
                        const processPath = (resourcePath)=>{
                            ...
                            if (UrlUtilities.isUrl(resourcePath) || isBaseUrlRelative) {
                                ...
                                return UrlUtilities.joinUrlSegments(projectContainerName, filename);
                            }
                        };
        ...
        const filesToUpload = [];
        await Async.forEachAsync(foldersToUpload, async ([localDirPath, deployDirName])=>{
        ...
        await buildManifestsJsAsync(manifestsArray, manifestsFileFullPath, undefined, this._options._manifestsJsFileMetadata);
        filesToUpload.push({
            localPath: manifestsFileFullPath,
            deployPath: MANIFESTS_JS_FILENAME
        });
        const deployBaseUrl = this._getDeployBaseUrl();
        this._devDeployManifestFileUrl = UrlUtilities.joinUrlSegments(deployBaseUrl, SpDevDeploy._debugManifestPath);
        if (spLoaderFolderPath && debugSPLoader) {
            this._devDeployModuleLoaderUrl = UrlUtilities.joinUrlSegments(deployBaseUrl, spLoaderFolderPath, debugSPLoader);
        }
        return {
            filesToUpload,
    ...
    async *_getFilesInFolderInnerAsync(localDirPath, deployDirName) {
        const localDirEntries = await FileSystem.readFolderItemsAsync(localDirPath);
        for (const localDirEntry of localDirEntries){
            const localDirEntryName = localDirEntry.name;
            const deployPath = UrlUtilities.joinUrlSegments(deployDirName, localDirEntryName);
    ...
    _getDeployBaseUrl() {
        ...
        return UrlUtilities.joinUrlSegments(containerUrlForRead, folderNameInContainer ?? '');
    }
                    ...
                    case 'localizedPath':
                        {
                            ...
                            const pathToProcess = (localizedPathResource.paths ? localizedPathResource.paths[DEFAULT_LOCALE] : localizedPathResource.defaultPath) || localizedPathResource.defaultPath;
                            if (!pathToProcess) {
   ...
   */ _getDestinationDirName(directoryMap, filesystemPath) {
        const projectFolder = PackageJsonLookup.instance.tryGetPackageFolderFor(filesystemPath);
        if (!projectFolder) {
    ...
    _fixupIconPath(iconPath, processPath) {
        if (IconHandler.getIconPathType(iconPath) === IconPathType.RelativeUrl) {
    ...
    constructor(options, configFileData){
        ...
        this._options = {
            ...
            _projectDiscoveryMode: DependencyDiscoveryMode.deepSparse,
            ...options
        };
