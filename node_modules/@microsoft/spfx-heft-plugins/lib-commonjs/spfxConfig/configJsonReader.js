    ...
    try {
        ...
        if (v1Config.entries) {
            for (const entry of v1Config.entries){
                const name = _nodepath.basename(entry.outputPath, _nodepath.extname(entry.outputPath));
        ...
        if (configData.localizedResources) {
            for (const resourceName of Object.getOwnPropertyNames(configData.localizedResources)){
                ...
                configData.localizedResources[resourceName] = _nodepath.posix.join('lib', resourcePath);
            }
        }
...
async function readConfigFileAsync(logVerbose, filename) {
    ...
    logVerbose(`Found config file: ${_nodepath.basename(filename)}`);
