...
Object.defineProperty(exports, "migrateToLatestVersion", {
    enumerable: true,
    get: function() {
        return migrateToLatestVersion;
    }
});
...
function migrateToLatestVersion(logWarning, configJson) {
    const bundleNameRegexString = Object.keys(_config20schemajson.default.properties.bundles.patternProperties)[0];
    ...
    for(const bundleName in configJson.bundles){
        if (bundleName.match(bundleNameRegex)) {
            ...
            if (bundles[newBundleName]) {
                ...
                throw new Error(`Bundle name "${bundleName}" translates to the same name ("${newBundleName}") as another bundle name when ` + 'the name is migrated to be compatible with the latest version of SPFx. One of the names must be changed.');
            }
    ...
    if (modifiedNames.length) {
        logWarning('Bundle names have been migrated to follow the V2 config.json schema. Modified bundle names: ' + `[${modifiedNames.join(', ')}]`);
    }
    if (tooShortNames.length) {
        logWarning('One or more of your bundle names are too short (less than 6 characters). Please update them: ' + `[${tooShortNames.join(', ')}]`);
    }
