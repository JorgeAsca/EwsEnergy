 ...
 */ "default", {
    enumerable: true,
    get: function() {
        return normalizeSolutionDefinition;
    }
});
...
function resolveFeatureFile(filename, basePath) {
    ...
    if (!_nodecorelibrary.FileSystem.exists(basePath)) {
        //
        // Import.resolveModule fails if the baseFolderPath doesn't exist.
        // But it will not if the modulePath is an absolute path
        //
        modulePath = _nodepath.join(basePath, filename);
    }
    try {
        return _nodecorelibrary.Import.resolveModule({
        ...
        if (!_nodepath.isAbsolute(modulePath) && !modulePath.startsWith('.')) {
            return _nodecorelibrary.Import.resolveModule({
                modulePath: `./${modulePath}`,
                baseFolderPath: basePath
            });
