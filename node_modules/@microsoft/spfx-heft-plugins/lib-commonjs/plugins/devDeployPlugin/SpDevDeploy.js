...
class SpDevDeploy {
    ...
    async getFilesAsync() {
            ...
            if (manifests.hasOwnProperty(id)) {
                for(const version in manifests[id]){
                    if (manifests[id].hasOwnProperty(version)) {
                        ...
                        const isBaseUrlRelative = loaderConfig.internalModuleBaseUrls.length > 0 && !_nodeurl.parse(loaderConfig.internalModuleBaseUrls[0]).protocol;
                        // If the Base URL is a relative path, keep the relative path
                        if (!isBaseUrlRelative) {
                            loaderConfig.internalModuleBaseUrls = [];
                        }
                        const manifestFolderPath = _nodepath.dirname(referencedProject.manifestPath);
                        const processPath = (resourcePath)=>{
                            ...
                            if (_UrlUtilities.UrlUtilities.isUrl(resourcePath) || isBaseUrlRelative) {
                                ...
                                const filesystemPath = _nodepath.join(manifestFolderPath, resourcePath);
                                const filename = _nodepath.basename(resourcePath);
                                ...
                                const filesystemFolder = _nodepath.dirname(filesystemPath);
        ...
        const manifestsFileFullPath = _nodepath.join(this._options.rootPath, 'temp', 'dev-deploy', MANIFESTS_JS_FILENAME);
   ...
   */ _getDestinationDirName(directoryMap, filesystemPath) {
        ...
        let destinationDirName = _nodepath.basename(projectFolder).toLowerCase();
    ...
    _doesFilenameAppearToContainHash(filePath) {
        const filename = _nodepath.basename(filePath, _nodepath.extname(filePath));
