...
Object.defineProperty(exports, "LegacyExternals", {
    enumerable: true,
    get: function() {
        return LegacyExternals;
    }
});
...
const ESCAPED_TOKEN_REGEXP = new RegExp((0, _lodash.escapeRegExp)((0, _lodash.escapeRegExp)(LOCALE_TOKEN)));
// eslint-disable-next-line @rushstack/security/no-unsafe-regexp
const ESCAPED_TOKEN_GLOBAL_REGEXP = new RegExp((0, _lodash.escapeRegExp)((0, _lodash.escapeRegExp)(LOCALE_TOKEN)), 'g');
// eslint-disable-next-line @rushstack/security/no-unsafe-regexp
...
class LegacyExternals {
    updateWebpackConfiguration(webpackConfiguration) {
        ...
        for (const plugin of webpackConfiguration.plugins){
            if (plugin instanceof _ManifestPlugin.ManifestPlugin) {
                plugin._options._tryGetLegacyExternalByNameAsync = async (name)=>{
    ...
    async _collectResourcesForKeyAsync(rawPattern) {
        ...
        if (!pattern.match(LOCALE_TOKEN_REGEXP)) {
            ...
            const fullPath = _nodepath.join(this._buildFolder, pattern);
            if (await _nodecorelibrary.FileSystem.existsAsync(fullPath)) {
            ...
            const files = await (0, _fastglob.default)(globPattern, {
            ...
            if (!files || files.length === 0) {
                ...
                const normalizedPattern = _nodepath.posix.resolve('/', pattern).substr(1);
                // eslint-disable-next-line @rushstack/security/no-unsafe-regexp
                const regexPattern = new RegExp((0, _lodash.escapeRegExp)(normalizedPattern).replace(ESCAPED_TOKEN_REGEXP, '(.+)').replace(ESCAPED_TOKEN_GLOBAL_REGEXP, '(?:\\1)'));
                const result = {};
                for (const file of files){
                    ...
                    if (!match) {
                        ...
                        result[locale] = _nodepath.join(this._buildFolder, file);
                    }
                }
                return result;
            }
        ...
        if (!result) {
            ...
            if (localizedFiles) {
                ...
                for (const [localeName, filePath] of Object.entries(localizedFiles)){
                    const fileContents = await _nodecorelibrary.FileSystem.readFileAsync(filePath);
                    const assetFilenameParts = [
                        `${externalName}_${localeName}`
                    ];
                    if (!this._serveMode && this._production) {
                        const fileHash = (0, _getFullHash.getFullHash)(fileContents, hashSalt);
                        assetFilenameParts.push(`_${fileHash}`);
                    }
                    assetFilenameParts.push(_nodepath.extname(filePath));
                ...
                if (singleLocaleName) {
                    ...
                    manifestModule = {
                        ...
                        defaultPath: _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.findDefaultString(manifestModulePaths)
                    };
                }
                ...
                const includeExternalAsync = async (name, externalModule)=>{
                    ...
                    if (/^https?\:/.test(externalPath)) {
                        ...
                        let fileContents;
                        const externalFullPath = _nodepath.join(this._buildFolder, externalPath);
                        try {
                            fileContents = await _nodecorelibrary.FileSystem.readFileAsync(externalPath);
                        } catch (e) {
                        ...
                        if (!this._serveMode) {
                            const fileHash = (0, _getFullHash.getFullHash)(fileContents);
                            assetFilenameParts.push(`_${fileHash}`);
                        }
                        assetFilenameParts.push(_nodepath.extname(externalPath));
                        const assetFilename = assetFilenameParts.join('');
