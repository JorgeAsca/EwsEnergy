...
_export(exports, {
    COMMON_RULE_OPTIONS: function() {
        return COMMON_RULE_OPTIONS;
    },
    CONFIG_JSON_SYMBOL: function() {
        return CONFIG_JSON_SYMBOL;
    },
    FILE_LOADER_EXTENSIONS: function() {
        return FILE_LOADER_EXTENSIONS;
    },
    FILE_LOADER_RULE_TEST: function() {
        return FILE_LOADER_RULE_TEST;
    },
    IS_SPFX_WEBPACK_CONFIGURATION_SYMBOL: function() {
        return IS_SPFX_WEBPACK_CONFIGURATION_SYMBOL;
    },
    WebpackConfigurationGenerator: function() {
        return WebpackConfigurationGenerator;
    }
});
...
const _webpack5moduleminifierplugin = require("@rushstack/webpack5-module-minifier-plugin");
...
const _CopyReleaseAssetsPlugin = require("./webpackPlugins/CopyReleaseAssetsPlugin");
const _WebpackStatsPlugin = require("./webpackPlugins/WebpackStatsPlugin");
const _AsyncComponentPlugin = require("./webpackPlugins/AsyncComponentPlugin");
const _CumulativeManifestProcessor = require("../../spfxManifests/cumulativeManifestProcessor/CumulativeManifestProcessor");
const _ExternalsProcessor = require("../../spfxManifests/utilities/ExternalsProcessor");
const _getFullHash = require("../../spfxManifests/utilities/getFullHash");
const _constants = require("../../utilities/constants");
const _ManifestPlugin = require("../../spfxManifests/webpack/ManifestPlugin");
const _VariableNameGenerator = require("../../utilities/VariableNameGenerator");
...
const IS_SPFX_WEBPACK_CONFIGURATION_SYMBOL = Symbol('IS_SPFX_WEBPACK_CONFIGURATION');
const CONFIG_JSON_SYMBOL = Symbol('CONFIG_JSON');
const FILE_LOADER_RULE_TEST = {
    or: FILE_LOADER_EXTENSIONS.map(// eslint-disable-next-line @rushstack/security/no-unsafe-regexp
    (ext)=>new RegExp(`\\.${_nodecorelibrary.Text.escapeRegExp(ext)}((\\?|\\#).+)?$`))
};
...
const DEFAULT_GENERATE_CSS_CLASS_NAME = (existingClassName, cssFilename, cssContent)=>{
    ...
    if (!hash) {
        hash = _nodecrypto.createHmac('sha1', cssFilename).update(cssContent).digest('hex').substring(0, 8);
...
class WebpackConfigurationGenerator {
    static async generateWebpackConfigurationAsync(options) {
        ...
        const distFolderName = _nodepath.relative(buildFolder, outputFolder).replace(/^\.[\\\/]/, '');
        const tempFolderName = _nodepath.relative(buildFolder, tempFolder).replace(/^\.[\\\/]/, '');
        ...
        const referencedProjects = cumulativeManifestProcessor.discoverManifests(options.folders.buildFolder, options._useManifestsJsonForComponentDependencies ? _CumulativeManifestProcessor.DependencyDiscoveryMode.shallow : _CumulativeManifestProcessor.DependencyDiscoveryMode.deepSparseIgnoreFirstProject);
        ...
        const nonStandardExternals = (0, _ExternalsProcessor.getNonStandardExternals)(options.folders.buildFolder, referencedProjects);
        ...
        for (const [bundleName, configEntry] of Object.entries(configBundleEntries)){
            ...
            for (const componentEntry of configEntry.components){
                const manifestData = await _nodecorelibrary.JsonFile.loadAsync(_nodepath.resolve(options.folders.buildFolder, componentEntry.manifest));
                ...
                if (manifestData.version === '*') {
                    const packageJson = _nodecorelibrary.PackageJsonLookup.instance.tryLoadPackageJsonFor(options.folders.buildFolder);
                    if (!packageJson) {
                ...
                bundleEntry.components[manifestData.id] = {
                    ...componentEntry,
                    manifestData,
                    manifestPath: _nodepath.join(options.folders.buildFolder, componentEntry.manifest),
            ...
            if (components.length === 1) {
                ...
                entryImport = await _nodecorelibrary.Import.resolveModuleAsync({
        ...
        if (webpackUniqueNameParts.length === 1) {
            ...
            webpackUniqueName = (0, _getFullHash.getFullHash)(webpackUniqueNameParts.join(''));
        }
        ...
        if (options.emitStats) {
            plugins.push(new _WebpackStatsPlugin.WebpackStatsPlugin({
                dropPath: _nodepath.resolve(options.folders.releaseFolder, 'webpack-stats', `${_nodepath.basename(options.folders.buildFolder)}.stats.json`)
            }));
        }
        if (!options.disableAsyncComponentLoading) {
            ...
            plugins.push(new _AsyncComponentPlugin.AsyncComponentPlugin({
        ...
        if (options.getLocalization) {
            ...
            if (localizedDataKeys.length > 0) {
                if (options.selectedLocales) {
                    for (const selectedLocale of options.selectedLocales){
                        if (selectedLocale !== _constants.DEFAULT_LOCALE && !localizedData.hasOwnProperty(selectedLocale)) {
            ...
            const { localizationPlugin, stringsLoaders, additionalPlugins } = await localizationPackage.getLocalizationConfigurationAsync(selectedLocales);
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            config.module.rules.push(...stringsLoaders);
            plugins.push(localizationPlugin);
            additionalPlugins?.forEach((plugin)=>plugins.push(plugin));
        }
        let serveConfig = {};
        try {
            serveConfig = await _nodecorelibrary.JsonFile.loadAsync(_nodepath.join(options.folders.buildFolder, 'config', 'serve.json'));
        } catch (e) {
        ...
        let cdnBaseUrl;
        try {
            const writeManifestsConfig = await _nodecorelibrary.JsonFile.loadAsync(_nodepath.join(options.folders.buildFolder, 'config', 'write-manifests.json'));
            cdnBaseUrl = writeManifestsConfig.cdnBasePath;
        } catch (e) {
            if (!_nodecorelibrary.FileSystem.isFileDoesNotExistError(e)) {
                throw e;
            }
        }
        plugins.push(new _ManifestPlugin.ManifestPlugin({
            terminal,
            selectedLocales,
            production: options.production,
            includeIntegrity: options._includeIntegrityInManifestPaths,
            sourceLocaleName: _constants.DEFAULT_LOCALE,
            bundleEntries,
            ...
            _asyncOnlyDependencies: {
                asyncOnlyDependencyNames: options.configJson.asyncComponents || [],
                violationsAsErrors: options.production
            },
            componentDependenciesAuditDropPath: _nodepath.join(options.folders.releaseFolder, 'component-dependency-audit', `${_nodepath.basename(options.folders.buildFolder)}.json`),
            _manifestsJsFileMetadata: options._manifestsJsFileMetadata,
            _flightedDependencyConfig: options._flightedDependencyConfig
        }));
        if (options.production) {
            let minifier = options._moduleMinifier;
            if (!minifier) {
                const { WorkerPoolMinifier } = await Promise.resolve().then(()=>/*#__PURE__*/ _interop_require_wildcard(require("@rushstack/module-minifier")));
                minifier = new WorkerPoolMinifier({
                    // Cap the number of worker processes to avoid CPU saturation.
                    maxThreads: Math.min(8, _nodeos.cpus().length),
                    terserOptions: {
                        compress: {
                            passes: 3
                        },
            ...
            config.optimization.minimizer = [
                new _webpack5moduleminifierplugin.ModuleMinifierPlugin({
                    minifier,
                    sourceMap: !!devtool
                })
            ];
        ...
        plugins.push(new _CopyReleaseAssetsPlugin.CopyReleaseAssetsPlugin({
   ...
   */ static async _generateBundleEntrypointFileAsync(options) {
        ...
        const variableNameGenerator = await _VariableNameGenerator.VariableNameGenerator.initializeAsync();
        const fileLines = [];
        for (const bundleComponent of Object.values(options.bundleEntry.components)){
            const fullEntrypointPath = await _nodecorelibrary.Import.resolveModuleAsync({
                modulePath: bundleComponent.entrypoint,
                baseFolderPath: options.buildFolder
            });
            const relativeEntrypointPath = _nodepath.relative(entrypointFileFolderPath, fullEntrypointPath);
            ...
            fileLines.push(`export * as ${exportName} from '${_nodecorelibrary.Path.convertToSlashes(relativeEntrypointPath)}';`);
            fileLines.push('');
        }
        const fileContents = fileLines.join('\n');
        await _nodecorelibrary.FileSystem.writeFileAsync(filePath, fileContents, {
            ensureFolderExists: true
        });
        return filePath;
    }
    static _requireResolveWithCorrectCase(packageName) {
        const resolveResult = _nodecorelibrary.Import.resolvePackage({
            packageName,
            baseFolderPath: __dirname,
            // fs.realpathSync.native fixes casing issues on Windows
            getRealPath: (filePath)=>{
                try {
                    return _nodefs.realpathSync.native(filePath);
                } catch (error) {
        ...
        if (configJson.bundles) {
            ...
            for (const [, bundleEntry] of Object.entries(configJson.bundles)){
                for (const component of bundleEntry.components){
                    const entrypointPath = _nodecorelibrary.Import.resolveModule({
                    ...
                    const manifestPath = _nodepath.resolve(_nodepath.join(rootPath, component.manifest)).toUpperCase();
