...
class ConfigureServe {
    static loadFromServeConfig(terminal, heftConfiguration) {
        const configPath = `${heftConfiguration.buildFolderPath}/config/serve.json`;
        try {
            const serveConfig = (0, _serveJsonReader.readServeConfigFile)(configPath);
            return new ConfigureServe(terminal, heftConfiguration, serveConfig);
        } catch (err) {
            terminal.writeErrorLine('Unable to parse serve config file.', String(err));
            throw new Error('Serve configuration could not be found or parsed.');
        }
    }
    generateServeConfig() {
        const serveConfig = {
            hostname: this._serveConfig?.ipAddress ?? 'localhost',
        ...
        if (this._serveConfig.api) {
            let apiMap = require(_nodepath.join(this._buildFolder, this._serveConfig.api.entryPath));
            if (apiMap.default) {
                apiMap = apiMap.default;
            }
            serveConfig._apiMap = apiMap;
        }
        return serveConfig;
    }
    ...
    constructor(terminal, configuration, serveConfig){
