...
class IconHandler {
    ...
    static processIconPath(terminal, compilation, manifestPath, iconPath) {
        ...
        switch(iconPathType){
            case 3:
                {
                    const iconAbsolutePath = _nodepath.resolve(_nodepath.dirname(manifestPath), iconPath || '');
                    ...
                    if (!_nodecorelibrary.FileSystem.exists(iconAbsolutePath)) {
                        ...
                        const ext = _nodepath.extname(iconAbsolutePath);
                        const iconBaseName = _nodepath.basename(iconAbsolutePath, ext);
