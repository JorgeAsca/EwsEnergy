...
Object.defineProperty(exports, "LocalizedManifestModuleProcessor", {
    enumerable: true,
    get: function() {
        return LocalizedManifestModuleProcessor;
    }
});
const _normalizeLocaleName = require("../../utilities/normalizeLocaleName");
const _ManifestPlugin = require("./ManifestPlugin");
...
function _getManifestPathOrIntegrityPath(localizedPath, bundleName, assets) {
    if (typeof localizedPath !== 'string') {
        throw new Error(`The "${bundleName}" bundle is generating integrity paths, but appears to already have an integrity path. ` + 'This is not supported.');
    }
    const asset = assets[localizedPath];
    if (!asset) {
        throw new Error(`Unable to find the "${localizedPath}" asset for the "${bundleName}" bundle.`);
    }
    const integrity = (0, _ManifestPlugin.computeIntegrity)(asset.buffer());
    return {
        path: localizedPath,
        integrity
    };
...
class LocalizedManifestModuleProcessor {
    static generateLocalizedManifestModule({ bundleName, localizedPaths, firstPassLocaleRemapping, production, selectedLocales, assets, generateIntegrity }) {
        ...
        if (localeNames.length === 1) {
            ...
            if (!defaultPath) {
                throw new Error(`Unable to find a default localized entry for the "${bundleName}" bundle. Tried to match ` + `${JSON.stringify(DEFAULT_LOCALES)}.`);
            ...
            for (const [localeName, localizedPath] of Object.entries(localizedPaths)){
                ...
                const path = generateIntegrity ? _getManifestPathOrIntegrityPath(localizedPath, bundleName, assets) : localizedPath;
            ...
            if (production || selectedLocales?.size) {
                ...
                if (selectedLocales?.size && !production) {
...
_define_property(LocalizedManifestModuleProcessor, "LOCALE_MAPPING", new Map([
    [
        'ar-sa',
        [
            'ar',
...
_define_property(LocalizedManifestModuleProcessor, "LOCALE_FALLBACK", new Map());
