...
const _localizationutilities = require("@rushstack/localization-utilities");
const _CumulativeManifestProcessor = require("../cumulativeManifestProcessor/CumulativeManifestProcessor");
const _LocalizedManifestModuleProcessor = require("./LocalizedManifestModuleProcessor");
const _IconHandler = require("../utilities/IconHandler");
const _TerminalHelper = require("../utilities/TerminalHelper");
const _normalizeLocaleName = require("../../utilities/normalizeLocaleName");
const _AsyncComponentPlugin = require("../../plugins/webpackConfigurationPlugin/webpackPlugins/AsyncComponentPlugin");
const _ManifestReferenceResolver = require("./ManifestReferenceResolver");
...
function computeIntegrity(source) {
    const hash = _nodecrypto.default.createHash(INTEGRITY_ALGORITHM_NAME);
...
class ManifestPlugin {
    apply(compiler) {
        compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation)=>{
            ...
            compilation.hooks.processAssets.tapPromise({
                ...
                const asyncOnlyDependencies = new Set(this._options._asyncOnlyDependencies?.asyncOnlyDependencyNames || []);
                const asyncOnlyDependenciesViolationsAsErrors = !!this._options._asyncOnlyDependencies?.violationsAsErrors;
                const terminal = _TerminalHelper.TerminalHelper.getTerminalForWebpackPlugin(compilation);
                ...
                for (const bundleEntry of this._options.bundleEntries){
                    try {
                        ...
                        for (const [componentId, bundleComponent] of Object.entries(bundleEntry.components)){
                            const manifest = await this._generateComponentManifest(terminal, thisWebpack, compilation, bundleComponent, baseLoaderConfiguration);
                ...
                if (this._options.componentDependenciesAuditDropPath) {
                    for (const untypedChunk of compilation.chunks){
                        ...
                        if (chunk.hasRuntime()) {
                            const asyncChunks = {};
                            if (chunk[_AsyncComponentPlugin.CHUNK_COMPONENT_MAP]) {
                                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                                for (const [asyncChunk, externalComponents] of chunk[_AsyncComponentPlugin.CHUNK_COMPONENT_MAP].entries()){
                                    const dependenciesForAudit = new Map();
                    ...
                    await _nodecorelibrary.JsonFile.saveAsync(componentDependenciesAuditFile, this._options.componentDependenciesAuditDropPath, {
                        ensureFolderExists: true,
                        ignoreUndefinedValues: true
                    });
                }
                await this._options.cumulativeManifestProcessor.generateCumulativeManifestAsync(producedManifests, this._options._manifestsJsFileMetadata, this._options.useManifestsJsonForComponentDependencies ? _CumulativeManifestProcessor.DependencyDiscoveryMode.shallow : _CumulativeManifestProcessor.DependencyDiscoveryMode.deepSparseIgnoreFirstProject);
            });
        });
    }
    /**
   ...
   */ async _getExternalsScriptResourcesAsync(terminal, thisWebpack, bundleName, compilation, runtimeChunk, asyncOnlyDependencies, asyncOnlyDependenciesViolationsAsErrors, dependenciesForAudit) {
        ...
        for (const module of compilation.chunkGraph.getChunkModules(runtimeChunk)){
            if ((0, _AsyncComponentPlugin.isExternalModule)(module)) {
                ...
                if (externalComponent) {
                    ...
                    const flightedDependency = this._options._flightedDependencyConfig?.[id];
                    if (flightedDependency) {
                        externalComponentModule.version = flightedDependency.activeVersion;
                        externalComponentModule.fallbackVersion = {
                            version: flightedDependency.fallbackVersion,
                            condition: flightedDependency.condition
                        };
                    }
                    ...
                    const legacyExternal = this._options._tryGetLegacyExternalByNameAsync ? await this._options._tryGetLegacyExternalByNameAsync(externalName) : undefined;
                    if (legacyExternal) {
   ...
   */ _getRuntimeChunkInfo(bundleName, runtimeChunk, assets) {
        ...
        for (const file of runtimeChunk.files){
            const isCandidate = _nodepath.extname(file) === '.js' && // TODO - figure out if there is a way to determine this from module metadata
    ...
    async _generateComponentManifest(terminal, thisWebpack, compilation, bundleComponent, baseLoaderConfiguration) {
        const loaderConfig = {
        ...
        const inputManifest = _lodash.cloneDeep(bundleComponent.manifestData);
        delete inputManifest.$schema;
        switch(inputManifest.componentType){
            case 'AdaptiveCardExtension':
            case 'WebPart':
                {
                    await this._processWebpartManifest(terminal, thisWebpack, compilation, bundleComponent.manifestPath, inputManifest);
                    break;
                }
            case 'Application':
                {
    ...
    async _processAIProperties(terminal, thisWebpack, compilation, manifestPath, aiProperties) {
        if (!aiProperties) {
            return;
        }
        try {
            // Process each AI property that might be a reference
            for (const [key, value] of Object.entries(aiProperties)){
                if (this._isManifestReference(value)) {
                    const processedValue = await this._manifestReferenceResolver.processManifestReference(thisWebpack, compilation, manifestPath, value);
                    if (processedValue) {
                        aiProperties[key] = processedValue;
                    }
                }
            }
        } catch (error) {
            terminal.writeWarningLine(`AI Properties processing warning for ${manifestPath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    _isManifestReference(value) {
        return typeof value === 'object' && value !== null && 'id' in value && typeof value.id === 'string' && value.id.startsWith('$');
    }
    /**
   ...
   */ async _processpropertiesMetadataForReInstate(terminal, thisWebpack, compilation, manifestPath, manifest) {
        // Only process for opted-in components
        const experimentalData = manifest.experimentalData;
        const hasOptInAIProperties = !!(manifest.aiProperties || experimentalData?.aiProperties);
        if (!hasOptInAIProperties) {
            return;
        }
        if (!experimentalData) {
            return;
        }
        const runtimeRef = experimentalData.propertiesMetadataForReInstate;
        if (!this._isManifestReference(runtimeRef)) {
            return;
        }
        try {
            const processed = await this._manifestReferenceResolver.processManifestReference(thisWebpack, compilation, manifestPath, runtimeRef);
            if (processed && !(typeof processed === 'object' && 'id' in processed)) {
                // Replace the reference with the resolved object
                experimentalData.propertiesMetadataForReInstate = processed;
            }
        } catch (error) {
            terminal.writeWarningLine(`propertiesMetadataForReInstate processing warning for ${manifestPath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
   * Process and localize WebPart manifest.
   * @remarks Also used for AdaptiveCardExtension manifests
   */ async _processWebpartManifest(terminal, thisWebpack, compilation, manifestPath, manifest) {
        // read aiProperties from experimentalData.aiProperties if top-level is absent,
        // and always inject processed values back into experimentalData.aiProperties.
        const experimentalData = manifest.experimentalData;
        const aiPropsSource = manifest.aiProperties || experimentalData?.aiProperties;
        // Process AI Properties if they exist (from either top-level or experimentalData)
        await this._processAIProperties(terminal, thisWebpack, compilation, manifestPath, aiPropsSource);
        // Ensure experimentalData container exists when we need to inject
        if (aiPropsSource) {
            if (!manifest.experimentalData) {
                manifest.experimentalData = {};
            }
            // Mirror processed AI properties into experimentalData.aiProperties for current consumption
            manifest.experimentalData.aiProperties = aiPropsSource;
        }
        // If opted into AI, process propertiesMetadataForReInstate reference under experimentalData
        await this._processpropertiesMetadataForReInstate(terminal, thisWebpack, compilation, manifestPath, manifest);
        for (const entry of manifest.preconfiguredEntries){
            ...
            entry.iconImageUrl = _IconHandler.IconHandler.processIconPath(this._options.terminal, compilation, manifestPath, entry.iconImageUrl);
        }
    }
            ...
            case 'ListViewCommandSet':
                ...
                for(const itemId in commandSetManifest.items){
                    if (commandSetManifest.items.hasOwnProperty(itemId)) {
                        ...
                        item.iconImageUrl = _IconHandler.IconHandler.processIconPath(this._options.terminal, compilation, manifestPath, item.iconImageUrl);
                    }
                }
                break;
            case 'Unknown':
   ...
   */ _localizeString(terminal, thisWebpack, compilation, manifestPath, stringToLocalize) {
        ...
        if (matches?.length === 3) {
            ...
            const locFileAbsolutePath = _nodepath.resolve(_nodepath.dirname(manifestPath), locFilePath);
            if (!_nodecorelibrary.FileSystem.exists(locFileAbsolutePath)) {
                ...
                const normalizedSourceLocaleName = (0, _normalizeLocaleName.normalizeLocaleName)(this._options.sourceLocaleName);
                ...
                if (!locFile) {
                    ...
                    if (!locFile) {
                        const parsedSourceFile = (0, _localizationutilities.parseLocFile)({
                            filePath: locFileAbsolutePath,
                            content: _nodecorelibrary.FileSystem.readFile(locFileAbsolutePath),
                            terminal,
                            resxNewlineNormalization: _nodecorelibrary.NewlineKind.Lf,
                ...
                if (!locFile[stringName]) {
                    ...
                    for (const localeName of selectedLocales){
                        ...
                        const normalizedLocaleName = (0, _normalizeLocaleName.normalizeLocaleName)(localeName);
                    ...
                    if (this._options.firstPassLocaleRemapping) {
                        _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.applyLocaleRemapping(localizedStrings, this._options.firstPassLocaleRemapping);
                    }
                    _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.applyLocaleRemapping(localizedStrings, _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.LOCALE_MAPPING);
                    // Do this twice because some locales (like en-gb) appear both as values and as keys
                    _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.applyLocaleRemapping(localizedStrings, _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.LOCALE_MAPPING);
                    const defaultString = _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.findDefaultString(localizedStrings);
                    if (defaultString) {
    ...
    _getEntryManifestModule(bundleName, runtimeChunk, assets) {
        if (runtimeChunk.localizedFiles) {
            ...
            return _LocalizedManifestModuleProcessor.LocalizedManifestModuleProcessor.generateLocalizedManifestModule({
                bundleName,
   ...
   */ _define_property(this, "_options", void 0);
        ...
        // Resolves $<path>:<export>; style module references to JSON-serializable values
        _define_property(this, "_manifestReferenceResolver", void 0);
        this._options = options;
        this._manifestReferenceResolver = new _ManifestReferenceResolver.ManifestReferenceResolver(options.terminal);
    }
}
