...
class CumulativeManifestProcessor {
   ...
   */ async generateCumulativeManifestAsync(debugManifests, manifestsJsFileMetadata, discoveryMode) {
        ...
        for (const [id, referencedProject] of Object.entries(referencedProjectManifests)){
            ...
            for (const [version, reference] of Object.entries(referencedProject)){
                ...
                flattenedReferences.set(key, {
                    ...
                    relativeManifestPath: _nodecorelibrary.Path.convertToSlashes(_nodepath.relative(this._options.rootPath, reference.manifestPath)),
        ...
        const manifestsArray = Array.from(flattenedReferences.values(), (reference)=>{
            const relativePath = _nodepath.dirname(reference.relativeManifestPath);
    ...
    _discoverManifestsInner(packagePath, forceSearchMode, options, canSkipPackage) {
        ...
        const resolvedTempPath = `${resolvedPackagePath}/${this._options.tempFolderName || _constants.DEFAULT_TEMP_FOLDER}`;
            ...
            try {
                ...
                for (const reference of manifestReferences){
                    ...
                    if (!isAssembly || this._options.includeAssemblies || this._options.explicitInclude.includes(manifestData.id)) {
                        ...
                        this._insertManifestIntoMap(referencedProjectManifests, _nodepath.join(preferredPackagePath, relativeManifestPath), reference.packageName ?? '', manifestCreationTime, manifestData, isAssembly);
                    }
        ...
        if (!foundCumulativeManifestsFile) {
            ...
            try {
                for (const targetFilename of _nodecorelibrary.FileSystem.readFolderItemNames(resolvedTargetFolderPath)){
                    if (targetFilename.match(manifestFileRegex)) {
                        const resolvedTargetFilePath = `${resolvedTargetFolderPath}/${targetFilename}`;
                        try {
                            const fileStats = _nodecorelibrary.FileSystem.getStatistics(resolvedTargetFilePath);
                            ...
                            const manifestData = (0, _lodash.cloneDeep)(_nodecorelibrary.JsonFile.load(resolvedTargetFilePath));
    ...
    _resolvePackage(packageName, rootPath, resolveSymlinks) {
        ...
        try {
            return _nodepath.dirname(_resolve.sync(normalizedPackageName, {
