...
import { _ClientManifestData, Validate } from '@microsoft/sp-core-library';
import { _QosMonitor } from '@microsoft/sp-diagnostics';
import { Client as _InternalGraphClient, AuthenticationHandler, MiddlewareFactory } from '@msinternal/ms-graph-v3-bundle';
import { _GraphHttpClientContext, _AadTokenProviders } from '@microsoft/sp-http-base';
import { isCustomHostsKSActivated, isPassSdkVersionKSActivated } from './common/Killswitches';
import { MSGraphAuthenticationProvider } from './MSGraphAuthenticationProvider';
/**
...
var MSGraphClientV3 = /** @class */ (function () {
        ...
        serviceScope.whenFinished(function () {
            if (!MSGraphClientV3._graphBaseUrl) {
                MSGraphClientV3._graphBaseUrl = serviceScope
                    .consume(_GraphHttpClientContext.serviceKey)
                    .getGraphEndpointUrl();
            }
            if (!isPassSdkVersionKSActivated()) {
            ...
            if (internalClient) {
                ...
                if (!MSGraphClientV3._internalInstance) {
                    MSGraphClientV3._internalInstance = _this._createInternalGraphClientInstance(_AadTokenProviders.configurable, options);
                }
            }
            else if (!MSGraphClientV3._instance) {
                if (clientOptions !== undefined) {
                    MSGraphClientV3._instance = _this._createGraphClientInstanceWithMiddleware(_AadTokenProviders.configurable, clientOptions);
                }
                else {
                    MSGraphClientV3._instance = _this._createGraphClientInstance(_AadTokenProviders.configurable, options);
                }
            }
        });
    }
    ...
    MSGraphClientV3.prototype._createGraphClientInstance = function (tokenProvider, options) {
        var qosMonitor = new _QosMonitor('MSGraphClientV3.CreateGraphClientInstance');
        var graphClient;
        try {
            graphClient = this._createClientInstance(__assign(__assign({}, options), { authProvider: undefined }), tokenProvider, false);
        }
        catch (e) {
    ...
    MSGraphClientV3.prototype._createGraphClientInstanceWithMiddleware = function (tokenProvider, options) {
        var qosMonitor = new _QosMonitor('MSGraphClientV3.CreateGraphClientInstanceWithMiddleware');
        var graphClient;
        try {
    ...
    MSGraphClientV3.prototype._createInternalGraphClientInstance = function (tokenProvider, options) {
        var qosMonitor = new _QosMonitor('MSGraphClientV3.CreateInternalGraphClientInstance');
        var graphClient;
        try {
            graphClient = this._createClientInstance(__assign(__assign({}, options), { authProvider: undefined }), tokenProvider, true);
        }
        catch (e) {
            qosMonitor.writeUnexpectedFailure(e.message, e);
            throw e;
    ...
    MSGraphClientV3.prototype._createClientInstance = function (options, tokenProvider, isInternal) {
        ...
        if (!clientOptions.middleware) {
            ...
            middlewareChain = MiddlewareFactory.getDefaultMiddlewareChain(new MSGraphAuthenticationProvider(tokenProvider, MSGraphClientV3._graphBaseUrl, isInternal));
        }
        else {
            // a developer has provided middleware(s)
            var authMiddleware = new AuthenticationHandler(new MSGraphAuthenticationProvider(tokenProvider, MSGraphClientV3._graphBaseUrl, isInternal));
        ...
        if (!isPassSdkVersionKSActivated() && this._consumerComponentManifest) {
        ...
        if (!isCustomHostsKSActivated()) {
            try {
                var url = new URL(MSGraphClientV3._graphBaseUrl);
                clientOptions.customHosts = new Set([url.host]);
            }
            catch (e) {
                var qosMonitor = new _QosMonitor('MSGraphClientV3.CustomHostsAdditionsFailed');
                qosMonitor.writeUnexpectedFailure(e.message, e);
            }
        }
        return _InternalGraphClient.initWithMiddleware(clientOptions);
    };
    return MSGraphClientV3;
}());
export default MSGraphClientV3;
