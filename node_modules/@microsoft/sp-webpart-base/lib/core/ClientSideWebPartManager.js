...
import { _ensureIsolatedDomainUrl, _LegacyThirdPartyFabricCoreLoader, _WebPartLoadDataCollector, _DeferredTeamsAppsClient } from '@microsoft/sp-component-base';
import { DisplayMode, Environment, EnvironmentType, Guid, SPEvent, Text, UrlQueryParameterCollection, UrlUtilities, Validate, _SPEventManager, _SPFlight, Session, _BrowserUtilities } from '@microsoft/sp-core-library';
...
import { isIsolatedAdaptiveCardExtensionWebPropertyPaneWorkAroundKSActivated, isIsolatedAdaptiveCardExtensionIframedPropertyPaneKSActivated, isFixMissingContextKSActivated, isLogWebPartLoadErrorKSActivated, isWebPartManifestListCacheRefreshKSActivated, isUsePromiseAllForFetchWebPartsKSActivated, isShowErrorAceForBlockedAceKSActivated, isRefactorLoadWebPartKSActivated, isFixUnstableThumbnailKSActivated, isHandleDisposeRaceConditionKSActive, isTranspileAlreadyLoadedWPKSActivated, isClickToAddFromContentPaneKSActivated, isAddLoggingForSwapManifestsKSActivated, isFixAceWebPartTagMismatchKSActivated } from '../common/KillSwitches';
import { is1PIsolationEnabled, isBotAcesFromTeamsEnabled, isConfigurationToolsEnabled, isIsolatedAdaptiveCardExtensionWebEnabled, isOptionalSupportedHostsFilteringEnabled, shouldUseListWebPartV2, shouldUseListWebPartFileBrowser, isLoadEditModeWebPartsInParallelEnabled } from '../common/Flights';
...
import { SPClientSideComponentType, SupportedHostType } from '../common/Enums';
import { CONTENT_PANEL_TOGGLED, CONTENT_PANEL_WILL_OPEN, PROPERTY_PANE_TOGGLED, PROPERTY_PANE_WILL_OPEN } from '../common/constants';
import { getErrorAceLoadConfiguration } from '../utils/getErrorAceLoadConfiguration';
import { isWebPartData } from '../utils/isWebPartData';
...
var ClientSideWebPartManager = /** @class */ (function () {
    ...
    ClientSideWebPartManager._isLoadingIsolatedWebPart = function (context, siteUrl) {
        ...
        if (is1PIsolationEnabled() && manifest.isInternal && manifest.isolationLevel === 'DomainIsolation') {
        ...
        return (_SPFlight.isEnabled(SPPPLAT_IFRAMED_WEBPART_CONTROLLER_FLIGHT_ID) &&
            ...
            (manifest.isolationLevel === WebPartIsolationLevel.DomIsolation ||
    ...
    ClientSideWebPartManager.prototype.enablePrefetching = function (controls) {
        if (!isLoadEditModeWebPartsInParallelEnabled()) {
            return;
        }
    ...
    ClientSideWebPartManager.prototype.loadWebPart = function (webPartManagerContext) {
        var _this = this;
        try {
            if (isRefactorLoadWebPartKSActivated()) {
            ...
            return (this._loadAndInitWebPart(context_1)
                ...
                if (!isHandleDisposeRaceConditionKSActive()) {
                ...
                if (!isIsolatedAdaptiveCardExtensionIframedPropertyPaneKSActivated() &&
                    isIsolated_1 &&
                ...
                var extraData = _WebPartLoadDataCollector.collect(context_1.manifest, context_1.webPartTag, _this._pageContext);
                monitor_1.writeSuccess(extraData);
            })
                .catch(function (e) {
                var extraData = _WebPartLoadDataCollector.collect(context_1.manifest, context_1.webPartTag, _this._pageContext);
                if (!isLogWebPartLoadErrorKSActivated() && !(e.error instanceof Error)) {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                if ('isDisposed' in e || (webPartInstance_1 === null || webPartInstance_1 === void 0 ? void 0 : webPartInstance_1.isDisposed)) {
                    ...
                    if (errorWebPart === null || errorWebPart === void 0 ? void 0 : errorWebPart.modifiedByGraph) {
                        ...
                        additionalMessage = Text.format(strings.WebpartErrorPagesAPIRelated, errorWebPart === null || errorWebPart === void 0 ? void 0 : errorWebPart.modifiedByGraph.apiVersion, date.toLocaleDateString(), date.toLocaleTimeString());
                    }
    ...
    ClientSideWebPartManager.prototype.fetchWebPartManifests = function () {
        var _this = this;
        var shouldFetchACEs = !isOptionalSupportedHostsFilteringEnabled();
        if (shouldFetchACEs) {
        ...
        if (!this._toolboxManifestsPromise) {
            ...
            var endpoint = void 0;
            if (isOptionalSupportedHostsFilteringEnabled()) {
                endpoint = "_api/web/GetClientSideComponentsByComponentType(componentTypesString='".concat(SPClientSideComponentType.WebPart, ",").concat(SPClientSideComponentType.AdaptiveCardExtension, "', includeManifestActivatedTime=true)");
            }
            else {
                // ACEs will be fetched in another request
                endpoint = "_api/web/GetClientSideComponentsByComponentType(componentTypesString='".concat(SPClientSideComponentType.WebPart, "',supportedHostTypeValue=").concat(SupportedHostType.SharePointWebPart, ")");
            }
            var requestUrl_1 = "".concat(UrlUtilities.removeEndSlash(this._pageContext.web.serverRelativeUrl), "/").concat(endpoint);
            this._host.serviceScope.whenFinished(function () {
                ...
                _this._toolboxManifestsPromise = _this._fetchManifestsWithCache(httpClient, requestUrl_1)
                    ...
                    if (!isWebPartManifestListCacheRefreshKSActivated() &&
                        !_this._isDebugSession &&
                        !_BrowserUtilities.isWorkbenchHosted()) {
                        SPComponentLoader.refreshWebPartManifests(manifests);
                    }
                    else {
        ...
        if (shouldFetchACEs) {
            ...
            if (!isUsePromiseAllForFetchWebPartsKSActivated()) {
    ...
    ClientSideWebPartManager.prototype.getAdaptiveCardExtensionManifests = function () {
        ...
        //
        if (isBotAcesFromTeamsEnabled() && this._toolboxTeamsAppComponentsManifests) {
    ...
    ClientSideWebPartManager.prototype.setWebPartData = function (contextOrWebPartData, instanceId, shouldFallback) {
        ...
        Validate.isNotNullOrUndefined(contextOrWebPartData, 'web part manager context');
        Validate.isNonemptyString(instanceId, 'web part instance id');
        var wp = this._webParts.get(instanceId);
        var isWebPartDataUsed = !isClickToAddFromContentPaneKSActivated() &&
            isConfigurationToolsEnabled() &&
            isWebPartData(contextOrWebPartData);
    ...
    ClientSideWebPartManager.prototype.tryGeneratePreviewImageUrl = function (instanceIds) {
        ...
        if (!isFixUnstableThumbnailKSActivated()) {
                    ...
                    if (webPartThumbnail) {
                        ...
                        if (webPartThumbnail.length < THUMBNAIL_MAX_URL_LENGTH) {
                            ...
                            if (isSupportedByOAuth ||
                                (isFixUnstableThumbnailKSActivated() && _this._isTitleAreaBannerWebPart(webPart))) {
    ...
    context) {
        ...
        if (this._iframedWebpartInstanceIds.has(instanceId)) {
            ...
            if ((_a = this._propertyPane) === null || _a === void 0 ? void 0 : _a.isOpen(isConfigurationToolsEnabled() ? instanceId : undefined)) {
            ...
            if (!isHandledByWPHost) {
                ...
                if (this._propertyPane) {
                    this._propertyPane.requestAction(instanceId, propertyPaneAction, renderedByWebPart, !isFixMissingContextKSActivated() ? context : undefined);
                }
    ...
    ClientSideWebPartManager.prototype.onWebPartDelete = function (instanceId) {
        var _a;
        this.disposeWebparts(instanceId);
        if (isLoadEditModeWebPartsInParallelEnabled() && instanceId) {
    ...
    ClientSideWebPartManager.prototype.getWebPartTranspileRenderProps = function (webPartManagerContext, request) {
        var monitor = new _QosMonitor('WebPart.GetTranspileRenderProps');
        return (this._getLoadedWebPart(webPartManagerContext)
            ...
            if (error instanceof SPWebPartError && error.id === SPWebPartErrorCode.TranspileNotImplemented) {
    ...
    ClientSideWebPartManager.prototype._getWebPartContext = function (context) {
        ...
        var parameters = {
            ...
            formFactor: context.formFactor || WebPartFormFactor.Standard, // Default behavior is Canvas web part
            host: this._host,
    ...
    ClientSideWebPartManager.prototype._getLoadedWebPart = function (webPartManagerContext) {
        var webpart;
        if (!isTranspileAlreadyLoadedWPKSActivated() && this._webParts.has(webPartManagerContext.instanceId)) {
        ...
        try {
            ...
            return syncTeamsComponentPromise.then(function () {
                ...
                return _this._doesUserHasPermissions(context_2.manifest).then(function (hasPermissions) {
                    var _a, _b, _c, _d;
                    if (!hasPermissions) {
                        var errorMessage = Text.format(strings.UserHasNoPermissionsError, ((_a = context_2.webPartData) === null || _a === void 0 ? void 0 : _a.title) || context_2.manifest.alias);
                        if (context_2.displayMode === DisplayMode.Edit) {
                            if (!isShowErrorAceForBlockedAceKSActivated()) {
                                // replace with error ace for edit mode to allow an operator to delete the card if needed
                                var _e = getErrorAceLoadConfiguration(context_2.webPartData, errorMessage), errorAceManifest = _e.manifest, errorAceWebPartData = _e.webPartData;
                    ...
                    if (isACE_1) {
                        ...
                        if ((!isIsolatedAdaptiveCardExtensionWebPropertyPaneWorkAroundKSActivated() &&
                            isIsolatedAdaptiveCardExtensionIframedPropertyPaneKSActivated() &&
                            context_2.displayMode === DisplayMode.Edit) ||
                            (!isACEWPManifest_1 && !isIsolated_2)) {
                            // The ACE WP manifest should be preloaded at this point
                            context_2.manifest = _ManifestStore.instance.tryGetManifest('f3bc67ee-015d-4678-a95f-c7e90ea62c1e');
                        }
                        else if (isIsolatedAdaptiveCardExtensionWebEnabled() &&
                            !isIsolatedACEWPManifest_1 &&
                            isIsolated_2) {
                            context_2.manifest = _ManifestStore.instance.tryGetManifest(ISOLATED_ACE_WP_MANIFEST_ID);
                        }
                        // @todo [VSO:WEX!#1076592] Clean up temporary ACE WP upgrade code
                        if (context_2.webPartData) {
                            context_2.webPartData = __assign(__assign({}, context_2.webPartData), (_d = context_2.webPartData.properties) === null || _d === void 0 ? void 0 : _d.aceData);
                        }
                        if (context_2.displayMode === DisplayMode.Edit &&
                            !isIsolatedAdaptiveCardExtensionIframedPropertyPaneKSActivated() &&
                            isIsolated_2 &&
                    ...
                    var monitor = new _QosMonitor('WebPart.Load', true);
                    var loadError;
                    return (new Promise(function (resolve) {
                        ...
                        .then(function () {
                        var extraData = _WebPartLoadDataCollector.collect(context_2.manifest, context_2.webPartTag, _this._pageContext);
                        monitor.writeSuccess(extraData);
                    })
                        .catch(function (e) {
                        var extraData = _WebPartLoadDataCollector.collect(context_2.manifest, context_2.webPartTag, _this._pageContext);
        ...
        try {
            ...
            return syncTeamsComponentPromise.then(function () {
                ...
                return _this._doesUserHasPermissions(context_3.manifest).then(function (hasPermissions) {
                    var _a, _b, _c, _d;
                    if (!hasPermissions) {
                        var errorMessage = Text.format(strings.UserHasNoPermissionsError, ((_a = context_3.webPartData) === null || _a === void 0 ? void 0 : _a.title) || context_3.manifest.alias);
                        if (context_3.displayMode === DisplayMode.Edit) {
                            if (!isShowErrorAceForBlockedAceKSActivated()) {
                                // replace with error ace for edit mode to allow an operator to delete the card if needed
                                var _e = getErrorAceLoadConfiguration(context_3.webPartData, errorMessage), errorAceManifest = _e.manifest, errorAceWebPartData = _e.webPartData;
                    ...
                    if (isACE_2) {
                        // at this point context.manifest should still be the manifest of a specific ACE (not the ACE WP),
                        // we need to use this manifest for the tag because CanvasWebPartViewPortLayer uses the original
                        // manifest for its tag and if the tags don't match, canvas control post-render callbacks won't fire
                        if (!isFixAceWebPartTagMismatchKSActivated()) {
                        ...
                        if ((!isIsolatedAdaptiveCardExtensionWebPropertyPaneWorkAroundKSActivated() &&
                            isIsolatedAdaptiveCardExtensionIframedPropertyPaneKSActivated() &&
                            context_3.displayMode === DisplayMode.Edit) ||
                            (!isACEWPManifest_2 && !isIsolated_3)) {
                            // The ACE WP manifest should be preloaded at this point
                            context_3.manifest = _ManifestStore.instance.tryGetManifest('f3bc67ee-015d-4678-a95f-c7e90ea62c1e');
                        }
                        else if (isIsolatedAdaptiveCardExtensionWebEnabled() &&
                            !isIsolatedACEWPManifest_2 &&
                            isIsolated_3) {
                            context_3.manifest = _ManifestStore.instance.tryGetManifest(ISOLATED_ACE_WP_MANIFEST_ID);
                        }
                        // @todo [VSO:WEX!#1076592] Clean up temporary ACE WP upgrade code
                        if (context_3.webPartData) {
                            context_3.webPartData = __assign(__assign({}, context_3.webPartData), (_d = context_3.webPartData.properties) === null || _d === void 0 ? void 0 : _d.aceData);
                        }
                        if (context_3.displayMode === DisplayMode.Edit &&
                            !isIsolatedAdaptiveCardExtensionIframedPropertyPaneKSActivated() &&
                            isIsolated_3 &&
                        ...
                        context_3.manifest = _ManifestStore.instance.tryGetManifest(PREFAB_WEBPART_MANIFEST_ID);
                    }
                    // To fix a11y bug https://dev.azure.com/onedrive/ODSP%20Product%20Experiences/_workitems/edit/2578132
                    // for ACEs, avoid overriding the tag set earlier now that ACE manifest has been replaced by ACE WP manifest.
                    // Avoiding overriding webPartTag here might be safe in general but only validated for ACEs so far.
                    if (isFixAceWebPartTagMismatchKSActivated() || !isACE_2 || !context_3.webPartTag) {
                        context_3.webPartTag = ClientSideWebPartManager.createWebPartTag(context_3.manifest, context_3.instanceId);
                    }
                    ...
                    return (new Promise(function (resolve) {
                        ...
                        var _a;
                        var webPartData = context_3.webPartData;
                        if (isLoadEditModeWebPartsInParallelEnabled() && webPartData) {
                            (_a = _this._loadWebPartPrefetching()) === null || _a === void 0 ? void 0 : _a.prefetchWebPart(webPartData, context_3.displayMode, context_3.manifest);
                        }
                        return results;
                    }));
                });
    ...
    ClientSideWebPartManager.prototype._fetchManifestsWithCache = function (httpClient, requestUrl) {
        if (!ClientSideWebPartManager._manifestRequestPromiseCache[requestUrl]) {
            ClientSideWebPartManager._manifestRequestPromiseCache[requestUrl] = httpClient.post(requestUrl, SPHttpClient.configurations.v1, {
                body: JSON.stringify({
                    project: Session.clientSideApplicationId
                }),
                headers: {
                    'x-odsp-web-version': _ManifestStore.instance.buildNumber
                }
            });
        }
    ...
    ClientSideWebPartManager.prototype._fetchAdaptiveCardExtensionManifests = function () {
        ...
        var endpoint = "_api/web/GetClientSideComponentsByComponentType(componentTypesString='".concat(SPClientSideComponentType.AdaptiveCardExtension, "',supportedHostTypeValue=").concat(SupportedHostType.Dashboard, ", includeManifestActivatedTime=true)");
        // if ACEs have already been fetched we don't have anything to do here.
        if (!this._toolboxACEManifestsPromise) {
            var requestUrl_2 = "".concat(UrlUtilities.removeEndSlash(this._pageContext.web.serverRelativeUrl), "/").concat(endpoint);
            this._host.serviceScope.whenFinished(function () {
                var httpClient = _this._host.serviceScope.consume(SPHttpClient.serviceKey);
    ...
    ClientSideWebPartManager.prototype._loadIsolatedWebPart = function (context, isIsolatedACEWP) {
        ...
        var domainUrl = _ensureIsolatedDomainUrl(context.manifest.isolatedDomain, this._pageContext.web.absoluteUrl);
    ...
    ClientSideWebPartManager.prototype._waitForTeamsAppSync = function (context) {
        ...
        return this._teamsAppsClient
            ...
            SPComponentLoader.registerManifests([syncedManifest]);
            //
    ...
    ClientSideWebPartManager.prototype._onPropertyPaneNotifications = function (event) {
        if (event && event.origin === window.location.origin) {
            switch (event.data) {
                case CONTENT_PANEL_WILL_OPEN:
                case PROPERTY_PANE_WILL_OPEN:
                    this._closeIsolatedPropertyPaneIfRequired();
                    break;
                case CONTENT_PANEL_TOGGLED:
                case PROPERTY_PANE_TOGGLED:
    ...
    Object.defineProperty(ClientSideWebPartManager.prototype, "_isDebugSession", {
        get: function () {
            return !!_BrowserUtilities.safelyAccessSessionStorage('spfx-debug');
        },
        enumerable: false,
        configurable: true
    });
    ...
    // Add monitor here to track if the swapping was successful
    ClientSideWebPartManager.prototype._swapManifestForWebparts = function (context) {
        var _a;
        // Image WebPart v2 substitution
        // TODO: Update ImageWebPartNext project with V1 Manifest ID - https://onedrive.visualstudio.com/WEX!/_workitems/edit/2665736
        var monitor;
        if (!isAddLoggingForSwapManifestsKSActivated()) {
            monitor = new _QosMonitor('ClientSideWebPartManager.SwapManifests');
        }
        if (context.manifest.id === IMAGE_WEBPART_V1_MANIFEST_ID &&
            ...
            var manifestCandidate = _ManifestStore.instance.tryGetManifest(IMAGE_WEBPART_V2_MANIFEST_ID);
            // replace the manifest in context if we have a valid manifest, else don't do anything
            if (manifestCandidate) {
                if (!isAddLoggingForSwapManifestsKSActivated() && monitor) {
                    monitor.writeSuccess();
                }
                context.manifest = manifestCandidate;
            }
            else if (!isAddLoggingForSwapManifestsKSActivated() && monitor) {
                monitor.writeUnexpectedFailure('Image WebPart manifest swap failed');
            }
        }
        ...
        if (context.manifest.id === LIST_WEBPART_V1_MANIFEST_ID) {
            ...
            var listManifestId = shouldUseListWebPartFileBrowser()
                ? LIST_WEBPART_FILEBROWSER_MANIFEST_ID
                : shouldUseListWebPartV2()
                    ? LIST_WEBPART_V2_MANIFEST_ID
                    : undefined;
            if (listManifestId) {
                // get the manifest for the v2 of the webpart
                var manifestCandidate = _ManifestStore.instance.tryGetManifest(listManifestId);
                // replace the manifest in context if we have a valid manifest, else don't do anything
                if (manifestCandidate) {
                    context.manifest = manifestCandidate;
                }
    ...
    ClientSideWebPartManager.prototype._doesUserHasPermissions = function (manifest) {
        ...
        if (manifest.componentType === 'AdaptiveCardExtension' &&
            ...
            isBotAcesFromTeamsEnabled() // the flight is checked in the middle of the condition as in most cases ACE will not be connected to teams app
        ) {
            return this._teamsAppsClient
    ...
    ClientSideWebPartManager.prototype._loadWebPartPrefetching = function () {
        if (!isLoadEditModeWebPartsInParallelEnabled()) {
            return undefined;
        }
        this.webPartPrefetching || (this.webPartPrefetching = new WebPartIdleLoad());
        return this.webPartPrefetching;
    };
    /**
