...
import { _PerformanceLogger, _EngagementLogger } from '@msinternal/sp-telemetry';
...
var ClientSideWebPartStatusRenderer = /** @class */ (function () {
    ...
    ClientSideWebPartStatusRenderer.prototype._renderError = function (domElement, error, options) {
        ...
        if (error instanceof SPError) {
            ...
            var callStack = stack
                ? Text.format(strings.WebpartErrorCallStackText, newLineSeparator, stack)
                : '';
            errorText = Text.format(strings.WebpartErrorErrorText, newLineSeparator, "".concat(vanillaError.message || error), callStack, extraMessage);
        }
        var cacheEntry = this._getCacheEntry(domElement);
