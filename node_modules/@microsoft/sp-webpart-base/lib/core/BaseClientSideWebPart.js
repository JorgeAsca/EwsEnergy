...
import { _logFailuresForEditCustomerPromise } from '@msinternal/sp-edit-customer-promise';
...
import { isChainDisposeCallsAfterOninitKSActivated, isIsolatedACEWPRenderAsyncKSActivated, isAddMoreRenderErrorInfoKSActivated, isAccessibleRootKSActivated, isAsyncCompleteKSActivated, isHandleDisposeRaceConditionKSActive } from '../common/KillSwitches';
import { isWEXWebPartOnDisposePromiseChainEnabled } from '../common/Flights';
/**
...
var BaseClientSideWebPart = /** @class */ (function (_super) {
    ...
    Object.defineProperty(BaseClientSideWebPart.prototype, "isVisible", {
         ...
         */
        get: function () {
            return this.displayMode === DisplayMode.Edit || this._isVisible;
        },
        set: function (value) {
            var _a, _b;
            if (this.displayMode !== DisplayMode.Edit && value !== this._isVisible) {
    ...
    BaseClientSideWebPart.prototype._internalInitialize = function (webPartContext, addedFromPersistedData, mode) {
        ...
        this._internalThemeProvider = this.context.serviceScope.consume(ThemeProvider.serviceKey);
    ...
    BaseClientSideWebPart.prototype.onTranspile = function (context) {
        ...
        throw SPWebPartError.create(SPWebPartErrorCode.TranspileNotImplemented);
    };
    /**
    ...
    BaseClientSideWebPart.prototype.renderError = function (error) {
        ...
        if (this.displayMode === DisplayMode.Edit &&
            ...
            (isHandleDisposeRaceConditionKSActive() || !this.isDisposed)) {
            var err = '';
            if (!isAddMoreRenderErrorInfoKSActivated()) {
                if (error.name) {
                    err += 'name:' + error.name + ';';
                }
                if (error.message) {
            ...
            _logFailuresForEditCustomerPromise(errorDetails, this.context.serviceScope);
        }
    ...
    BaseClientSideWebPart.prototype._disposeAfterRenderPromises = function () {
        var _this = this;
        if (!this.isDisposed &&
            !isChainDisposeCallsAfterOninitKSActivated() &&
            isWEXWebPartOnDisposePromiseChainEnabled()) {
            ...
            Promise.all(this._firstTimeRenderPromises).then(function () {
                ...
                _TraceLogger.logVerboseData({ source: _this._logSource, message: message });
    ...
    BaseClientSideWebPart.prototype._onInViewport = function () {
        ...
        initPromise
            ...
            var renderPromise = getDataPromise.then(function () {
                ...
                if (!isIsolatedACEWPRenderAsyncKSActivated() && _this.isRenderAsync) {
                    _this._syncRenderTime = _PerformanceLogger.now();
                }
                else {
    ...
    BaseClientSideWebPart.prototype._renderWithAccessibleTitle = function () {
        _PerformanceLogger.devMark("webpart(".concat(this.context.webPartTag, ")._renderWithAccessibleTitle"));
        var accessibleRoot = isAccessibleRootKSActivated()
            ? this.domElement
    ...
    BaseClientSideWebPart.prototype._startAsyncRenderGuardTimer = function () {
        ...
        if (!isAsyncCompleteKSActivated() && this._asyncRenderCompleteCalled === true) {
    ...
    BaseClientSideWebPart.prototype._renderCompleted = function (didUpdate) {
        if (this._asyncRenderGuardTimer) {
            ...
            if (!isIsolatedACEWPRenderAsyncKSActivated()) {
    ...
    BaseClientSideWebPart.prototype._resolveOrRejectOnRenderPromise = function (error, didUpdate) {
        ...
        _PerformanceLogger.devMark("webpart(".concat(this.context.webPartTag, ").complete"));
        if (error) {
            ...
            if (this._renderPromiseResolver) {
                this._renderPromiseResolver(isIsolatedACEWPRenderAsyncKSActivated() || didUpdate
                    ? _PerformanceLogger.now()
