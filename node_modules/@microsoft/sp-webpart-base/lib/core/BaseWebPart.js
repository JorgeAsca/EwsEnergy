...
import { BaseComponent, DynamicProperty, _PropertyMetadataProcessor } from '@microsoft/sp-component-base';
import { DisplayMode, Text, Validate, Version } from '@microsoft/sp-core-library';
import { _EngagementLogger, _LogEntry, _LogSource, _LogType, _QosMonitorShareFailureWithCP, _TraceLogger } from '@microsoft/sp-diagnostics';
import * as lodash from '@microsoft/sp-lodash-subset';
import { _EditModeCustomerPromiseHandler } from '@msinternal/sp-edit-customer-promise';
import { executeAndReThrow } from '../utils/ExecuteAndReThrow';
import { deepFreeze } from '../utils/Object';
import { isAmplifyHostType } from '../utils/isAmplifyHostType';
import { PropertyPaneFieldType } from '../SPPropertyPane';
import { SPWebPartError, SPWebPartErrorCode } from './error/SPWebPartError';
import { audienceFormServiceKey } from '../services/AudienceForm/AudienceFormServiceKey';
import { isWebPartAudienceTargetingEnabled, isServerPropertiesSanitizedEnabled, isWebPartHideOnMobileEnabled } from '../common/Flights';
import strings from './loc/Strings.resx';
import { isPropertyPaneUpdateASHAVetoKSActivated, isExemptWebPartWithDynamicDataFromLazyLoadKSActivated, isAudienceTargetingForMultiColumnPropertyPaneKSActive, isFixUnwantedAudienceTargetingForExpandedPropertyPaneKSActivated, isWebPartHideOnMobileScreenReaderFixKSActivated, isDisableHideOnMobileInAmplifyKSActivated, isRefactorReInstateServerProcessedDataKSActive } from '../common/KillSwitches';
import { reInstateServerProcessedData } from './deserializationHelpers';
...
var BaseWebPart = /** @class */ (function (_super) {
    ...
    BaseWebPart.prototype._getPropertyPaneData = function (isDetails, context) {
        ...
        return this._loadPropertyPaneResources().then(function () {
            ...
            var shouldAddAudiencePicker = _this.context.manifest.id === ACE_WEBPART_MANIFEST_ID ||
                (isWebPartAudienceTargetingEnabled() && !configuration.disableAudiencePicker);
            ...
            var shouldAddVisibilityGroup = isWebPartHideOnMobileEnabled() &&
                !configuration.disableVisibilityGroup &&
                (isDisableHideOnMobileInAmplifyKSActivated() || !isAmplifyHostType(_this.context.host.hostType));
    ...
    newValue, fieldType) {
        ...
        try {
            ...
            if (!lodash.isEqual(oldValue, newValue) ||
                ...
                if (!isPropertyPaneUpdateASHAVetoKSActivated() && this.manifest.isInternal) {
                    // create a monitor only when an update is required.
                    monitor = new _QosMonitorShareFailureWithCP("PropertyPane.".concat(componentType, ".").concat(this.manifest.alias, ".Update"), false, [
                        {
                            errorModuleName: "".concat(componentType, " PropertyPaneUpdate Failure"),
                            veto: 'WebPart Property Pane Failure',
                            logFailure: this.context.serviceScope.consume(_EditModeCustomerPromiseHandler.serviceKey)
                                .logFailure
                        }
                    ]);
                }
                ...
                if (oldValue instanceof DynamicProperty && !(newValue instanceof DynamicProperty)) {
                    var newDynamicProperty = new DynamicProperty(this.context.dynamicDataProvider, (function () {
                        _this._dynamicPropertyRefresh();
                    }).bind(this));
                ...
                if (propertyPath === AUDIENCES_PROPERTY) {
                    ...
                    if (isWebPartAudienceTargetingEnabled()) {
                        _EngagementLogger.logEvent("".concat(componentType, ".PropertyPane"), 'AudienceUpdated');
                        (_b = (_a = this.context.host).onAudiencesChanged) === null || _b === void 0 ? void 0 : _b.call(_a, this.instanceId, newValue);
                    }
                }
                else if (isWebPartHideOnMobileEnabled() && propertyPath === HIDE_ON_MOBILE_PROPERTY) {
                    ...
                    _EngagementLogger.logEventWithLogEntry(new _LogEntry('WebPart', 'ShowInMobileAndEmail.Toggle', _LogType.Event, {
                ...
                if (!isPropertyPaneUpdateASHAVetoKSActivated() && monitor) {
        ...
        switch (event) {
            ...
            case 'ApplyClicked':
                ...
                this._onPropertyPaneChangesApplied()
                    ...
                    .catch(function (err) {
                    _TraceLogger.logVerboseData({ source: _this._baseLogSource, message: err.message });
                    throw err;
                });
                break;
        }
    ...
    BaseWebPart.prototype._internalSerialize = function () {
        ...
        executeAndReThrow(function () {
            ...
            data = __assign(__assign({ 
                ...
                id: _this.context.manifest.id, instanceId: _this.context.instanceId, title: _this.title, description: _this.description, audiences: _this.audiences }, (isWebPartHideOnMobileEnabled() && {
                hideOn: _this.hideOn
            })), { 
            ...
            if (!isExemptWebPartWithDynamicDataFromLazyLoadKSActivated()) {
    ...
    BaseWebPart.prototype._internalDeserialize = function (data) {
        ...
        if (data) {
            ...
            var shouldProcessDynamicDataAfterServerScrub = isServerPropertiesSanitizedEnabled();
            var deserializedPropsObject = void 0;
            if (shouldProcessDynamicDataAfterServerScrub) {
                deserializedPropsObject = !isRefactorReInstateServerProcessedDataKSActive()
                    ? reInstateServerProcessedData({
    ...
    BaseWebPart.prototype._reInstateServerProcessedData = function (deserializedProperties, serverProcessedContent) {
        ...
        _PropertyMetadataProcessor.reInstateServerProcessedData(metadata, fixedProperties, serverProcessedContent, this.context.manifest);
        return fixedProperties;
    };
    /**
    ...
    BaseWebPart.prototype._addAudienceTargetingConfiguration = function (configuration) {
        ...
        var audienceFormService = this.context.serviceScope.consume(audienceFormServiceKey);
        if (audienceFormService && audienceFormService.renderAudienceFormPropertyPaneField) {
            ...
            var audienceTargetingPropertyPaneGroup = {
                groupName: strings.AudiencePickerLabel,
                groupFields: [
                    audienceFormService.renderAudienceFormPropertyPaneField({
                        ...
                        tooltipText: isAceWebPart ? strings.AudiencePickerTooltip : strings.WebPartAudiencePickerTooltip
                    })
                ]
            };
            if (!isAudienceTargetingForMultiColumnPropertyPaneKSActive()) {
                var pageIndex = 0; //in future, this index can be read from audiencePickerPosition in IPropertyPaneConfiguration;
                var pageConfig = configuration.pages[pageIndex];
                var columnIndex = !isFixUnwantedAudienceTargetingForExpandedPropertyPaneKSActivated()
                    ? (_a = configuration.audiencePickerPosition) === null || _a === void 0 ? void 0 : _a.columnIndex
                    : ((_b = configuration.audiencePickerPosition) === null || _b === void 0 ? void 0 : _b.columnIndex) || 1;
                if (pageConfig &&
                    pageConfig.columns !== undefined &&
                    columnIndex &&
                    columnIndex < pageConfig.columns.length) {
                    pageConfig.columns[columnIndex].push(audienceTargetingPropertyPaneGroup);
                }
                else {
                    configuration.pages[pageIndex].groups.push(audienceTargetingPropertyPaneGroup);
                }
            }
            else {
                configuration.pages[0].groups.push(audienceTargetingPropertyPaneGroup);
            }
        }
    };
    /**
     ...
     */
    BaseWebPart.prototype._addVisibilityConfiguration = function (configuration) {
        // Initialize property pane configuration if currently empty.
        if (!configuration.pages || configuration.pages.length === 0) {
            configuration.pages = [
                {
                    groups: []
        ...
        // TODO: Replace with `PropertyPaneControlVisibility`; this requires an async import (VSO:2613841).
        var showInMobileAndEmailToggle = {
            type: PropertyPaneFieldType.Toggle,
            targetProperty: HIDE_ON_MOBILE_PROPERTY,
            properties: __assign(__assign(__assign(__assign({ label: strings.ShowInMobileAndEmailToggleLabel }, (isWebPartHideOnMobileScreenReaderFixKSActivated() && {
                offAriaLabel: strings.ShowInMobileAndEmailToggleOffAriaLabel
            })), { offText: strings.ShowInMobileAndEmailToggleOffText }), (isWebPartHideOnMobileScreenReaderFixKSActivated() && {
                onAriaLabel: strings.ShowInMobileAndEmailToggleOnAriaLabel
            })), { onText: strings.ShowInMobileAndEmailToggleOnText, dataAutomationId: 'controlShowInMobileAndEmailToggle', checked: !this._hideOn.mobile })
        };
        var visibilityGroup = {
            groupName: strings.VisibilityGroupName,
            groupFields: [showInMobileAndEmailToggle]
        };
        configuration.pages[0].groups.push(visibilityGroup);
    };
    /**
    ...
    BaseWebPart.prototype._serialize = function () {
        ...
        _PropertyMetadataProcessor.serializeServerProcessedData(this.propertiesMetadata, properties, serverProcessedContent);
        return {
        ...
        if (data.dynamicDataPaths) {
            for (var path in data.dynamicDataPaths) {
                if (data.dynamicDataPaths.hasOwnProperty(path)) {
                    var dynamicProperty = new DynamicProperty(this.context.dynamicDataProvider, (function () {
                        _this._dynamicPropertyRefresh();
                    }).bind(this));
