...
var SPAlternativeUrls = tslib_1.__importStar(require("@msinternal/odsp-utilities/lib/alternativeUrls/SPAlternativeUrls"));
var Thumbnail_1 = require("@msinternal/odsp-datasources/lib/Thumbnail");
...
var SPAlternativeUrls_1 = require("@msinternal/odsp-utilities/lib/alternativeUrls/SPAlternativeUrls");
var PreviewUtility_2 = require("./PreviewUtility");
var Uri_1 = tslib_1.__importDefault(require("@msinternal/odsp-utilities/lib/uri/Uri"));
var ThumbnailHealingFlights_1 = require("./ThumbnailHealingFlights");
...
var DEFAULT_IMG_TYPE = 'jpg';
var LQIP_PARAM = 'lqip';
// List of knowns Air gap (aka AG) cloud environments
var AIRGAP_CLOUDS = ['ag08', 'ag09'];
/**
...
var _ThumbnailUrlGenerator = /** @class */ (function () {
    ...
    _ThumbnailUrlGenerator._isFileTypeSupportedByMediaService = function (fileType) {
        if (!sp_core_library_1._SPKillSwitch.isActivated('991c0e1c-05b0-4ecb-af32-6eefb92fe9d7' /* '2021-09-08', 'Remove SVG from META supporting list' */)) {
            ...
            PreviewUtility_1.PreviewUtility.mediaServiceSupportedMap.delete('svg');
            PreviewUtility_1.PreviewUtility.mediaServiceSupportedMap.delete('svgz');
        }
        return !!fileType && PreviewUtility_1.PreviewUtility.mediaServiceSupportedMap.has(fileType.toLowerCase());
    };
    _ThumbnailUrlGenerator._getMediaSupportFileType = function (fileType, imageUrl) {
        var supportedImageType = fileType || PreviewUtility_1.PreviewUtility.getFileExtension(imageUrl);
        return _ThumbnailUrlGenerator._isFileTypeSupportedByMediaService(supportedImageType)
            ? supportedImageType
            : DEFAULT_IMG_TYPE; /* Fallback to jpg in the absence of known type */
    };
    ...
    _ThumbnailUrlGenerator._getSPResourceAuthority = function (request) {
        var webAbsoluteUri = new SPResourcePath_1.SPResourcePath(request.absoluteUrl || location.href);
        if (request.spResource) {
            var resourcePath = new SPResourcePath_1.SPResourcePath(request.spResource);
            // Is Relative.
            if (resourcePath.format === SPResourcePath_1._SPResourcePathFormat.relative ||
                ...
                return new SPResourcePath_1.SPResourcePath(resourcePath.authority);
            }
        }
        else {
    ...
    _ThumbnailUrlGenerator.prototype.isFileTypeSupported = function (fileType) {
        ...
        return (PreviewUtility_1.PreviewUtility.getPreviewSupportedMap.has(normalizedType) ||
        ...
        if (request.spResource) {
            ...
            switch (provider) {
                ...
                case ThumbnailProviderType_1._ThumbnailProviderType.AFDPrivateCDN:
                    if (!request.nonExpiring && SPAlternativeUrls.isAlterantiveUrlFresh(request.spResource)) {
                ...
                case ThumbnailProviderType_1._ThumbnailProviderType.AFDPublicCDN:
                ...
                case ThumbnailProviderType_1._ThumbnailProviderType.External:
                    return this._getPublicCDNMeTAUrl(request.spResource, request, provider);
                ...
                case ThumbnailProviderType_1._ThumbnailProviderType.SPOLayoutsImage:
                    return this._getPublicCDNMeTAUrl(this._rewriteOptimalLayoutsImageUrl(request), request, ThumbnailProviderType_1._ThumbnailProviderType.MediaCDNLayoutsImage);
                default:
                    break;
            }
        }
        if (defaultImage || provider === ThumbnailProviderType_1._ThumbnailProviderType.Stock) {
            return this._getPublicCDNMeTAUrl(defaultImage || request.spResource, request, provider);
        }
    ...
    Object.defineProperty(_ThumbnailUrlGenerator.prototype, "isAirGap", {
        /**
         * Returns true if current environment is Air gap (aka AG)
         * Note AIRGAP_CLOUDS update is required if new AG environments are added in future.
         *
         * TODO: Potentially replace this with the existing helper located in EnvironmentHelper.ts.
         */
        get: function () {
            var _a;
            var cloudType = ((_a = this._pageContext) === null || _a === void 0 ? void 0 : _a.legacyPageContext).cloudType;
            return !!cloudType && AIRGAP_CLOUDS.includes(cloudType);
        },
        enumerable: false,
        configurable: true
    });
    ...
    _ThumbnailUrlGenerator.prototype.getVROOMThumbnailUrl = function (request, provider) {
        ...
        if (path) {
            ...
            if (provider === ThumbnailProviderType_1._ThumbnailProviderType.VROOMMeTACache) {
                return PreviewUtility_1.PreviewUtility.updateVROOMCustomThumbnailSize(request.spResource, request.width);
            }
            ...
            switch (provider) {
                ...
                case ThumbnailProviderType_1._ThumbnailProviderType.PublicCDN:
                case ThumbnailProviderType_1._ThumbnailProviderType.AFDPublicCDN:
                    var cdnPath = new SPResourcePath_1.SPResourcePath(request.spResource);
                    path = "https:/".concat(cdnPath.path);
                    path = path.split('?')[0]; // Get rid of query param
                    break;
                case ThumbnailProviderType_1._ThumbnailProviderType.AFDPrivateCDN:
                    var afdPath = new SPResourcePath_1.SPResourcePath(request.spResource);
            ...
            var resourcePath = new SPResourcePath_1.SPResourcePath(path);
            if (resourcePath.format === SPResourcePath_1._SPResourcePathFormat.absolute) {
                resourceAuthority = new SPResourcePath_1.SPResourcePath(path);
            }
        }
        ...
        if ((isAspx ||
            ...
            var vroomUrl = PreviewUtility_1.PreviewUtility.getVROOMItemUrl(request.siteId, request.webId, request.listId, request.uniqueId, resourceAuthority.authority, resourceAuthority.host, path);
            ...
            if (isCroppingRequest || isCropModeSpecified) {
                return PreviewUtility_1.PreviewUtility.getVROOMThumbnailUrl(vroomUrl, request.width, request.height, request.cacheBuster, undefined, request.originalWidth, request.originalHeight, request.cropMode, request.viewportTop, request.viewportLeft, request.viewportWidth, request.viewportHeight, undefined, request.encodedLQIPImage);
            }
            else {
                //Apply format parameter if it is an optimizable format
                var optimizeformatKillswitchActivated = PreviewUtility_1.PreviewUtility._optimizableFormats.get(request.fileType);
                var vroomFormatParam = !(0, ThumbnailHealingKillSwitches_1.isPassOptimizeFormatToVroomKSActivated)() &&
                    optimizeformatKillswitchActivated &&
                    !optimizeformatKillswitchActivated()
                    ? PreviewUtility_1.PreviewUtility.CURRENT_OPTIMIZED_FORMAT
                    : undefined;
                return !(0, ThumbnailHealingKillSwitches_1.isSendOriginalHeightWidthKSActivated)()
                    ? PreviewUtility_1.PreviewUtility.getVROOMThumbnailUrl(vroomUrl, request.width, request.height, request.cacheBuster, undefined, //resolution?: number | undefined,
                    request.originalWidth, //originalWidth?: number | undefined,
                    request.originalHeight, //originalHeight?: number | undefined,
                    undefined, //cropMode?: CropMode | undefined,
                    undefined, //viewportTop?: number | undefined,
                    undefined, //viewportLeft?: number | undefined,
                    undefined, //viewportWidth?: number | undefined,
                    undefined, //viewportHeight?: number | undefined,
                    vroomFormatParam, request.encodedLQIPImage)
                    : PreviewUtility_1.PreviewUtility.getVROOMThumbnailUrl(vroomUrl, request.width, request.height, request.cacheBuster, undefined, //resolution?: number | undefined,
                    undefined, //originalWidth?: number | undefined,
                    undefined, //originalHeight?: number | undefined,
                    undefined, //cropMode?: CropMode | undefined,
                    undefined, //viewportTop?: number | undefined,
                    undefined, //viewportLeft?: number | undefined,
                    undefined, //viewportWidth?: number | undefined,
    ...
    _ThumbnailUrlGenerator.prototype.getCDNUrl = function (request) {
        ...
        if (request.spResource && !request.nonExpiring) {
            var cdnUrl = PreviewUtility_1.PreviewUtility.getAlternateUrl(new SPResourcePath_1.SPResourcePath(request.spResource), request.width, request.height, request.originalWidth, request.originalHeight, request.cropMode, request.viewportTop, request.viewportLeft, request.viewportWidth, request.viewportHeight, request.cacheBuster, request.encodedLQIPImage);
            if (!(0, ThumbnailHealingKillSwitches_1.isCacheBusterCDNUrlFixKSActivated)() && this._shouldAddCacheBuster(cdnUrl, request)) {
                return this._addCacheBuster(cdnUrl, request.cacheBuster);
            }
            if ((0, ThumbnailHealingFlights_1.isLQIPQueryParamEnabled)() && this._shouldAddLQIP(cdnUrl, request)) {
                this._addLQIP(cdnUrl, request.encodedLQIPImage);
            }
            return cdnUrl;
        }
        return undefined;
    ...
    _ThumbnailUrlGenerator.prototype.updateThumbnailWidth = function (thumbnailUrl, newWidth, provider) {
        ...
        var normalizedWidth = PreviewUtility_1.PreviewUtility.normalizeWidth(newWidth);
        var thumbnailUri = new SPResourcePath_1.SPResourcePath(thumbnailUrl);
        if (thumbnailUri.searchParams.has(PreviewHelper_1.PreviewHelper.WIDTH_SHORT)) {
            return _ThumbnailUrlGenerator._replaceQueryValue(thumbnailUri, PreviewHelper_1.PreviewHelper.WIDTH_SHORT, normalizedWidth);
        }
        else if (thumbnailUri.searchParams.has(PreviewHelper_1.PreviewHelper.WIDTH)) {
            return _ThumbnailUrlGenerator._replaceQueryValue(thumbnailUri, PreviewHelper_1.PreviewHelper.WIDTH, normalizedWidth);
        }
        else if (thumbnailUri.searchParams.has(PreviewHelper_1.PreviewHelper.RESOLUTION)) {
            return _ThumbnailUrlGenerator._replaceQueryValue(thumbnailUri, PreviewHelper_1.PreviewHelper.RESOLUTION, PreviewUtility_1.PreviewUtility.normalizeWidthToResolution(newWidth));
        }
        else {
            ...
            switch (provider) {
                case ThumbnailProviderType_1._ThumbnailProviderType.External:
                    return thumbnailUrl;
                case ThumbnailProviderType_1._ThumbnailProviderType.VROOMMeTACache:
                    return PreviewUtility_1.PreviewUtility.updateVROOMCustomThumbnailSize(thumbnailUrl, newWidth);
                case ThumbnailProviderType_1._ThumbnailProviderType.PrivateCDN:
                case ThumbnailProviderType_1._ThumbnailProviderType.PublicCDN:
                case ThumbnailProviderType_1._ThumbnailProviderType.AFDPublicCDN:
                case ThumbnailProviderType_1._ThumbnailProviderType.AFDPrivateCDN:
                ...
                case ThumbnailProviderType_1._ThumbnailProviderType.MediaCDNHubble:
                case ThumbnailProviderType_1._ThumbnailProviderType.MeTA:
                    return "".concat(thumbnailUrl).concat(thumbnailUrl.indexOf('?') === -1 ? '?' : '&').concat(PreviewHelper_1.PreviewHelper.WIDTH, "=").concat(normalizedWidth);
                case ThumbnailProviderType_1._ThumbnailProviderType.DocViz:
                    return "".concat(thumbnailUrl, "&").concat(PreviewHelper_1.PreviewHelper.RESOLUTION, "=").concat(PreviewUtility_1.PreviewUtility.normalizeWidthToResolution(newWidth));
                case ThumbnailProviderType_1._ThumbnailProviderType.HubbleStockThumbnail:
                ...
                case ThumbnailProviderType_1._ThumbnailProviderType.SharePointHost:
                case ThumbnailProviderType_1._ThumbnailProviderType.MySite:
                    if (this.isFileTypeSupported(PreviewUtility_1.PreviewUtility.getFileExtension(new SPResourcePath_1.SPResourcePath(thumbnailUrl)))) {
                        return "".concat(thumbnailUrl).concat(thumbnailUrl.indexOf('?') === -1 ? '?' : '&').concat(PreviewHelper_1.PreviewHelper.WIDTH, "=").concat(normalizedWidth);
                    }
            }
        }
    ...
    _ThumbnailUrlGenerator.prototype.getThumbnailWidth = function (thumbnailUrl, provider) {
        ...
        try {
            var thumbnailUri = new SPResourcePath_1.SPResourcePath(thumbnailUrl);
            if (thumbnailUri.searchParams.has(PreviewHelper_1.PreviewHelper.WIDTH_SHORT)) {
                return Number(thumbnailUri.searchParams.get(PreviewHelper_1.PreviewHelper.WIDTH_SHORT));
            }
            else if (thumbnailUri.searchParams.has(PreviewHelper_1.PreviewHelper.WIDTH)) {
                return Number(thumbnailUri.searchParams.get(PreviewHelper_1.PreviewHelper.WIDTH));
            }
            else if (thumbnailUri.searchParams.has(PreviewHelper_1.PreviewHelper.RESOLUTION)) {
                return PreviewUtility_1.PreviewUtility.getWidthByResolution(Number(thumbnailUri.searchParams.get(PreviewHelper_1.PreviewHelper.RESOLUTION)));
            }
            else {
                // Width does not exist already, add it
                provider = provider || this.getThumbnailProvider(thumbnailUrl);
                switch (provider) {
                    case ThumbnailProviderType_1._ThumbnailProviderType.External:
                        return undefined;
                    case ThumbnailProviderType_1._ThumbnailProviderType.VROOMMeTACache:
                        return PreviewUtility_1.PreviewUtility.getVROOMThumbnailWidth(thumbnailUrl);
                    case ThumbnailProviderType_1._ThumbnailProviderType.PrivateCDN:
                    case ThumbnailProviderType_1._ThumbnailProviderType.PublicCDN:
                    case ThumbnailProviderType_1._ThumbnailProviderType.AFDPublicCDN:
                    case ThumbnailProviderType_1._ThumbnailProviderType.AFDPrivateCDN:
    ...
    _ThumbnailUrlGenerator.prototype.getThumbnailProvider = function (thumbnailUrl, isStockImage) {
        return (0, Thumbnail_1.getThumbnailProvider)(thumbnailUrl, isStockImage, this._pageContext);
    };
    /**
    ...
    _ThumbnailUrlGenerator.prototype.isCdnUrl = function (thumbnailUrl) {
        ...
        return (provider === ThumbnailProviderType_1._ThumbnailProviderType.PrivateCDN ||
            ...
            provider === ThumbnailProviderType_1._ThumbnailProviderType.AFDPublicCDN ||
     ...
     * @param callerId - Identification of the caller
    ...
    _ThumbnailUrlGenerator.prototype.fromGetPreview = function (getPreviewUrl, callerId, width) {
        ...
        var getPreviewUri = new SPResourcePath_1.SPResourcePath(getPreviewUrl);
        var baseUrl = getPreviewUri.authority;
        if (!baseUrl || baseUrl.length < 5) {
            // Relative URL can rely on current host
            var currentUri = new SPResourcePath_1.SPResourcePath(window.location.href);
            baseUrl = currentUri.authority;
        }
        var request = {
            fileType: getPreviewUri.searchParams.get(PreviewHelper_1.PreviewHelper.EXT) || undefined,
            absoluteUrl: baseUrl,
            spResource: getPreviewUri.searchParams.get(PreviewHelper_1.PreviewHelper.PATH) || undefined,
            siteId: getPreviewUri.searchParams.get(PreviewHelper_1.PreviewHelper.GUID_SITE) || undefined,
            webId: getPreviewUri.searchParams.get(PreviewHelper_1.PreviewHelper.GUID_WEB) || undefined,
            uniqueId: getPreviewUri.searchParams.get(PreviewHelper_1.PreviewHelper.GUID_FILE) || undefined,
            width: width,
            callerId: callerId
        };
        if (!request.fileType && request.spResource) {
            var resource = new SPResourcePath_1.SPResourcePath(request.spResource);
            request.fileType = PreviewUtility_1.PreviewUtility.getFileExtension(resource);
        }
    ...
    _ThumbnailUrlGenerator.prototype._fallbackToGetPreview = function (request) {
        return PreviewHelper_1.PreviewHelper.getPreviewImageUrl(request.fileType, request.thumbnail, request.absoluteUrl, request.spResource, request.siteId, request.webId, request.uniqueId, request.width, false, // isBannerImageUrl
        request.callerId);
    };
    /**
    ...
    _ThumbnailUrlGenerator.prototype._getPreLoadedImage = function (request) {
        ...
        if (request && request.spResource) {
            ...
            if (preloadImageDictionary) {
                ...
                if (!preloadedUrl) {
                    var path = new SPResourcePath_1.SPResourcePath(request.spResource).path;
                    if (path) {
        ...
        if (this._shouldAddCacheBuster(preloadedUrl, request)) {
            preloadedUrl = this._addCacheBuster(preloadedUrl, request.cacheBuster);
        }
        return this._validateResolution(preloadedUrl, request) ? preloadedUrl : undefined;
    };
    _ThumbnailUrlGenerator.prototype._shouldAddCacheBuster = function (url, request) {
        return request.cacheBuster && url && url.indexOf(request.cacheBuster) === -1;
    };
    _ThumbnailUrlGenerator.prototype._addCacheBuster = function (preloadedUrl, cacheBuster) {
        var url = new URL(preloadedUrl);
        url.searchParams.set('cb', cacheBuster);
        return url.toString();
    };
    _ThumbnailUrlGenerator.prototype._shouldAddLQIP = function (url, request) {
        return request.encodedLQIPImage && url && url.indexOf(request.encodedLQIPImage) === -1;
    };
    _ThumbnailUrlGenerator.prototype._addLQIP = function (preloadedUrl, lqip) {
        var url = new URL(preloadedUrl);
        url.searchParams.set(LQIP_PARAM, encodeURIComponent(lqip));
        return url.toString();
    };
     ...
     */
    _ThumbnailUrlGenerator.prototype._getPublicCDNMeTAUrl = function (publicImage, request, provider) {
        if (this.mediaCDNBaseUrl) {
            var defaultImagePath = new SPResourcePath_1.SPResourcePath(publicImage);
            if (!Thumbnail_1.ApprovedThumbnailHosts.isHostApprovedForThumbnail(defaultImagePath)) {
                if (!this.isAirGap && request.optimizePublicImage) {
                    publicImage = this._rewriteOptimalImageUrl(publicImage);
                }
                else {
                    return publicImage;
                }
            }
            if (!sp_core_library_1._SPKillSwitch.isActivated('bce5c76e-ffe1-4221-ad00-c8e89952a1b9' /* '2023-11-16', 'Remove SVG from META supporting list' */) &&
                (request.fileType || PreviewUtility_1.PreviewUtility.getFileExtension(defaultImagePath)) === 'svg') {
                // do not downscale svg, as they are good originally for quality
                return publicImage;
            }
            var meTAHostUrl = SPAlternativeUrls.tryGetAlternativeUrl(MEDIA_HOST_KEY);
            if (meTAHostUrl) {
                var width = request.width
                    ? PreviewUtility_1.PreviewUtility.normalizeWidth(request.width, true /* isPublicImage*/)
                    : PreviewUtility_2.CACHE_BREAKPOINT_S;
                var fileType = _ThumbnailUrlGenerator._getMediaSupportFileType(request.fileType, defaultImagePath);
                // Downscale Image using MeTA
                var metaAuthority = new SPResourcePath_1.SPResourcePath(SPAlternativeUrls.tryGetAlternativeUrl(MEDIA_HOST_KEY));
                var baseUrl = this.isAirGap
                    ? metaAuthority.authority
                    : "".concat(this.mediaCDNBaseUrl, "/").concat(metaAuthority.domain);
                ...
                var publicCdnUrl = "".concat(baseUrl, "/transform/thumbnail?provider=url&inputFormat=").concat(fileType, "&docid=").concat(encodeURIComponent(publicImage), "&w=").concat(width);
                ...
                if ((0, ThumbnailHealingFlights_1.isLQIPQueryParamEnabled)() && this._shouldAddLQIP(publicCdnUrl, request)) {
                    publicCdnUrl = "".concat(publicCdnUrl, "&lqip=").concat(encodeURIComponent(request.encodedLQIPImage));
                }
                return publicCdnUrl;
            }
        }
    ...
    _ThumbnailUrlGenerator.prototype._validateResolution = function (preloadedUrl, request) {
        ...
        var uri = new SPResourcePath_1.SPResourcePath(preloadedUrl);
        var resolution = uri.searchParams.get('resolution') || undefined;
        if (resolution) {
            var reqResolution = PreviewUtility_1.PreviewUtility.normalizeWidthToResolution(request.width);
        ...
        if (preloadedlWidth) {
            var requestedWidth = PreviewUtility_1.PreviewUtility.normalizeWidth(request.width);
    ...
    _ThumbnailUrlGenerator.prototype._rewriteOptimalLayoutsImageUrl = function (request) {
        ...
        var layoutsImagePath = new SPResourcePath_1.SPResourcePath(resourceLower);
        if (layoutsImagePath.format !== SPResourcePath_1._SPResourcePathFormat.absolute) {
            var basePath = new SPResourcePath_1.SPResourcePath(request.absoluteUrl || location.href);
            var relativePath = resourceLower.substring(resourceLower.indexOf('/_layouts/15/'));
            return this.isAirGap
                ? Uri_1.default.concatenate(basePath.authority, relativePath)
                : Uri_1.default.concatenate(this.mediaCDNBaseUrl, basePath.domain, relativePath);
        }
        else {
            var relativePath = layoutsImagePath.path.substring(layoutsImagePath.path.indexOf('/_layouts/15/'));
            return this.isAirGap
                ? Uri_1.default.concatenate(layoutsImagePath.authority, relativePath)
                : Uri_1.default.concatenate(this.mediaCDNBaseUrl, layoutsImagePath.domain, relativePath);
        }
    };
    /**
     * Rewrite image Url to media CDN cache
     * This will serve image behind CDN cache
     * e.g. https://media.akamai.odsp.cdn.office.net/microsoft.sharepoint.com/_layouts/15/images/sitepagethumbnail.png
     * @param publicImageUrl - Canonical public image Url
     */
    _ThumbnailUrlGenerator.prototype._rewriteOptimalImageUrl = function (publicImageUrl) {
        if (this.isAirGap) {
            return publicImageUrl;
        }
        var resourcePath = new SPResourcePath_1.SPResourcePath(publicImageUrl);
        if (resourcePath.format !== SPResourcePath_1._SPResourcePathFormat.absolute) {
            return publicImageUrl;
        }
        return (Uri_1.default.concatenate(this.mediaCDNBaseUrl, resourcePath.domain, resourcePath.path) +
    ...
    _ThumbnailUrlGenerator.prototype._getPathFromOptimalUrl = function (provider, url) {
        var resourcePath = new SPResourcePath_1.SPResourcePath(url);
     ...
     * Note: Hubble endpoint hostname may vary depending on the environment (e.g., AirGap's Hubble
     * endpoint is different from above). Please reference HubbleEndpointHelper.ts.
     *
    ...
    _ThumbnailUrlGenerator.prototype._updateHubbleThumbnailWidth = function (request) {
        ...
        var cdnResourcePath = new SPResourcePath_1.SPResourcePath(request.spResource);
        var originalHubbleUrl = "".concat(request.spResource.split('/thumbnails/')[0], "/").concat(cdnResourcePath.getQueryParameter('file'));
        return this._getPublicCDNMeTAUrl(originalHubbleUrl, request, ThumbnailProviderType_1._ThumbnailProviderType.HubbleStockThumbnail);
    };
