...
import AadHttpClientResponse from './AadHttpClientResponse';
...
import { CacheKeyType } from '../caching/ICacheDataProvider';
import { getCacheDataProviderServiceKey } from '../caching/CacheDataProviderService';
import { CacheManager } from '../caching/CacheManager';
import { isPoPTokenFlightEnabled } from '../oauthTokenProvider/AadFlights';
/**
...
var AadHttpClient = /** @class */ (function () {
    ...
    AadHttpClient.prototype.fetch = function (url, configuration, options, cacheConfiguration) {
        ...
        var cacheKey = this._cacheDataProvider.getCacheKey(CacheKeyType.HttpGenerated, url, configuration, options);
        // Fetch data and cache according to the cacheConfiguration
        return CacheManager.fetchAndCache(cacheKey, this._cacheDataProvider, cacheConfiguration, function () {
    ...
    AadHttpClient.prototype.get = function (url, configuration, options, cacheConfiguration) {
        var forcedGetOptions = HttpClientHelper.overrideHttpMethod(options, 'GET');
    ...
    AadHttpClient.prototype._fetch = function (url, configuration, options, useCachedToken) {
        ...
        if (isPoPTokenFlightEnabled()) {
                    ...
                    if (response.status === 401 && wwwAuthenticateHeader) {
                        ...
                        return tokenFetchPromise_1.then(function (retryToken) {
                            ...
                            return _this._getFetchCorePromise(url, configuration, options, retryToken).then(function (retryResponse) {
                                return new AadHttpClientResponse(retryResponse, retryTokenEndTimer - retryTokenStartTime_1);
                            });
                        });
                    }
                    else {
                        return new AadHttpClientResponse(response, endTimer - startTimer_1);
                    }
                });
            });
        }
            ...
            return tokenFetchPromise.then(function (token) {
                ...
                return HttpClientHelper.fetchCore(configuration, new Request(url, options), _this._serviceScope, _this._fetchProvider, AadHttpClient._className).then(function (response) {
                    return new AadHttpClientResponse(response, endTimer - startTimer_2);
                });
            });
        }
    };
    ...
    AadHttpClient.prototype._getFetchCorePromise = function (url, configuration, options, token) {
        ...
        return HttpClientHelper.fetchCore(configuration, new Request(url, options), this._serviceScope, this._fetchProvider, AadHttpClient._className);
    };
    /**
