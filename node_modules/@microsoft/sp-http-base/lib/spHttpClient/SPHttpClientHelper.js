import { _SPFlight, _SPKillSwitch, _ResponseUtilities as ResponseUtilities } from '@microsoft/sp-core-library';
...
var SPHttpClientHelper = /** @class */ (function () {
    ...
    SPHttpClientHelper._fetchWithInstrumentation = function (configuration, request, serviceScope, fetchProvider, logSourceId, cookieRefresher, isRetry) {
        ...
        return SPHttpClientHelper._handleFluidEnvironment(request, serviceScope).then(function (modifiedRequest) {
            return HttpClientHelper.fetchCore(configuration, modifiedRequest, serviceScope, fetchProvider, SPConstants.SP_HTTP_CLIENT_CLASS_NAME_FETCH).then(function (response) {
                var _a;
                if (!_SPKillSwitch.isActivated('51e16377-35f2-4af5-a6d7-f42f8c768141' /* '06/17/2025', 'grahamc' */) &&
                    !isRetry &&
                    response.status === 401) {
                    ...
                    if (authenticateHeader) {
                        claims = ResponseUtilities.getRequiredClaims(authenticateHeader);
                    }
    ...
    SPHttpClientHelper._handleFluidEnvironment = function (request, serviceScope) {
        if (_SPFlight.isEnabled(SPConstants.SP_TOKEN_FOR_FLUID_ID) && SPHttpClientHelper._isFluid()) {
            ...
            return AadTokenProviders.configurable
                ._getTokenInternal(new URL(graphContext.getWebAbsoluteUrl()).origin, AadTokenProviders.preAuthorizedConfiguration)
                .then(function (token) {
