import { _ResponseUtilities as ResponseUtilities, _BrowserUtilities } from '@microsoft/sp-core-library';
...
var HttpClientHelper = /** @class */ (function () {
    ...
    HttpClientHelper._fetchCoreImpl = function (configuration, request, serviceScope, fetchProvider, logSource, retry) {
        ...
        try {
            _TraceLogger.logVerbose(loggingSource, "".concat(request.method, " ").concat(request.url));
        }
        catch (error) {
        ...
        if (clientManifestData.manifest) {
            ...
            try {
                var xGeoUrlOrigins_1 = [];
                var graphContext = serviceScope.consume(GraphHttpClientContext.serviceKey);
                var spUrlOrigin = new URL(graphContext.getWebAbsoluteUrl()).origin; // TODO: this can be removed when graduating isUseXGeoOriginsForSdkVersionHeaderKSActivated
                var msGraphUrlOrigin = new URL(graphContext.getGraphEndpointUrl()).origin;
                var requestUrlOrigin = new URL(request.url).origin;
                if (!Killswitches.isUseXGeoOriginsForSdkVersionHeaderKSActivated()) {
                    var pageContext = serviceScope.consume(PageContext.serviceKey);
                    (_a = pageContext.legacyPageContext.multiGeoInfo) === null || _a === void 0 ? void 0 : _a.forEach(function (geoInfo) {
                        xGeoUrlOrigins_1.push(new URL(geoInfo.RootSiteUrl).origin);
                        xGeoUrlOrigins_1.push(new URL(geoInfo.MySiteHostUrl).origin);
                        xGeoUrlOrigins_1.push(new URL(geoInfo.TenantAdminUrl).origin);
                        if (geoInfo.PortalUrl !== geoInfo.RootSiteUrl)
                            xGeoUrlOrigins_1.push(new URL(geoInfo.PortalUrl).origin);
                        if (geoInfo.AdditionalUrls) {
                            geoInfo.AdditionalUrls.forEach(function (url) {
                                xGeoUrlOrigins_1.push(new URL(url).origin);
                            });
                        }
                    });
                }
                if (requestUrlOrigin === spUrlOrigin ||
                    requestUrlOrigin === msGraphUrlOrigin ||
                    xGeoUrlOrigins_1.includes(requestUrlOrigin)) {
                    request.headers.append('Access-Control-Request-Headers', 'SdkVersion');
                    request.headers.append('SdkVersion', "SPFx/".concat(clientManifestData.manifest.alias, "/").concat(clientManifestData.manifest.id, "/Is3PRequest:").concat(clientManifestData.manifest.isInternal ? '0' : '1'));
                }
            }
            catch (error) {
        ...
        return fetchProvider
            ...
            /* 401 */ requiredClaims &&
                ...
                _BrowserUtilities.isTeamsBrowserOrDesktop()) {
                return new Promise(function (resolve, reject) {
                    var claimsQOS = new _QosMonitor(HTTP_CLIENT_HANDLE_AUTH_CLAIM);
                    return _this._getTeamsAuthToken(new URL(request.url).origin, serviceScope, [requiredClaims])
                        ...
                        return fetch('/_api/SP.OAuth.NativeClient/Authenticate', {
                            ...
                            if (request.headers.get(HTTP_CLIENT_AUTHORIZATION) !== undefined) {
                ...
                qosMonitor = new _QosMonitor(HTTP_CLIENT_CLASS_NAME_FETCH);
    ...
    HttpClientHelper._getRequiredClaims = function (response) {
        if (response.status === 401) {
            ...
            if (wwwAuthenticate) {
                return ResponseUtilities.getRequiredClaims(wwwAuthenticate);
            }
        }
        return undefined;
    };
    ...
    HttpClientHelper._getBrokerClientChunk = function () {
        if (!this._brokerClient) {
            this._brokerClient = import(
            /* webpackChunkName: 'sp-secure-broker-lib-brokerclient' */ '@msinternal/sp-secure-broker-lib/lib/BrokerClient');
        }
        return this._brokerClient;
    };
    HttpClientHelper._getTeamsSDK = function () {
        if (!this._teamsJsP) {
            this._teamsJsP = import(
