...
import { _ResponseUtilities as ResponseUtilities, _BrowserUtilities, _SPKillSwitch } from '@microsoft/sp-core-library';
import { _LogSource, _QosMonitor, _TraceLogger } from '@microsoft/sp-diagnostics';
import { parseTokenPayload, isTokenValid } from '@msinternal/utilities-jwt';
...
import { isHandleClaimsChallengeOn1p3pExchangeFailureKSActivated } from '../../AadKillSwitches';
...
var OboTokenProvider = /** @class */ (function () {
    function OboTokenProvider(authContext, tokenParameters, _oboFirstPartyTokenCallback, _oboThirdPartyTokenCallback) {
        ...
        this._exchange1PTokenFor3P = function (
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        extraLogData, claims, refresh) {
                ...
                if (!_this._oboFirstPartyTokenRequest || refresh) {
                    _this._oboFirstPartyTokenRequest = _this._oboFirstPartyTokenCallback(claims, refresh)
                        ...
                        var urlToCall = "".concat(_this._serverRelativeUrl, "/_api/Microsoft.SharePoint.Internal") +
                            ...
                            "&clientId='".concat(AadConstants.PRE_AUTHORIZED_APP_PRINCIPAL_ID, "'");
                        var requestInfo = {
                            headers: {
                                'X-HTTP-Method': 'GET',
                                'OData-Version': '3.0',
                                OBOToken: unexpiredToken,
                                ClientId: AadConstants.PRE_AUTHORIZED_APP_PRINCIPAL_ID,
                                Resource: _this._authContext.config.clientId
                            }
                        };
                        return _this._spHttpClient.get(urlToCall, SPHttpClient.configurations.v1, requestInfo);
                    })
                        .then(function (response) {
                        if (!isHandleClaimsChallengeOn1p3pExchangeFailureKSActivated()) {
                            return _this._handleOboResponse(response, _this._authContext.config.clientId, extraLogData, claims);
                        }
                        if (response.ok) {
                            return response.json().then(function (json) { return JSON.parse(json.value).AccessToken; });
                        }
                            ...
                            return response.json().then(function (errorJson) {
                                ...
                                if (responseMessage && responseMessage.indexOf(MSAL_UI_REQUIRED_EXCEPTION_TEXT) > -1) {
                                    _TraceLogger.logError(OboTokenProvider._logSource, new Error('Please update your web application info to be {teamSiteDomain} as per https://docs.microsoft.com/en-us/sharepoint/dev/spfx/deployment-spfx-teams-solutions'));
                                }
        ...
        this._getTokenQosExtraData = { tokenFromCache: 'false' };
    }
    OboTokenProvider.prototype.getToken = function (resourceEndpoint, optionsOrUseCachedToken) {
        ...
        return this._getAccessToken(resourceEndpoint, useCachedToken, claims)
            .then(function (token) {
            acquireAccessTokenQosMonitor.writeSuccess(_this._getTokenQosExtraData);
            return token;
        })
            .catch(function (e) {
            acquireAccessTokenQosMonitor.writeUnexpectedFailure('TokenAcquisitionFailure', e, _this._getTokenQosExtraData);
            if (!_SPKillSwitch.isActivated('cbb2a51c-1456-49ee-92ca-adde5d7e70c7'
            /*'03/13/2023', 'cmalonzo - Do not store get token promise if the promise failed to get a token'*/
            ) &&
                useCachedToken &&
                !claims) {
                _this._cachedTokens.delete(resourceEndpoint);
            }
            throw e;
        });
    };
    OboTokenProvider.prototype._ensureState = function () {
        return Promise.resolve();
    };
    OboTokenProvider.prototype._ensureTelemetry = function () {
        return Promise.resolve();
    };
    OboTokenProvider.prototype._getAccessToken = function (resourceEndpoint, useCachedToken, claims) {
        var _this = this;
        if (useCachedToken && !claims) {
            if (!_SPKillSwitch.isActivated('fe22e094-281c-4b1d-8411-608af6d95172'
            /*'08/18/2022',
                ...
                if (cachedAccessToken) {
                    return cachedAccessToken.then(function (token) {
                        if (token && !_this._isTokenExpired(token)) {
                            _this._getTokenQosExtraData.tokenFromCache = 'true';
                            return Promise.resolve(token);
                        }
                        else {
                            _this._getTokenQosExtraData.tokenFromCache = 'expired';
    ...
    OboTokenProvider.prototype._getOboAuthToken = function (claims, refresh) {
        ...
        var acquireOboTokenMonitor = new _QosMonitor(!refresh ? 'OboTokenProvider.GetOboAuthToken' : 'OboTokenProvider.GetOboAuthTokenRetry');
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        var extraLogData = {
        ...
        if (clientId === AadConstants.PRE_AUTHORIZED_APP_PRINCIPAL_ID && this._oboFirstPartyTokenCallback) {
            retVal = this._oboFirstPartyTokenCallback(claims, refresh);
        }
        else if (this._oboThirdPartyTokenCallback &&
            (_SPKillSwitch.isActivated('1281dd06-6176-4d48-ad3c-9442e8294985'
            /* '01/13/2022',
                    'Only use 3rd party token callback if not in a secure broker scenario'*/
            ) ||
                !_BrowserUtilities.isUsingSecureBroker() ||
                !this._oboFirstPartyTokenCallback)) {
            // We ensure we are not using secure broker (except for isolated) because this method fetches a token
            // by ID and secure broker can only handle tokens by URI.  In the isolated case it uses the domain.
            // See ParentIFrameCommunicator.ts for details
            // If we can't get a 3P token then try to exchange a 1P token for the 3P token
            retVal = this._oboThirdPartyTokenCallback(this._authContext.config.clientId, claims, refresh).catch(function () { return _this._exchange1PTokenFor3P(extraLogData, claims, refresh); });
        }
        else {
            retVal = this._exchange1PTokenFor3P(extraLogData, claims, refresh);
        }
        return retVal
            .then(function (token) {
            if (_this._isTokenExpired(token, extraLogData)) {
                ...
                _SPKillSwitch.isActivated('cbe07593-122d-4ed8-9a3c-ec1944b9bcfa'
                /* '10/26/2022',
                    'If we get an empty or null token do not mark success'*/
                )) {
    ...
    OboTokenProvider.prototype._isTokenExpired = function (token, extraLogData) {
        if (token !== undefined) {
            var claims = parseTokenPayload(token);
            if (extraLogData) {
    ...
    extraLogData, claims) {
        ...
        return this._getOboAuthToken(claims).then(function (oboAuthToken) {
            if (_this._authContext.config.clientId === AadConstants.PRE_AUTHORIZED_ISOLATED_APP_PRINCIPAL_ID) {
                return _this._fetchAccessTokenFromSecureBroker(resourceEndpoint, oboAuthToken, extraLogData, claims);
            }
            else {
                return _this._fetchAccessTokenFromTenant(resourceEndpoint, oboAuthToken, extraLogData, claims);
            }
        });
    };
    OboTokenProvider.prototype._fetchAccessTokenFromSecureBroker = function (resourceEndpoint, oboToken, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    extraLogData, claims) {
        var _this = this;
        var urlToCall = "https://securebroker.sharepointonline.com/_api/SP.Broker.AcquireOBOToken";
        var queryParameters = new URL(window.location.href).searchParams;
        var tenantId = queryParameters.get('tenantId');
        if (!tenantId) {
            return Promise.reject('Missing tenantId');
        }
        return fetch(urlToCall, {
            method: 'POST',
            body: JSON.stringify({
                resource: resourceEndpoint,
                incomingToken: oboToken,
                tenantId: tenantId
            }),
            headers: {
            ...
            return _this._handleOboResponse(response, resourceEndpoint, extraLogData, claims);
        });
    };
    OboTokenProvider.prototype._fetchAccessTokenFromTenant = function (resourceEndpoint, oboAuthToken, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    extraLogData, claims) {
        var _this = this;
        var urlToCall = "".concat(this._serverRelativeUrl, "/_api/Microsoft.SharePoint.Internal") +
            ".ClientSideComponent.Token.AcquireOBOToken?resource='".concat(resourceEndpoint, "'") +
            "&clientId='".concat(this._authContext.config.clientId, "'");
        var requestInfo = {
            headers: {
                'X-HTTP-Method': 'GET',
                'OData-Version': '3.0'
        ...
        return this._spHttpClient.get(urlToCall, SPHttpClient.configurations.v1, requestInfo).then(function (response) {
            return _this._handleOboResponse(response, resourceEndpoint, extraLogData, claims);
        });
    };
    OboTokenProvider.prototype._handleOboResponse = function (response, resourceEndpoint, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    extraLogData, claims) {
        return this._getOboResponseJson(response, resourceEndpoint, extraLogData, claims).then(function (json) {
    ...
    OboTokenProvider.prototype._getOboResponseJson = function (response, resourceEndpoint, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    extraLogData, claims) {
        if (response.ok) {
            ...
            if (response.status === 401) {
                ...
                if (!claims && authenticateHeader) {
                    ...
                    if (!_SPKillSwitch.isActivated('d802e1d2-b1d5-4527-818b-d43ec3733f45'
                    /* '08/08/2022',
                        'Get Required Claims from utility function'*/
                    )) {
                        requiredClaims = ResponseUtilities.getRequiredClaims(authenticateHeader);
                    }
                    else {
                    ...
                    if (requiredClaims === '') {
                        extraLogData.emptyClaimsAfter401 = true;
                    }
                    else {
                        return this._fetchAccessTokenSilent(resourceEndpoint, extraLogData, [requiredClaims]);
                    }
                }
            }
            return response.json().then(function (jsonBody) { var _a, _b; return Promise.reject((_b = (_a = jsonBody['odata.error']) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.value); });
        }
    };
    /**
