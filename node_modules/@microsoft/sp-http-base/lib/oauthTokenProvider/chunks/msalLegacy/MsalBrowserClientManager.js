import { LogLevel, PublicClientApplication, createNestablePublicClientApplication, PairwiseBrokerApplication, InteractionType } from '@azure/msal-browser-legacy-1p';
import { shouldPrintMsalLogsToConsole } from './MsalUtilities';
import { _QosMonitor } from '@microsoft/sp-diagnostics';
import { isUseClientIndexKSActivated, isRemoveFloatingPromisesKSActivated, isAddClientCapabilitiesToMsalConfigKSActivated, isEnsureStateBeforeInitializeBrokerKSActivated } from '../../AadKillSwitches';
/**
...
var MsalBrowserClientManager = /** @class */ (function () {
    ...
    MsalBrowserClientManager.registerMsalBrowserClient = function (authority, clientId, redirectUri, isNaaSupported, isPairwiseBrokerEnabled) {
        ...
        if (!this._isClientIndexRegistered(clientIdIndex)) {
            if (isNaaSupported) {
                ...
                MsalBrowserClientManager._msalBrowserClients.set(clientIdIndex, this._createMsalBrowserPairwiseBrokerClient(authority, clientId, redirectUri, isNaaSupported));
            }
            else {
                MsalBrowserClientManager._msalBrowserClients.set(clientIdIndex, new Promise(function (resolve) {
                    ...
                    return msalBrowserClient
                        ...
                        return msalBrowserClient
                            ...
                            var handleRedirectPromiseQos = new _QosMonitor('MsalBrowserV3ClientManager._handleRedirectPromise');
                            handleRedirectPromiseQos.writeUnexpectedFailure(error.errorCode, error);
                            if (!isRemoveFloatingPromisesKSActivated()) {
                                throw error;
                            }
                        });
                    })
                        .catch(function (error) {
                        var initializePromiseQos = new _QosMonitor('MsalBrowserV3ClientManager._initializePromise');
    ...
    MsalBrowserClientManager._getClientIdIndex = function (clientId, isNaaSupported, isPairwiseBrokerEnabled) {
        if (!isUseClientIndexKSActivated()) {
            if (isNaaSupported) {
    ...
    MsalBrowserClientManager._createMsalBrowserNextClient = function (authority, clientId, redirectUri) {
        ...
        return createNestablePublicClientApplication(configuration);
    };
    MsalBrowserClientManager._createMsalBrowserPairwiseBrokerClient = function (authority, clientId, redirectUri, isNaaSupported, ensureStatePromise) {
        var msalConfiguration = this._getMsalBrowserConfiguration(authority, clientId, redirectUri, isNaaSupported);
        var brokerConfig = {
            actAsBroker: true,
            preferredInteractionType: InteractionType.Popup
        };
        /**
         * TODO: Remove this once MSAL is updated to version with fix to plumb through this configuration option
         * from the brokered application's configuration.
         */
        if (isAddClientCapabilitiesToMsalConfigKSActivated() && msalConfiguration && msalConfiguration.auth) {
            msalConfiguration.auth.clientCapabilities = ['CP1'];
        }
        var pwbApp = new PairwiseBrokerApplication(msalConfiguration, brokerConfig);
        return pwbApp.initialize().then(function () {
            if (!isEnsureStateBeforeInitializeBrokerKSActivated()) {
                return pwbApp;
            }
            return pwbApp.initializeBrokering().then(function () {
                return pwbApp;
            });
        });
    };
    MsalBrowserClientManager._getMsalBrowserConfiguration = function (authority, clientId, redirectUri, isNaaSupported) {
        ...
        if (MsalBrowserClientManager._isLocalStorageAvailable() && configuration.cache) {
            configuration.cache.cacheLocation = 'localStorage';
        }
        if (shouldPrintMsalLogsToConsole() && configuration.system) {
...
export { MsalBrowserClientManager };
