...
import { Guid, _Browser, _BrowserDetection, _BrowserUtilities, _SPEventManager } from '@microsoft/sp-core-library';
import AadConstants from '../../AadConstants';
import { AadErrorHandler } from '../../AadErrorHandler';
import { isNaaFlightEnabledForSharePointMobile, isNaaFlightEnabledForVCM, isPoPTokenFlightEnabled, isProcessEarCookieClienSideEnabled } from '../../AadFlights';
import AadTokenProvider, { BeforeRedirectEventArgs, PopupEventArgs } from '../../AadTokenProvider';
import { IAuthenticationScheme } from '../../ITokenProvider';
import { CORRELATION_ID_SESSION_STORAGE_KEY, CURRENT_REDIRECT_START_TIME_SESSION_STORAGE_KEY, MsalRedirectManager } from './MsalRedirectManager';
import { shouldPrintMsalLogsToConsole, shouldUsePopupDebug } from './MsalUtilities';
import { MsalBrowserClientManager } from './MsalBrowserClientManager';
import { MsalSessionIdManager } from './MsalSessionIdManager';
import { NonceManager } from '../../../nonceManager/NonceManager';
import { _QosMonitor } from '@microsoft/sp-diagnostics';
import { isEnsureStateWithGraphIfEARNotAvailableKSActivated, isEnsureStateBeforeInitializeBrokerKSActivated, isGetTokenDataQoSNewNameKSActivated, isAlwaysInitializeBrokerKsActivated, isUseLoginHintForRedirectKsActivated, isInteractionRequiredInIFrameExpectedKsActivated, isUpdateQosCorrelationIdKsActivated, isHandleFiveDayRedirectLoggingKsActivated, isTreatPopupErrorsAsExpectedInEnsureStateKsActivated, isHandleAccountSwitchKsActivated } from '../../AadKillSwitches';
...
var PREVIOUS_REFRESH_TIME_KEY = 'msal_browser_previous_refresh_time';
...
//Mark certain errors as expected failures
var EXPECTED_MSAL_ERRORS = new Set([
    'access_denied',
...
var MsalBrowserTokenProvider = /** @class */ (function () {
    function MsalBrowserTokenProvider(configuration) {
        ...
        this._redirectManager = new MsalRedirectManager(this._getAuthority(), this._getClientId(), true);
        this._shouldPrintDebugLogs = shouldPrintMsalLogsToConsole();
        MsalBrowserClientManager.registerMsalBrowserClient(this._getAuthority(), this._getClientId(), this._getRedirectUri(), this._defaultConfiguration.isNaaSupported ? true : false, this._defaultConfiguration.isPairwiseBrokerEnabled ? true : false);
        MsalSessionIdManager.initialize();
    }
    MsalBrowserTokenProvider.prototype.getToken = function (resourceEndpoint, optionsOrUseCachedToken) {
        var options = optionsOrUseCachedToken && typeof optionsOrUseCachedToken === 'object'
            ? __assign({}, optionsOrUseCachedToken) : {
            useCachedToken: optionsOrUseCachedToken,
            authenticationScheme: IAuthenticationScheme.BEARER
        };
    ...
    MsalBrowserTokenProvider.prototype.getTokenData = function (resourceEndpoint, optionsOrUseCachedToken, skipLoggingAndDisableRedirects) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            var getTokenQosName, getTokenDataMonitor, options_1, _a, shouldNotLogOrRedirect, _b, useCachedToken, claims, scopes, callersQosName, requestContext_1;
            var _this = this;
            return __generator(this, function (_c) {
                getTokenQosName = !isGetTokenDataQoSNewNameKSActivated()
                    ? 'MsalBrowserV3TokenProvider.GetTokenDataPublic'
                    : 'MsalBrowserV3TokenProvider.getTokenData';
                getTokenDataMonitor = new _QosMonitor(getTokenQosName);
                if (this._defaultConfiguration.isAnonymousGuestUser ||
                    ...
                    options_1 = optionsOrUseCachedToken && typeof optionsOrUseCachedToken === 'object'
                    ...
                    _a = options_1.skipLoggingAndDisableRedirects, shouldNotLogOrRedirect = _a === void 0 ? false : _a, _b = options_1.useCachedToken, useCachedToken = _b === void 0 ? true : _b, claims = options_1.claims, scopes = options_1.scopes, callersQosName = options_1.callersQosName;
                    this._shouldPrintDebugLogs = shouldPrintMsalLogsToConsole();
                    requestContext_1 = {
                    ...
                    if (isPoPTokenFlightEnabled() && options_1.authenticationScheme === IAuthenticationScheme.POP) {
                        if (!popNotSupported.has(resourceEndpoint) || this._isResourceEndpointExpired(resourceEndpoint)) {
                            return [2 /*return*/, this._getShrNonce(requestContext_1)
                                    ...
                                    if (error.name === 'NonceNotSupported') {
                                        ...
                                        options_1.authenticationScheme = IAuthenticationScheme.BEARER;
                                        return _this._getTokenDataInternal(requestContext_1, options_1);
                                    }
                                    else {
                            ...
                            options_1.authenticationScheme = IAuthenticationScheme.BEARER;
                            return [2 /*return*/, this._getTokenDataInternal(requestContext_1, options_1)];
                        }
                    }
                    return [2 /*return*/, this._getTokenDataInternal(requestContext_1, options_1)];
                }
    ...
    MsalBrowserTokenProvider.prototype._ensureState = function () {
        var _this = this;
        if (!this._ensureStateResult) {
            var ensureStateMonitor_1 = new _QosMonitor('MsalBrowserV3TokenProvider.EnsureState');
            if (!isEnsureStateBeforeInitializeBrokerKSActivated()) {
                this._logger('_ensureState', 'Starting ensure state');
                this._ensureStateResult = this._ensureStateHelper()
                    .catch(function (error) {
                    if (!isAlwaysInitializeBrokerKsActivated() &&
                        _this._defaultConfiguration.isPairwiseBrokerEnabled) {
                        return _this._initializeBrokering().then(function () {
                            throw error;
                        });
                    }
                    throw error;
                    ...
                    .then(function () {
                    _this._logger('_ensureState', 'Fetched RT successfully');
                })
                    .then(function () {
                    if (_this._defaultConfiguration.isPairwiseBrokerEnabled) {
                        return _this._initializeBrokering();
                    }
                })
                    .then(function () {
                    _this._logger('_ensureState', 'Ensured state successfully');
                    ensureStateMonitor_1.writeSuccess();
                })
                    .catch(function (error) {
                    _this._logger('_ensureState', 'Error ensuring state', error);
                    var errorMessage = error.message || '';
                    // If a consumer has used popup during ensure we can't help it if a user blocks the popup from
                    // succeeding so treat it as an expected failure
                    var isPopupOrCanceledError = !isTreatPopupErrorsAsExpectedInEnsureStateKsActivated() &&
                        (errorMessage.includes('popup_window_error') || errorMessage.includes('Canceled'));
                    if (error.name === 'Canceled' ||
                        isPopupOrCanceledError ||
                        (error instanceof window.msal.AuthError && _this._isExpectedFailure(error))) {
                        ensureStateMonitor_1.writeExpectedFailure(error.name, error);
                    }
                    else {
                        ensureStateMonitor_1.writeUnexpectedFailure(error.name, error);
                    }
                    throw error;
                });
            }
        ...
        return this._ensureStateResult;
    };
    MsalBrowserTokenProvider.prototype._ensureTelemetry = function () {
        try {
            if (window.sessionStorage.getItem(CURRENT_REDIRECT_START_TIME_SESSION_STORAGE_KEY)) {
    ...
    MsalBrowserTokenProvider.prototype._initializeBrokering = function () {
        var _this = this;
        var initializeBrokeringMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.InitializeBrokering');
        var extraData = {
            alias: 'false', // Alias column is used to mark cache hits
            CorrelationId: Guid.newGuid().toString(),
            isInternal: true,
            aadCorrelationId: Guid.newGuid().toString()
        };
        this._logger('_ensureState', 'Initializing pairwise brokering after ensuring state');
        return MsalBrowserClientManager.getMsalBrowserClient(this._getAuthority(), this._getClientId(), this._getRedirectUri(), this._defaultConfiguration.isNaaSupported ? true : false, this._defaultConfiguration.isPairwiseBrokerEnabled ? true : false)
            .then(function (clientApp) {
            return !isUpdateQosCorrelationIdKsActivated()
                ? clientApp.initializeBrokering(undefined, {
                    correlationId: extraData.aadCorrelationId
                })
                : clientApp.initializeBrokering();
        })
            .then(function () {
            _this._logger('_ensureState', 'Initialized pairwise brokering after ensuring state successfully');
            initializeBrokeringMonitor.writeSuccess(extraData);
        })
            .catch(function (error) {
            _this._logger('_ensureState', 'Error initializing pairwise brokering', error);
            initializeBrokeringMonitor.writeUnexpectedFailure(error.name, error, extraData);
            throw error;
        });
    };
    MsalBrowserTokenProvider.prototype._ensureStateHelper = function () {
        var _this = this;
        try {
            var earCookieProcessed = false;
            if (isProcessEarCookieClienSideEnabled()) {
                ...
                if (spa_rt_1.spa_rt) {
                    ...
                    var processedExpirationTime = this._getProcessedTokenExpirationDate();
                    ...
                    if (!isUpdateQosCorrelationIdKsActivated()) {
                        requestContext_2.telemetryData.aadCorrelationId = Guid.newGuid().toString();
                    }
                    if (!processedExpirationTime ||
                        (!isEnsureStateWithGraphIfEARNotAvailableKSActivated() &&
                            processedExpirationTime !== spa_rt_1.spa_refresh_token_exp)) {
                        earCookieProcessed = true;
                        var earMonitor_1 = new _QosMonitor('MsalBrowserV3TokenProvider.EncryptedAuthorizationResponse');
                        return this._ensureMsalClientIsInitialized(requestContext_2)
                            .then(function () {
                            var silentRequest = {
                                scopes: [],
                            ...
                            if (!isUpdateQosCorrelationIdKsActivated()) {
                                silentRequest.correlationId = requestContext_2.telemetryData.aadCorrelationId;
                            }
                            var serverResponse = {
                            ...
                            if (authResult) {
                                _this._msalBrowserClient.setActiveAccount(authResult.account);
                            }
                            if (!isUpdateQosCorrelationIdKsActivated()) {
                                earMonitor_1.writeSuccess(requestContext_2.telemetryData);
                            }
                            else {
                            ...
                            .catch(function (error) {
                            if (!isUpdateQosCorrelationIdKsActivated()) {
                                earMonitor_1.writeUnexpectedFailure('FailedToCacheEarToken', error, requestContext_2.telemetryData);
                            }
                            else {
                    ...
                    else if (this._isProcessedTokenExpirationDateValid(processedExpirationTime)) {
                        earCookieProcessed = true;
                        var earMonitorProcessed = new _QosMonitor('MsalBrowserV3TokenProvider.EncryptedAuthorizationResponseAlreadyProcessed');
                        earMonitorProcessed.writeSuccess();
                        return this._ensureMsalClientIsInitialized(requestContext_2).then(function () {
                            // If the EAR cookie is already processed, we should still ensure an active account is set
                            _this._getCurrentAccount();
                        });
                    }
                }
                else {
                    var cookieNotFound = new _QosMonitor('MsalBrowserV3TokenProvider.EncryptedAuthorizationResponseCookieNotFound');
                    cookieNotFound.writeUnexpectedFailure();
                }
            }
            if (!isEnsureStateWithGraphIfEARNotAvailableKSActivated() &&
                !earCookieProcessed &&
                this._defaultConfiguration.msGraphEndpointUrl) {
                // Using the empty resource causes msalJS to default to getting a graph token and it is
                // environment aware (ie sovereign vs prod vs airgap)
                return this.getTokenData(this._defaultConfiguration.msGraphEndpointUrl, {
                    isEnsureRequest: true
                }).then(function () {
                    return Promise.resolve();
                });
            }
    ...
    MsalBrowserTokenProvider.prototype._hasUserRedirectedInLastFiveDays = function () {
        var lastRedirectTime = this._getUsersLastRedirectTimeFromLocalStorage();
        if (lastRedirectTime) {
            var fiveDaysAgo = new Date();
            fiveDaysAgo.setDate(fiveDaysAgo.getDate() - 5);
            return lastRedirectTime > fiveDaysAgo;
        }
        return false;
    };
    MsalBrowserTokenProvider.prototype._getUsersLastRedirectTimeFromLocalStorage = function () {
        var returnValue = new Date(0);
        try {
            var lastRedirectTimeString = localStorage.getItem(PREVIOUS_REFRESH_TIME_KEY);
            if (lastRedirectTimeString) {
                returnValue = new Date(lastRedirectTimeString);
            }
        }
        catch (_a) {
            // If local storage is not available we don't log anything.  This is only used
            // for tracking users who end up with more than 1 full page redirect in less
            // than 5 days
            returnValue = new Date(0);
        }
        return returnValue;
    };
    MsalBrowserTokenProvider.prototype._setUsersLastRedirectTimeInLocalStorage = function () {
        try {
            localStorage.setItem(PREVIOUS_REFRESH_TIME_KEY, new Date().toString());
        }
        catch (_a) {
            // If local storage is not available we don't log anything.  This is only used
            // for tracking users who end up with more than 1 full page redirect in less
    ...
    MsalBrowserTokenProvider.prototype._clearUsersLastRedirectTimeInLocalStorage = function () {
        try {
            localStorage.removeItem(PREVIOUS_REFRESH_TIME_KEY);
        }
        catch (_a) {
            // If local storage is not available we don't log anything.  This is only used
            // for tracking users who end up with more than 1 full page redirect in less
    ...
    MsalBrowserTokenProvider.prototype._getProcessedTokenExpirationDate = function () {
        var expirationTime;
        try {
    ...
    MsalBrowserTokenProvider.prototype._isProcessedTokenExpirationDateValid = function (expirationTime) {
    ...
    MsalBrowserTokenProvider.prototype._getShrNonce = function (requestContext) {
        var _this = this;
        var nonceMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.GetShrNonce');
        return NonceManager.getNonceManager()
    ...
    MsalBrowserTokenProvider.prototype._getTokenDataInternal = function (requestContext, options, isRetry) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            var getTokenDataMonitorName, shouldCreateNAAMonitor, isNAAMobile, getTokenDataMonitor, handleSilentAuth, handlePopupAuth, handleRedirectAuth, authResult, authError, _loop_1, this_1, state_1, _a;
            var _this = this;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0: return [4 /*yield*/, this._ensureMsalClientIsInitialized(requestContext)];
                    case 1:
                        _b.sent();
                        if (!!options.isEnsureRequest) return [3 /*break*/, 3];
                        // We only await ensure state when it's not in an ensure request or we will deadlock
                        return [4 /*yield*/, this._ensureState()];
                    case 2:
                        // We only await ensure state when it's not in an ensure request or we will deadlock
                        _b.sent();
                        _b.label = 3;
                    case 3:
                        shouldCreateNAAMonitor = !!this._defaultConfiguration.isNaaSupported &&
                            this._defaultConfiguration.servicePrincipalId === AadConstants.PRE_AUTHORIZED_APP_PRINCIPAL_ID;
                        isNAAMobile = (isNaaFlightEnabledForSharePointMobile() && _BrowserUtilities.isMobileWebView()) ||
                            (isNaaFlightEnabledForVCM() && _BrowserUtilities.isMEEBridgeApplicationHosted());
                        if (isRetry) {
                        ...
                        getTokenDataMonitor = new _QosMonitor(getTokenDataMonitorName);
                        handleSilentAuth = function () { return __awaiter(_this, void 0, void 0, function () {
                            var tokenData, acquireTokenSilentMonitor, ssoSilentMonitor;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        if (!this._isCurrentUserLoggedInToMsal()) return [3 /*break*/, 2];
                                        acquireTokenSilentMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.AcquireTokenSilent');
                                        this._logger('getTokenData', 'User is logged in, calling acquireTokenSilent().');
                                        return [4 /*yield*/, this._acquireTokenSilent(requestContext, acquireTokenSilentMonitor)];
                                    case 1:
                                        tokenData = _a.sent();
                                        return [3 /*break*/, 4];
                                    case 2:
                                        ssoSilentMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.SsoSilent');
                                        this._logger('getTokenData', 'No user is logged in, calling ssoSilent().');
                                        return [4 /*yield*/, this._ssoSilent(requestContext, ssoSilentMonitor)];
                                    case 3:
                                        tokenData = _a.sent();
                                        _a.label = 4;
                                    case 4:
                                        this._logger('getTokenData', 'Token retrieved. Exiting.');
                        ...
                        handlePopupAuth = function (error) { return __awaiter(_this, void 0, void 0, function () {
                            var popupMonitor, pendingPopup;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        // eslint-disable-next-line require-atomic-updates
                                        requestContext.telemetryData.aadCorrelationId = (error === null || error === void 0 ? void 0 : error.correlationId) ? error.correlationId : '';
                                        this._logger('getTokenData', 'Caught an error in silent auth, fallback to popup auth.', error);
                                        if (!(this._defaultConfiguration.isNaaSupported && this._isInteractionRequiredError(error))) return [3 /*break*/, 2];
                                        ...
                                        return [4 /*yield*/, this._acquireTokenPopupNAA(requestContext)];
                                    case 1: return [2 /*return*/, _a.sent()];
                                    case 2:
                                        if (!(shouldUsePopupDebug() || (this._shouldUsePopup() && this._isInteractionRequiredError(error)))) return [3 /*break*/, 4];
                                        popupMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.Popup');
                                        pendingPopup = this._acquireTokenPopup(requestContext, popupMonitor);
                                        if (!pendingPopup) return [3 /*break*/, 4];
                                        return [4 /*yield*/, pendingPopup];
                                    case 3: return [2 /*return*/, _a.sent()];
                                    case 4: return [2 /*return*/];
                                }
                            });
                        }); };
                        handleRedirectAuth = function (error) { return __awaiter(_this, void 0, void 0, function () {
                            var isTopFrame, canRedirect, conditionalAcccessPolicyHitDuringEnsureState, monitorTimeoutFailure;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        isTopFrame = isInteractionRequiredInIFrameExpectedKsActivated() || window.top === window.self;
                                        canRedirect = this._isErrorEligibleForRedirect(error) &&
                                            this._redirectManager.isAllowedToRedirect() &&
                                            !requestContext.shouldNotLogOrRedirect &&
                                            !(options === null || options === void 0 ? void 0 : options.cancelRedirect);
                                        if (canRedirect &&
                                            options.isEnsureRequest &&
                                            AadErrorHandler._doesAadErrorCodeExist(error.message, AadConstants.CONDITIONAL_ACCESS_POLICY_ERRORS)) {
                                            canRedirect = false;
                                            conditionalAcccessPolicyHitDuringEnsureState = new _QosMonitor('MsalBrowserV3TokenProvider.ConditionalAccessDuringEnsureState');
                                            conditionalAcccessPolicyHitDuringEnsureState.writeUnexpectedFailure(error.errorCode, error, requestContext.telemetryData);
                                        }
                                        if (!canRedirect) return [3 /*break*/, 4];
                                        if (!!isTopFrame) return [3 /*break*/, 1];
                                        this._logger('getTokenData', 'Cannot handle redirect auth - not in top frame (expected behavior in iframe)');
                                        getTokenDataMonitor.writeExpectedFailure(error.errorCode, error, requestContext.telemetryData);
                                        error.isExpectedFailure = true;
                                        canRedirect = false;
                                        return [3 /*break*/, 3];
                                    case 1:
                                        this._logger('getTokenData', 'Calling _loginRedirect()');
                                        // eslint-disable-next-line require-atomic-updates
                                        requestContext.telemetryData.isTriggerFullPageRedirect = true;
                                        getTokenDataMonitor.writeExpectedFailure(error.errorCode, error, requestContext.telemetryData);
                                        error.isExpectedFailure = true;
                                        return [4 /*yield*/, this._loginRedirect(requestContext)];
                                    case 2:
                                        _a.sent();
                                        _a.label = 3;
                                    case 3: return [3 /*break*/, 8];
                                    case 4:
                                        if (!this._isExpectedFailure(error)) return [3 /*break*/, 5];
                                        getTokenDataMonitor.writeExpectedFailure(error.errorCode, error, requestContext.telemetryData);
                                        error.isExpectedFailure = true;
                                        return [3 /*break*/, 8];
                                    case 5:
                                        ...
                                        if (!this._isMonitorTimeoutFailure(error)) return [3 /*break*/, 8];
                                        if (!!isRetry) return [3 /*break*/, 7];
                                        return [4 /*yield*/, this._getTokenDataInternal(requestContext, options, true)];
                                    case 6: return [2 /*return*/, _a.sent()];
                                    case 7:
                                        monitorTimeoutFailure = new _QosMonitor('MsalBrowserV3TokenProvider.MonitorWindowTimeout');
                        ...
                        _loop_1 = function () {
                            var error_1, pendingPopupAuth, _c, resolvePopupPromise_1;
                            return __generator(this, function (_d) {
                                switch (_d.label) {
                                    case 0:
                                        _d.trys.push([0, 2, , 12]);
                                        if (shouldUsePopupDebug()) {
                                            throw new Error('Silent authentication is skipped due to debugging popup.');
                                        }
                                        return [4 /*yield*/, handleSilentAuth()];
                                    case 1:
                                        authResult = _d.sent();
                                        return [3 /*break*/, 12];
                                    case 2:
                                        ...
                                        authError = error_1;
                                        if (!(shouldUsePopupDebug() ||
                                            ((this_1._shouldUsePopup() || this_1._defaultConfiguration.isNaaSupported) &&
                                                this_1._isInteractionRequiredError(error_1)))) return [3 /*break*/, 11];
                                        pendingPopupAuth = this_1._pendingPopupAuth;
                                        if (!pendingPopupAuth) return [3 /*break*/, 7];
                                        _d.label = 3;
                                    case 3:
                                        _d.trys.push([3, 5, , 6]);
                                        return [4 /*yield*/, pendingPopupAuth];
                                    case 4:
                                        _d.sent();
                                        return [3 /*break*/, 6];
                                    case 5:
                                    ...
                                    case 7:
                                        this_1._pendingPopupAuth = new Promise(function (resolve) {
                                            resolvePopupPromise_1 = resolve;
                                        });
                                        _d.label = 8;
                                    case 8:
                                        _d.trys.push([8, , 10, 11]);
                                        return [4 /*yield*/, handlePopupAuth(error_1)];
                                    case 9:
                                        authResult = _d.sent();
                                        return [3 /*break*/, 11];
                                    case 10:
                                        this_1._pendingPopupAuth = undefined;
                                        resolvePopupPromise_1 === null || resolvePopupPromise_1 === void 0 ? void 0 : resolvePopupPromise_1();
                                        return [7 /*endfinally*/];
                                    case 11: return [2 /*return*/, "break"];
                        ...
                        _b.label = 4;
                    case 4:
                        if (!!authResult) return [3 /*break*/, 6];
                    ...
                    case 7:
                        _b.trys.push([7, 9, , 10]);
                        return [4 /*yield*/, handleRedirectAuth(authError)];
                    case 8:
                        authResult = _b.sent();
                        return [3 /*break*/, 10];
                    case 9:
                    ...
                    case 10:
                        ...
                        this._logger('getTokenData', 'Throwing the error back to caller. Exiting.');
                        throw authError;
                }
            });
        });
    ...
    MsalBrowserTokenProvider.prototype._ensureMsalClientIsInitialized = function (requestContext) {
        ...
        return new Promise(function (resolve, reject) {
            if (_this._msalBrowserClient) {
                ...
                MsalBrowserClientManager.getMsalBrowserClient(_this._getAuthority(), _this._getClientId(), _this._getRedirectUri(), _this._defaultConfiguration.isNaaSupported ? true : false, _this._defaultConfiguration.isPairwiseBrokerEnabled ? true : false)
                    .then(function (msalBrowserClient) {
                    _this._msalBrowserClient = msalBrowserClient;
                    resolve();
                })
                    .catch(function (error) {
                    var msalClientInitQos = new _QosMonitor('MsalBrowserV3TokenProvider._ensureMsalClientIsInitialized');
    ...
    MsalBrowserTokenProvider.prototype._acquireTokenSilent = function (requestContext, acquireTokenSilentMonitor, attempt) {
        ...
        if (!isFirstAttempt) {
            retryAcquireTokenSilentMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.RetryAcquireTokenSilent');
        }
        var silentRequest = this._generateSilentRequest(requestContext);
        if (!isUpdateQosCorrelationIdKsActivated()) {
            extraData.aadCorrelationId = silentRequest.correlationId;
        }
        ...
        return this._msalBrowserClient
            ...
            if (error && error.correlationId) {
                extraData.aadCorrelationId = error.correlationId;
            }
            else if (isUpdateQosCorrelationIdKsActivated()) {
                extraData.aadCorrelationId = '';
            }
    ...
    MsalBrowserTokenProvider.prototype._ssoSilent = function (requestContext, ssoSilentMonitor, attempt) {
        ...
        var isFirstAttempt = attempt === 0;
        if (!isFirstAttempt) {
            retrySsoSilentMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.RetrySsoSilent');
        }
        ...
        return this._msalBrowserClient
            ...
            extraData.alias = (!!authResult.fromCache).toString();
            if (!isUpdateQosCorrelationIdKsActivated()) {
                extraData.aadCorrelationId = ssoSilentRequest.correlationId;
            }
            if (authResult) {
                _this._msalBrowserClient.setActiveAccount(authResult.account);
            }
            ...
            if (ssoSilentRequest.sid &&
                ...
                AadErrorHandler._isSessionDoesNotExistError(error.errorMessage, error.errorCode)) {
                MsalSessionIdManager.addInvalidSessionId(ssoSilentRequest.sid);
    ...
    MsalBrowserTokenProvider.prototype._loginRedirect = function (requestContext) {
        ...
        if (!isUpdateQosCorrelationIdKsActivated()) {
            extraData.aadCorrelationId = loginRedirectRequestData.correlationId;
        }
        ...
        return this._msalBrowserClient.loginRedirect(loginRedirectRequestData).catch(function (error) {
            ...
            var loginRedirectMonitor = new _QosMonitor('MsalBrowserV3TokenProvider._loginRedirect');
    ...
    MsalBrowserTokenProvider.prototype._acquireTokenPopupNAA = function (requestContext) {
        var _this = this;
        var popupMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.PopupMonitorNAA');
        ...
        if (!isUpdateQosCorrelationIdKsActivated()) {
            extraData.aadCorrelationId = popupRequest.correlationId;
        }
        return this._msalBrowserClient
    ...
    MsalBrowserTokenProvider.prototype._acquireTokenPopup = function (requestContext, popupMonitor) {
        ...
        var showPopup = function () {
            return new Promise(function (resolve, reject) {
                ...
                resolveAuthData === null || resolveAuthData === void 0 ? void 0 : resolveAuthData(_this._msalBrowserClient
                    .acquireTokenPopup(popupRequest)
                    .then(function (authResult) {
                ...
                var undefinedAuthDataError = new Error('_acquireTokenPopup: AuthData promise is undefined');
                popupMonitor.writeUnexpectedFailure(undefinedAuthDataError.message);
                reject(undefinedAuthDataError);
            });
        };
        var cancel = function (error) {
            popupMonitor.writeExpectedFailure((error === null || error === void 0 ? void 0 : error.message) || 'Popup cancelled');
            if (rejectAuthData) {
                rejectAuthData(error);
            }
        };
        _SPEventManager.instance.raiseEvent(AadTokenProvider._popupEventId, new PopupEventArgs(cancel, requestPopup, showPopup, resourceEndpoint, {
            claims: requestContext.claims,
            skipLoggingAndDisableRedirects: requestContext.shouldNotLogOrRedirect,
            useCachedToken: requestContext.useCachedToken,
            authenticationScheme: requestContext.authenticationScheme,
            resourceRequestMethod: requestContext.resourceRequestMethod,
            scopes: requestContext.scopes,
            shrClaims: requestContext.shrClaims,
            callersQosName: (_a = requestContext.telemetryData) === null || _a === void 0 ? void 0 : _a.callersQosName
        }));
        return promise;
    };
    /**
    ...
    MsalBrowserTokenProvider.prototype._generateSilentRequest = function (requestContext) {
        ...
        var silentRequestData = __assign(__assign({ account: this._getCurrentAccount(), authority: this._getAuthority(), correlationId: correlationId, scopes: scopes || this._getScopes(resourceEndpoint) }, (useCachedToken ? {} : { cacheLookupPolicy: window.msal.CacheLookupPolicy.RefreshTokenAndNetwork })), (claims ? { claims: claims } : {}));
        if (!isUpdateQosCorrelationIdKsActivated()) {
            silentRequestData.correlationId = Guid.newGuid().toString();
        }
        if (isPoPTokenFlightEnabled() && requestContext.authenticationScheme === IAuthenticationScheme.POP) {
    ...
    MsalBrowserTokenProvider.prototype._setLoginHint = function (requestData, requestContext) {
        if (requestContext.loginHintType === MsalBrowserLoginHint.SESSION_ID) {
            ...
            if (sessionId && !MsalSessionIdManager.isSessionIdInvalid(sessionId)) {
    ...
    MsalBrowserTokenProvider.prototype._generateSsoSilentRequest = function (requestContext) {
        ...
        var ssoSilentRequestData = __assign({ authority: this._getAuthority(), correlationId: correlationId, scopes: scopes || this._getScopes(resourceEndpoint) }, (claims
            ? {
                claims: claims
            }
            : {}));
        if (!isUpdateQosCorrelationIdKsActivated()) {
            ssoSilentRequestData.correlationId = Guid.newGuid().toString();
        }
    ...
    MsalBrowserTokenProvider.prototype._generatePopupRequest = function (requestContext) {
        ...
        var popupRequestData = __assign({ account: this._getCurrentAccount(), authority: this._getAuthority(), correlationId: correlationId, scopes: scopes || this._getScopes(resourceEndpoint) }, (claims
            ? {
                claims: claims
            }
            : {}));
        if (!isUpdateQosCorrelationIdKsActivated()) {
            popupRequestData.correlationId = Guid.newGuid().toString();
        }
    ...
    MsalBrowserTokenProvider.prototype._generateTelemetryData = function (resourceEndpoint, authenticationScheme, callersQosName) {
        var isInternal = this._defaultConfiguration.servicePrincipalId === AadConstants.PRE_AUTHORIZED_APP_PRINCIPAL_ID;
        var telemetryData = {
            alias: 'false', // Alias column is used to mark cache hits
            aadSessionId: this._getSessionId(),
            CorrelationId: Guid.newGuid().toString(),
    ...
    MsalBrowserTokenProvider.prototype._isErrorEligibleForRedirect = function (error) {
        ...
        var isBrowserIE = _BrowserDetection.getBrowserInformation().browser === _Browser.IE;
    ...
    MsalBrowserTokenProvider.prototype._shouldUsePopup = function () {
        return !!this._defaultConfiguration.isMsalTokenProviderPopupEnabled;
    };
    /**
    ...
    MsalBrowserTokenProvider.prototype._isExpectedFailure = function (error) {
        ...
        var doesAadErrorCodeExist = AadErrorHandler._doesAadErrorCodeExist(error.message, AadConstants.EXPECTED_AAD_ERRORS);
        var isTabStopTest = this._isTabStopTest();
        var isUserCancelledError = USER_CANCELLED_ERROR_RESULT_CODES.indexOf(error.errorCode) > -1;
        var isExpectedError = EXPECTED_MSAL_ERRORS.has(error.errorCode);
        return (canAttemptRedirect || doesAadErrorCodeExist || isTabStopTest || isUserCancelledError || isExpectedError);
    };
    MsalBrowserTokenProvider.prototype._isMonitorTimeoutFailure = function (error) {
        return error.errorCode === AadConstants.MONITOR_WINDOW_TIMEOUT;
    };
    /**
    ...
    MsalBrowserTokenProvider.prototype._generateLoginRedirectRequest = function (requestContext) {
        ...
        if (!isUpdateQosCorrelationIdKsActivated()) {
            redirectRequestData.correlationId = Guid.newGuid().toString();
        }
        if (!isUseLoginHintForRedirectKsActivated()) {
            this._setLoginHint(redirectRequestData, requestContext);
        }
        if (isPoPTokenFlightEnabled() && requestContext.authenticationScheme === IAuthenticationScheme.POP) {
    ...
    MsalBrowserTokenProvider.prototype._getOnRedirectNavigate = function (requestContext) {
        ...
        var onRedirectNavigateCallback = function (url) {
            ...
            _SPEventManager.instance.raiseEvent(AadTokenProvider._onBeforeRedirectEventId, new BeforeRedirectEventArgs(url, _this._cancelRedirect.bind(_this), _this._defaultConfiguration));
            ...
            if (_this._redirectCancelled) {
                ...
                var fullPageRedirectQos = new _QosMonitor('MsalBrowserV3TokenProvider.FullPageRedirectTriggering', true, true);
                var handleFiveDayRedirectLogging = !isHandleFiveDayRedirectLoggingKsActivated();
                if (!handleFiveDayRedirectLogging || _this._hasUserRedirectedInLastFiveDays()) {
                    fullPageRedirectQos.writeUnexpectedFailure('FullPageRedirect', undefined, requestContext.telemetryData);
                }
                else {
                    fullPageRedirectQos.writeExpectedFailure('FullPageRedirect', undefined, requestContext.telemetryData);
                }
                if (handleFiveDayRedirectLogging) {
                    _this._setUsersLastRedirectTimeInLocalStorage();
                }
            }
    ...
    MsalBrowserTokenProvider.prototype._getCurrentAccount = function () {
        ...
        if (currentAccountByUsername || currentAccountByLocalId) {
            ...
            if (!currentActiveAccount) {
                this._logger('_getActiveAccount', 'No active account found, setting one.');
                this._msalBrowserClient.setActiveAccount(currentAccountByUsername || currentAccountByLocalId);
            }
        }
        else {
            this._logger('_getCurrentAccount', 'No account found');
            if (userName || localId) {
                if (!isHandleAccountSwitchKsActivated() && currentActiveAccount) {
                    // This happens when somebody switches users but doesn't close the browser
                    // We want to clear our last redirect state if it exists as a new user with 3p cookies
                    // disabled we expect a redirect
                    this._clearUsersLastRedirectTimeInLocalStorage();
                    var userSwitchQos = new _QosMonitor('MsalBrowserV3TokenProvider.UserSwitchDetected', true, true);
                    userSwitchQos.writeSuccess();
                    this._msalBrowserClient.setActiveAccount(null);
                }
                // If a specific account was to be used, but neither resolves, return `undefined`
                // to avoid falling back to the active account.
                return undefined;
            }
        }
    ...
    MsalBrowserTokenProvider.prototype._isRetriableNetworkError = function (error) {
        return (AadErrorHandler._isTokenRenewalTimeout(error === null || error === void 0 ? void 0 : error.errorCode) ||
            AadErrorHandler._isPostRequestFailedError(error === null || error === void 0 ? void 0 : error.errorCode));
    };
    /**
    ...
    MsalBrowserTokenProvider.prototype._getScopes = function (resourceEndpoint) {
        // If no resource endpoint is provided we will pass no scopes to MSAL and the graph token
        // will be requested by default.  We use this for getting the environment specific
        // graph endpoint
        if (!resourceEndpoint) {
            return [];
        }
    ...
    MsalBrowserTokenProvider.prototype._getSessionId = function () {
        ...
        var isValidSessionId = !!Guid.tryParse(sessionId);
        if (!isValidSessionId) {
            sessionId = undefined;
        }
        if (sessionId) {
    ...
    MsalBrowserTokenProvider.prototype._getAuthenticationScheme = function (authenticationScheme) {
        var authScheme;
        if (authenticationScheme === IAuthenticationScheme.BEARER) {
            authScheme = window.msal.AuthenticationScheme.BEARER;
        }
        else {
            authScheme = window.msal.AuthenticationScheme.POP;
        }
        return authScheme;
    };
    ...
    MsalBrowserTokenProvider.prototype._monitorFullPageRedirectReliability = function () {
        var redirectMonitor = new _QosMonitor('MsalBrowserV3TokenProvider.fullPageRedirect');
        var correlationId = window.sessionStorage.getItem(CORRELATION_ID_SESSION_STORAGE_KEY) || '';
        var redirectStartTimeDateString = window.sessionStorage.getItem(CURRENT_REDIRECT_START_TIME_SESSION_STORAGE_KEY);
        ...
        window.sessionStorage.removeItem(CORRELATION_ID_SESSION_STORAGE_KEY);
        window.sessionStorage.removeItem(CURRENT_REDIRECT_START_TIME_SESSION_STORAGE_KEY);
    };
...
export { MsalBrowserTokenProvider };
