...
var AadErrorHandler = /** @class */ (function () {
    ...
    AadErrorHandler._isInteractionRequired = function (errorMessage, errorCode) {
        if (!AadErrorHandler._doesAadErrorCodeExist(errorMessage, AadConstants.UNRESOLVABLE_AAD_ERROR_CODES)) {
            return AadConstants.RESOLVABLE_ERROR_CODES.has(errorCode);
        }
        return false;
    };
    AadErrorHandler._isTokenRenewalTimeout = function (errorCode) {
        return errorCode === AadConstants.TOKEN_RENEWAL_TIMEOUT;
    };
    AadErrorHandler._isPostRequestFailedError = function (errorCode) {
        return errorCode === AadConstants.POST_REQUEST_FAILED;
    };
    AadErrorHandler._isEndpointsResolutionError = function (errorCode) {
        return errorCode === AadConstants.ENDPOINTS_RESOLUTION_ERROR;
    };
    AadErrorHandler._isSessionDoesNotExistError = function (errorMessage, errorCode) {
        return (errorMessage.indexOf(AadConstants.SESSION_DOES_NOT_EXIST_ERROR) > -1 ||
            errorCode.indexOf(AadConstants.SESSION_DOES_NOT_EXIST_ERROR) > -1);
    };
    /**
