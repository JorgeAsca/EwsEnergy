...
import { isEnsureStateOnNAAKSActivated, isAlwaysCallEnsureStateActivated, isAllowFullPageRedirectFromEnsureAfterFiveMinutesKsActivated, isLogNaaDisabledSActivated } from './AadKillSwitches';
import { DeferredAadTokenProvider } from './DeferredAadTokenProvider';
...
var BeforeRedirectEventArgs = /** @class */ (function (_super) {
    __extends(BeforeRedirectEventArgs, _super);
...
var PopupEventArgs = /** @class */ (function (_super) {
    __extends(PopupEventArgs, _super);
    function PopupEventArgs(cancel, requestPopup, showPopup, resource, options) {
        var _this = _super.call(this) || this;
        _this.cancel = cancel;
...
var TICKS_IN_5_MINUTES = 5 * 60 * 1000;
/**
...
var AadTokenProvider = /** @class */ (function () {
    ...
    function AadTokenProvider(configuration, oboConfiguration) {
        ...
        this.onBeforeRedirectEvent = new SPEvent(AadTokenProvider._onBeforeRedirectEventId);
        this.popupEvent = new SPEvent(AadTokenProvider._popupEventId);
    }
    ...
    AadTokenProvider.prototype._setSupportsNestedAppAuth = function (isNAASupported) {
        if (!isLogNaaDisabledSActivated() && !isNAASupported && this._aadConfiguration.isNaaSupported) {
            var disableNaaQosMonitor = new _QosMonitor('AadTokenProvider.naaDisabled');
            disableNaaQosMonitor.writeUnexpectedFailure();
        }
    ...
    AadTokenProvider.prototype._ensureState = function () {
        var _this = this;
        var _a;
        if (isEnsureStateOnNAAKSActivated() || !((_a = this._aadConfiguration) === null || _a === void 0 ? void 0 : _a.isNaaSupported)) {
            return this._ensureTelemetry().then(function () {
                var hasRedirectedRecently = isAllowFullPageRedirectFromEnsureAfterFiveMinutesKsActivated()
                    ? Date.now() - TICKS_IN_5_DAYS <= AadTokenProvider._getLastEnsureStateRedirectTime()
                    : Date.now() - TICKS_IN_5_MINUTES <= AadTokenProvider._getLastEnsureStateRedirectTime();
                if (!isAlwaysCallEnsureStateActivated() || !hasRedirectedRecently) {
                    ...
                    var redirectHandler_1 = function (redirectEvent) {
                        if (!isAlwaysCallEnsureStateActivated() && hasRedirectedRecently) {
                            redirectEvent.cancel();
                            if (!isAllowFullPageRedirectFromEnsureAfterFiveMinutesKsActivated()) {
                                var cancelRedirectQos = new _QosMonitor('AadTokenProvider.ensureStateRedirectCancelled');
                                cancelRedirectQos.writeUnexpectedFailure('AadTokenProvider.ensureStateRedirectCancelled', new Error('AadTokenProvider._ensureState was called but the last redirect was recent'));
                            }
                        }
                        else {
    ...
    AadTokenProvider.prototype._getAadTokenProvider = function () {
        if (!this._aadTokenProvider) {
            var aadTokenProvider = new DeferredAadTokenProvider(this._tokenAcquisitionEvent, this.onBeforeRedirectEvent, this.popupEvent, this._aadConfiguration, this._oboConfiguration, this._oboFirstPartyTokenCallback, this._oboThirdPartyTokenCallback);
 ...
 * @internal
 */
var InternalAadTokenProvider = /** @class */ (function (_super) {
    __extends(InternalAadTokenProvider, _super);
    function InternalAadTokenProvider() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    return InternalAadTokenProvider;
}(AadTokenProvider));
export { InternalAadTokenProvider };
