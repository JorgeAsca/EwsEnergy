...
var MockFetchProvider = /** @class */ (function () {
    ...
    MockFetchProvider._getNormalizedText = function (text) {
        return text
            .split('\n')
    ...
    MockFetchProvider.prototype.assertQueueEmpty = function () {
        ...
        if (this.mockedRequest.length !== 0) {
            ...
            throw new Error("A request was expected but never issued: ".concat(next.expectedUrl));
        }
    };
    /**
    ...
    MockFetchProvider.prototype.fetch = function (request) {
        ...
        return request.text().then(function (requestBodyText) {
            ...
            if (_this.mockedRequest.length === 0) {
                throw new Error("No request expected at this time: ".concat(url.toString()));
            }
            ...
            if (_this.practiceRun) {
                ...
                console.log("MockFetchProvider: REQUEST #".concat(observeOnlyCounter));
                var headerLines_1 = '';
                request.headers.forEach(function (value, name) {
                    headerLines_1 += "  ".concat(name, ": ").concat(value, "\n");
                });
                // eslint-disable-next-line no-console
                console.log([
                    '',
                    "==[REQUEST #".concat(observeOnlyCounter, "]==========================================="),
                    "URL: ".concat(request.url),
                    "HEADERS:",
                    "".concat(headerLines_1) +
                        "== BODY: ====================================================================",
                    "".concat(requestBodyText),
                    "=============================================================================",
                    ''
                ].join('\n'));
            }
                ...
                if (url !== mockedRequest.expectedUrl) {
                    throw new Error("Expecting this: ".concat(mockedRequest.expectedUrl, "\r\nBut got this:").concat(url));
                }
                (mockedRequest.expectedRequestHeaders || []).forEach(function (mockedHeader) {
                    ...
                    if (mockedHeader.value !== undefined) {
                        if (!value) {
                            throw new Error("The \"".concat(mockedHeader.name, "\" header was expected,  but is missing from the response"));
                        }
                        if (value !== mockedHeader.value) {
                            throw new Error("The \"".concat(mockedHeader.name, "\" header had this value:\r\n") +
                                "\"".concat(value, "\"\r\nbut this was expected:\r\n\"").concat(mockedHeader.value, "\""));
                        }
                    }
                    else {
                        if (value) {
                            throw new Error("The \"".concat(mockedHeader.name, "\" header was NOT expected,") +
                ...
                if (mockedRequest.expectedRequestObject) {
                    ...
                    if (MockFetchProvider._getNormalizedText(expectedBodyText) !==
                        MockFetchProvider._getNormalizedText(requestBodyText)) {
