...
var SPHttpClientResponse_1 = tslib_1.__importDefault(require("./SPHttpClientResponse"));
var AadTokenProviders_1 = tslib_1.__importDefault(require("../oauthTokenProvider/AadTokenProviders"));
var SPConstants_1 = tslib_1.__importDefault(require("./SPConstants"));
/**
...
var SPHttpClientHelper = /** @class */ (function () {
    ...
    SPHttpClientHelper.applyDefaultHeaders = function (headers, configuration, options, oDataVersion, originalCorrelationId) {
        // Add some default headers that people should specify for any REST query,
        // assuming it's not missing
        // If fetch core has been sent an original corellation Id and
        // KS is not active, include it in the headers
        if (originalCorrelationId) {
            if (!headers.has('OriginalCorrelationId')) {
                headers.append('OriginalCorrelationId', originalCorrelationId);
            }
        }
    ...
    SPHttpClientHelper.getWebUrlFromRequestUrl = function (requestUrl) {
        ...
        for (var _i = 0, _a = SPConstants_1.default.RESERVED_URL_SEGMENTS; _i < _a.length; _i++) {
     ...
     */
    SPHttpClientHelper.fetchCore = function (url, configuration, options, serviceScope, digestCache, fetchProvider, cookieRefresher, logSourceId, originalCorrelationId) {
        return Promise.resolve().then(function () {
            ...
            SPHttpClientHelper.applyDefaultHeaders(headers, configuration, options, oDataVersion, originalCorrelationId);
                    ...
                    if (method !== 'GET' && method !== 'HEAD' && method !== 'OPTIONS') {
                        ...
                        if (!headers.has('X-RequestDigest')) {
                            ...
                            var promise = digestCache.fetchDigest(webUrl).then(function (digest) {
                                ...
                                return SPHttpClientHelper._fetchWithInstrumentation(configuration, new Request(url, modifiedOptions), serviceScope, fetchProvider, logSourceId, cookieRefresher);
                            });
            ...
            return SPHttpClientHelper._fetchWithInstrumentation(configuration, new Request(url, modifiedOptions), serviceScope, fetchProvider, logSourceId, cookieRefresher).then(function (response) {
                return new SPHttpClientResponse_1.default(response);
            });
        });
    };
    ...
    SPHttpClientHelper.overrideHttpMethod = function (options, httpRequestMethod) {
        return HttpClientHelper_1.default.overrideHttpMethod(options, httpRequestMethod);
    };
    SPHttpClientHelper._fetchWithInstrumentation = function (configuration, request, serviceScope, fetchProvider, logSourceId, cookieRefresher, isRetry) {
        var _this = this;
        ...
        return SPHttpClientHelper._handleFluidEnvironment(request, serviceScope).then(function (modifiedRequest) {
            return HttpClientHelper_1.default.fetchCore(configuration, modifiedRequest, serviceScope, fetchProvider, SPConstants_1.default.SP_HTTP_CLIENT_CLASS_NAME_FETCH).then(function (response) {
                var _a;
                if (!sp_core_library_1._SPKillSwitch.isActivated('51e16377-35f2-4af5-a6d7-f42f8c768141' /* '06/17/2025', 'grahamc' */) &&
                    !isRetry &&
                    response.status === 401) {
                    var claims = undefined;
                    var reauthid = (_a = response.headers.get('Reauthid')) !== null && _a !== void 0 ? _a : undefined;
                    var authenticateHeader = response.headers.get('WWW-Authenticate');
                    if (authenticateHeader) {
                        claims = sp_core_library_1._ResponseUtilities.getRequiredClaims(authenticateHeader);
                    }
                    return cookieRefresher
                        .refreshCookie(claims, reauthid, configuration.usePopupForCookieRefresh)
                        .then(function () {
                        return _this._fetchWithInstrumentation(configuration, request, serviceScope, fetchProvider, logSourceId, cookieRefresher, true);
                    });
                }
                else {
                    return response;
    ...
    SPHttpClientHelper._isFluid = function () {
        return SPConstants_1.default.FLUID_DOMAINS.indexOf(window.location.host) > -1;
    };
    SPHttpClientHelper._handleFluidEnvironment = function (request, serviceScope) {
        if (sp_core_library_1._SPFlight.isEnabled(SPConstants_1.default.SP_TOKEN_FOR_FLUID_ID) && SPHttpClientHelper._isFluid()) {
            var graphContext = serviceScope.consume(GraphHttpClientContext_1.default.serviceKey);
            return AadTokenProviders_1.default.configurable
                ._getTokenInternal(new URL(graphContext.getWebAbsoluteUrl()).origin, AadTokenProviders_1.default.preAuthorizedConfiguration)
                .then(function (token) {
