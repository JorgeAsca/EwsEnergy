...
import { ActiveEcsClientFlights } from '@msinternal/ecs-flight';
import { PRIMARY_VERSIONED_PATH as _FluentIconVersionedCdnPath } from '@msinternal/odsp-core-bundle';
import { PRIMARY_VERSIONED_PATH_TEMP as _FluentIconVersionedCdnPathTemp } from '@msinternal/odsp-core-bundle';
...
import { VetoValues as _VetoValues } from '@msinternal/odsp-core-bundle';
...
/**
 * Utility type to evaluate if {@link _RecursivelyExtractFromSet} should continue recursion.
 *
 * @remarks Use {@link ExtractFromSet}. This type is not meant to be used by itself.
 *
 * @public
 */
 ...
 * @public
 */
export declare type EnsureExtends<T, U> = T extends U ? T : never;
...
/* Excluded from this release type: _ES5CompatibleClass */

/* Excluded from this release type: _Event */

/* Excluded from this release type: _EventArgs */

/**
 * Utility type to extract `Type` from `Set` if and only if `Type` is in `Set`.
 *
 * @remarks If `Type` is not within `Set` then `never` is assigned.
 *
 * @public
 */
export declare type ExtractFromSet<Set extends unknown[], Type> = _RecursivelyExtractFromSet<Set, [], Type>;

export { _FluentIconVersionedCdnPath }

export { _FluentIconVersionedCdnPathTemp }
...
export declare class Guid {
     ...
     *
     * @param guidString - The input string.
...
/* Excluded from this release type: _isDebugEnabled */
 ...
 * @public
 */
export declare interface IServiceKey<T> {
    /**
     ...
     */
    readonly id: string;
    /* Excluded from this release type: _isRootService */
    /**
 ...
 * @public
 */
export declare interface IServiceScope {
    /**
     ...
     */
    getParent(): IServiceScope | undefined;
    /**
     ...
     * for the current scope, and an implementation of a given rootServiceKey for the root scope.
     * It may only be used when the ServiceScope is in an "unfinished"
 ...
 *
 * @remarks Since `never` is an empty set, the brackets are used to disable distribution over the generic type argument.
 *
 * @public
 */
 ...
 * @public
 */
export declare type IsStringUnion<T> = IsUnion<T> extends true ? (T extends string ? true : false) : false;
 ...
 *
 * @remarks Use {@link ExtractFromSet}. This type is not meant to be used by itself.
 * @remarks `Set` should be the full set of values to recurse over and `Current` is the set of values already evaluated. Therefore, indexing into `Set` with `Current`'s length yields the next value to evaluate.
 *
 * @public
 */
export declare type _NextValue<Set extends unknown[], Current extends unknown[]> = Set[Current['length']];

/* Excluded from this release type: _NumericRange */
...
/* Excluded from this release type: _ResponseUtilities */

/* Excluded from this release type: RootServiceKey */

/* Excluded from this release type: _safeWindowOpen */

/**
 * This is a type definition for the Service.
 * @public
 */
export declare type ServiceClass<T> = {
    new (serviceScope: IServiceScope): T;
} | {
 ...
 * @public
 */
export declare type ServiceClassConstructor<T> = (new (serviceScope: IServiceScope) => T) | (new (serviceScope: ServiceScope) => T);

/**
 * This is a callback that is used by ServiceKey.createCustom().
 * @public
 */
export declare type ServiceCreator<T> = ((serviceScope: IServiceScope) => T) | ((serviceScope: ServiceScope) => T);
 ...
 * @public
 */
export declare class ServiceKey<T> implements IServiceKey<T> {
    /**
     * A unique identifier for this service key.
     *
     * @remarks
     ...
     */
    static create<TKey>(name: string, serviceClass: ServiceClass<TKey>): ServiceKey<TKey>;
    /**
    ...
    /* Excluded from this release type: _generateServiceId */
 ...
 *
 * @remarks Use {@link ExtractFromSet}. This type is not meant to be used by itself.
 * @remarks If `Set` and `Current` are the same length, then all values have been recursed upon.
 *
 * @public
 */
 ...
 * @public
 */
export declare type StringUnion<T> = [T] extends [string] ? string extends T ? never : T : never;
...
/**
 * Utility type to evaluate if `Type1` and `Type2` are the same.
 *
 * @remarks Do not use with unions.
 * @remarks Use {@link ExtractFromSet}. This type is not meant to be used by itself.
 *
 * @public
 */
export declare type _UniqueCondition<Type1, Type2> = Type1 | Type2 extends Type2 ? true : false;

/**
 * Utility type to recursively evaluate if `Type` is within `Set`.
 *
 * @remarks Use {@link ExtractFromSet}. This type is not meant to be used by itself.
 * @remarks `Current` is used to track the recursion, which is why `CurrentValue` is added to `Current` after {@link _UniqueCondition} is evaluated.
 *
 * @public
 */
