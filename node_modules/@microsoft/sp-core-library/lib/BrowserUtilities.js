...
import { SPExperiment } from './SPExperiment';
 ...
 * @internal
 */
export var BROKER_IN_USE_KEY = 'useSecureBroker';
/**
...
var BrowserUtilities = /** @class */ (function () {
    ...
    BrowserUtilities.getTeamsAppType = function () {
        ...
        if (BrowserUtilities.isTeamsHosted()) {
            ...
            appHostType = "".concat(appHostType).concat(isSB ? '_SB_' : '_').concat(teamsHostType);
    ...
    BrowserUtilities.isUsingSecureBroker = function () {
        var _a, _b;
        try {
            if (!SPKillSwitch.isActivated('6efb55f0-7812-4b62-9443-988f9d8bbdbe'
            /* 02/24/2023, 'Any teams hosted scenario should use secure broker' */
            )) {
                return (BrowserUtilities.isTeamsHosted() &&
                    (!!new URL(window.location.href).searchParams.get(BROKER_IN_USE_KEY) ||
                        !!((_a = window.sessionStorage) === null || _a === void 0 ? void 0 : _a.getItem(BROKER_IN_USE_KEY))));
            }
            else {
                return (BrowserUtilities.isTeamsBrowserOrDesktop() &&
                    (!!new URL(window.location.href).searchParams.get(BROKER_IN_USE_KEY) ||
                        !!((_b = window.sessionStorage) === null || _b === void 0 ? void 0 : _b.getItem(BROKER_IN_USE_KEY))));
            }
        }
        catch (_c) {
    ...
    BrowserUtilities.setUsingSecureBroker = function () {
        if (window.sessionStorage) {
            try {
                window.sessionStorage.setItem(BROKER_IN_USE_KEY, 'true');
            }
            catch (_a) {
    ...
    BrowserUtilities.isTeamsAndroidHosted = function () {
        if (BrowserUtilities._isTeamsAndroid === undefined) {
            ...
            if (SPKillSwitch.isActivated('ab168592-38f6-4ca4-a5f4-87bc8057da3f'
            /* '03/04/2024', 'Ensure MEE Bridge is considered for Hosted apps' */
            )) {
                ...
                BrowserUtilities._isTeamsAndroid =
                    ...
                    Mozilla/5.0 (Linux; Android 9; Mi A1 Build/PKQ1.180917.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36 TeamsMobile-Android
                    Mozilla/5.0 (Linux; Android 9; Mi A1 Build/PKQ1.180917.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36 TeamsMobile-Android-dev
                    Mozilla/5.0 (Linux; Android 9; Mi A1 Build/PKQ1.180917.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36 Viva Connections for Mobile (MEE App)
                    */
                    /.* (TeamsMobile-Android|Android.*Viva Connections for Mobile).*/.test(ua);
            }
        }
        ...
        if (BrowserUtilities._isTeamsIOS === undefined) {
            ...
            /* '03/04/2024', 'Ensure MEE Bridge is considered for Hosted apps' */
                ...
                BrowserUtilities._isTeamsIOS =
                    ...
                    Mozilla/5.0 (iPhone; CPU iPhone OS 12_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 TeamsMobile-iOS
                    Mozilla/5.0 (iPhone; CPU iPhone OS 12_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 TeamsMobile-iOS-dev
                    Mozilla/5.0 (iPhone; CPU iPhone OS 16_6_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Viva Connections for Mobile (MEE App)
                    */
                    /.* (TeamsMobile-iOS|iPhone.*Viva Connections for Mobile).*/.test(ua);
            }
        }
    ...
    BrowserUtilities.isTemplatesGallery = function () {
        return (window.location.pathname.toLowerCase().indexOf('/_layouts/15/createpagefromtemplate.aspx') !== -1 &&
            (SPExperiment._getVariantWithoutLogging(90369 /* TemplateGalleryV2Experiment */) === 1 ||
                SPFlight.isEnabled(62046 /* TemplateGallery */)));
    };
    /**
    ...
    /**
     * Returns true, if we are in the SharePoint Mobile app.
     */
    BrowserUtilities.isSharePointMobileHosted = function () {
        if (BrowserUtilities._isSharePointMobileHosted === undefined) {
            BrowserUtilities._isSharePointMobileHosted = false;
            try {
                BrowserUtilities._isSharePointMobileHosted = /.*SharePoint for (Android|iOS).*/.test(navigator.userAgent);
            }
            catch (e) {
                /* no-op */
            }
        }
        return BrowserUtilities._isSharePointMobileHosted;
    };
    /**
     * Returns true, if we are in the VCM app.
     */
    BrowserUtilities.isVivaConnectionsMobileHosted = function () {
        if (BrowserUtilities._isVivaConnectionsMobileHosted === undefined) {
            BrowserUtilities._isVivaConnectionsMobileHosted = false;
            // check whether we are in VCM, whether that's via MEE Bridge or a Web View
            try {
                BrowserUtilities._isVivaConnectionsMobileHosted = /.* Viva Connections for Mobile.*/.test(navigator.userAgent);
            }
            catch (e) {
                /* no-op */
            }
        }
        return BrowserUtilities._isVivaConnectionsMobileHosted;
    };
    /**
    ...
    BrowserUtilities.isEmbedded = function () {
        if (BrowserUtilities._isEmbedded === undefined) {
            if (BrowserUtilities.isWebViewHosted() || /[?&]PageVersion=[^&]/.test(location.search)) {
                ...
                (SPFlight.isEnabled(60668 /* WexWebPartAudienceTargeting */) &&
    ...
    BrowserUtilities.isOutlookWebViewHosted = function () {
        if (SPKillSwitch.isActivated('05dfabf6-5844-4295-b4f1-07adfe1700d0'
    ...
    BrowserUtilities.isVivaHome = function () {
        return (SPFlight.isEnabled(60840 /* VivaHome */) &&
    ...
    BrowserUtilities.isFromVivaHome = function () {
        return (!SPKillSwitch.isActivated('e67cbc4f-48e9-4078-9352-269082e1631c'
    ...
    BrowserUtilities.safelyAccessLocalStorage = function (key) {
        try {
            ...
            if (!SPKillSwitch.isActivated('274b24bf-e198-427e-ade6-a472ecbb02c3'
