...
var UrlUtilities = /** @class */ (function () {
    ...
    UrlUtilities.resolve = function (url, baseUrl) {
        ...
        if (isRelativeUrl) {
            return "".concat(UrlUtilities.removeEndSlash(baseUrl), "/").concat(UrlUtilities.removeLeadingSlash(url));
        }
        else {
            return url;
        }
    ...
    UrlUtilities.isDataUrl = function (url) {
        Validate_1.default.isNotNullOrUndefined(url, 'url');
    ...
    /**
     * Creates an absolute URL from a relative URL and decodes it if needed.
     */
    UrlUtilities.absolutizeUrl = function (url, decode) {
        if (decode === void 0) { decode = false; }
        var a = document.createElement('a');
        a.href = url;
        return decode ? decodeURIComponent(a.href) : a.href;
    };
    /**
     * Creates a relative URL from an absolute URL.
     */
    UrlUtilities.relativizeUrl = function (url, removeQueryString) {
        if (UrlUtilities.isRelativeUrl(url)) {
            return url;
        }
        var urlObj = new URL(url);
        return urlObj.pathname + (removeQueryString ? '' : urlObj.search + urlObj.hash);
    };
    return UrlUtilities;
}());
