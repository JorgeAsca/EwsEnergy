...
import { Environment, EnvironmentType, Text, Validate, _SPEventManager } from '@microsoft/sp-core-library';
import { _LogSource, _QosMonitor, _TraceLogger } from '@microsoft/sp-diagnostics';
import { SPPagesContentPanelQOSScenarios, getQOSName } from '@msinternal/sp-pages-content-panel-context';
import { PropertyPaneWidthMode } from '../propertyPane/IPropertyPane';
import strings from '../loc/Strings.resx';
import styles from './PropertyPaneController.module.scss';
import { isCardDesignerPlusPlusFlight2Enabled, isConfigurationToolsEnabled, isContentPanelRenderLoggingEnabled, isFabEnabled, isODSPChatPaneResizeEnabled } from '../common/Flights';
import { isDispatchAllPropertyListenersKSActivated, isLeverageContentPanelForTopActionUpdatesKSActivated, isShowBackButtonBackKSActivated } from '../common/KillSwitches';
import { isPropertyPaneMultiColumn } from '../utils/MultiColumnHelper';
import { CONFIGURATION_COMPLETE_TIMEOUT } from './constants';
import { CustomError, markPrivacyData } from '@msinternal/odsp-core-bundle';
import { FAB_TOGGLE_EVT } from '@msinternal/sp-component-utilities';
import { PropertyPaneFieldChangeListeners } from './PropertyPaneFieldChangeListeners';
/**
...
/**
 * Property pane default width.
 */
var PROPERTY_PANE_DEFAULT_WIDTH = 340;
/**
 * Property pane narrow width.
 */
var PROPERTY_PANE_NARROW_WIDTH = 320;
/**
...
var PropertyPaneController = /** @class */ (function () {
    ...
    function PropertyPaneController() {
        ...
        this._propertyPaneChangeListeners = new PropertyPaneFieldChangeListeners();
        this._constructor();
    }
    /**
    ...
    PropertyPaneController.prototype.registerOnPropertyPaneFieldChangedListener = function (instanceId, callback) {
        return this._propertyPaneChangeListeners.register(instanceId, callback);
    };
    /**
    ...
    context) {
        ...
        else if (propertyPaneAction === 'Refresh') {
    ...
    PropertyPaneController.prototype.forceClose = function () {
    ...
    PropertyPaneController.prototype.onConsumerDelete = function (id) {
        ...
        if (id === this._currentlyConfiguredConsumerId) {
            if (this._isOpen) {
                if (isCardDesignerPlusPlusFlight2Enabled() && this._isPropertyPaneExpandedClassActive()) {
                    this._propertyPaneContainer.classList.remove(styles.spPropertyPaneContainerExpanded);
                }
        ...
        if (!isLeverageContentPanelForTopActionUpdatesKSActivated() &&
            isDispatchAllPropertyListenersKSActivated()) {
            this._propertyPaneChangeListeners.remove(id);
        }
    };
    /**
    ...
    PropertyPaneController.prototype._setPropertyPaneToNarrowRender = function (narrow) {
        this._paneWidthMode = narrow ? PropertyPaneWidthMode.Narrow : PropertyPaneWidthMode.Default;
    };
    /**
     * Instantiates the PropertyPaneController object.
     *
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    context) {
        if (isODSPChatPaneResizeEnabled()) {
            this._paneWidthMode = (toBeConfiguredConsumer === null || toBeConfiguredConsumer === void 0 ? void 0 : toBeConfiguredConsumer._getPropertyPaneWidthMode)
                ? toBeConfiguredConsumer._getPropertyPaneWidthMode()
                : PropertyPaneWidthMode.Default;
        }
        else {
            this._paneWidthMode = PropertyPaneWidthMode.Default;
        }
        switch (propertyPaneAction) {
             ...
             */
            case 'Refresh':
                if (!isDispatchAllPropertyListenersKSActivated()) {
                    this._propertyPaneChangeListeners.dispatchAll();
                }
                else {
                    this._propertyPaneChangeListeners.dispatch(toBeConfiguredConsumerInstanceId);
                }
                if (this._isOpen) {
                    this._renderPropertyPane(toBeConfiguredConsumer, true, false);
                }
     ...
     * Modifying the width of property pane requires updating the FAB right offset accordingly.
     */
    PropertyPaneController.prototype._showPropertyPane = function () {
        var _this = this;
        if (isFabEnabled()) {
            // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
            var updateFabPosition = function () {
                var position = { isShiftedForPane: true };
                if (isODSPChatPaneResizeEnabled()) {
                    switch (_this._paneWidthMode) {
                        case PropertyPaneWidthMode.Narrow:
                            position.rightOffset = PROPERTY_PANE_NARROW_WIDTH;
                            break;
                        case PropertyPaneWidthMode.Wide:
                            position.rightOffset = Math.max(window.innerWidth * 0.28, 320);
                            break;
                        default:
                            position.rightOffset = PROPERTY_PANE_DEFAULT_WIDTH;
                    }
                }
                else {
                    position.rightOffset = _this._paneWidthMode === PropertyPaneWidthMode.Narrow ? 320 : 340;
                }
                postMessage({
                    type: FAB_TOGGLE_EVT,
                    position: position
            ...
            updateFabPosition();
            if (isODSPChatPaneResizeEnabled()) {
                this._resizeHandler = updateFabPosition;
        ...
        if (isODSPChatPaneResizeEnabled()) {
            this._removeAllPropertyPaneContainerClasses();
            switch (this._paneWidthMode) {
                case PropertyPaneWidthMode.Narrow:
                    this._propertyPaneContainer.classList.add(styles.showPaneNarrow);
                    break;
                case PropertyPaneWidthMode.Wide:
                    this._propertyPaneContainer.classList.add(styles.showPaneWide);
                    break;
                default:
            ...
            this._propertyPaneContainer.classList.add(this._paneWidthMode === PropertyPaneWidthMode.Narrow ? styles.showPaneNarrow : styles.showPane);
        }
        this._propertyPaneContainer.dataset.automationId = 'showPane';
        this._isOpen = true;
    };
    PropertyPaneController.prototype._removeAllPropertyPaneContainerClasses = function () {
        this._propertyPaneContainer.classList.remove(styles.showPaneNarrow, styles.showPaneWide, styles.showPane);
    };
    /**
    ...
    PropertyPaneController.prototype._hidePropertyPane = function () {
        ...
        if (isFabEnabled()) {
            postMessage({ type: FAB_TOGGLE_EVT, position: { isShiftedForPane: false } });
            if (this._resizeHandler && isODSPChatPaneResizeEnabled()) {
        ...
        if (isODSPChatPaneResizeEnabled()) {
            ...
            this._propertyPaneContainer.classList.remove(this._paneWidthMode === PropertyPaneWidthMode.Narrow ? styles.showPaneNarrow : styles.showPane);
        }
        if (isCardDesignerPlusPlusFlight2Enabled() && this._isPropertyPaneExpandedClassActive()) {
            this._propertyPaneContainer.classList.remove(styles.spPropertyPaneContainerExpanded);
        }
    ...
    context) {
        ...
        if (currentConsumer) {
            var contentPanelMonitor_1 = isConfigurationToolsEnabled()
                ? new _QosMonitor(getQOSName('PropertyPaneController', SPPagesContentPanelQOSScenarios.rendering))
                : undefined;
            currentConsumer
                ...
                contentPanelMonitor_1 === null || contentPanelMonitor_1 === void 0 ? void 0 : contentPanelMonitor_1.writeSuccess();
            })
                ...
                contentPanelMonitor_1 === null || contentPanelMonitor_1 === void 0 ? void 0 : contentPanelMonitor_1.writeUnexpectedFailure('UnhandledError', new CustomError({
                    message: markPrivacyData(String(error.message), isContentPanelRenderLoggingEnabled() ? 'safe' : 'unsafe'),
                    innerError: error
                }));
                throw error;
            });
    ...
    PropertyPaneController.prototype._buildAndLoadPropertyPane = function (propertyPaneData, reset) {
        ...
        if (isCardDesignerPlusPlusFlight2Enabled()) {
            ...
            if (shouldExpand && !isExpandedClassAdded) {
                this._propertyPaneContainer.classList.add(styles.spPropertyPaneContainerExpanded);
            }
        ...
        import(/* webpackChunkName: 'property-pane-component' */ '../propertyPane/PropertyPane')
            ...
            if (_this._propertyPaneElement) {
                ...
                var el = React.createElement(propertyPane.default, {
                    ...
                    propertyPaneWidthMode: _this._paneWidthMode
                }, _this._topControl);
                // eslint-disable-next-line @rushstack/pair-react-dom-render-unmount
    ...
    PropertyPaneController.prototype._onPropertyPaneFieldChanged = function (propertyName, newValue, fieldType) {
        ...
        if (consumer) {
            ...
            if (!isDispatchAllPropertyListenersKSActivated()) {
                ...
                this._propertyPaneChangeListeners.dispatch(this._currentPropertyPaneData.webPartId);
            }
    ...
    PropertyPaneController.prototype._onConfigurationEvent = function (configurationEvent) {
        switch (configurationEvent) {
            ...
            case 'LostFocus':
                ...
                break;
            case 'BackClicked':
                !isShowBackButtonBackKSActivated() &&
                    this._fireConfigurationEvent(this._currentlyConfiguredConsumerId, configurationEvent);
        }
    };
    /**
    ...
    PropertyPaneController.prototype._fireConfigurationEvent = function (componentId, configurationEvent) {
        ...
        if (consumer) {
            ...
            _SPEventManager.instance.raiseEvent('propertyPaneEvent', eventArgs);
    ...
    PropertyPaneController.prototype._isPropertyPaneExpanded = function () {
        return (this._currentPropertyPaneData && isPropertyPaneMultiColumn(this._currentPropertyPaneData.configuration));
    };
    PropertyPaneController.prototype._isPropertyPaneExpandedClassActive = function () {
        return this._propertyPaneContainer.classList.contains(styles.spPropertyPaneContainerExpanded);
    };
