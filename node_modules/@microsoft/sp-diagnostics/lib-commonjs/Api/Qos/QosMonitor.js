...
var QosMonitor = /** @class */ (function () {
    ...
    extraData) {
        ...
        var errorString = !ex ? undefined : (0, odsp_core_bundle_1.getErrorMessage)(ex);
        if (ex instanceof odsp_core_bundle_1.CustomError) {
            var endSchema = (0, odsp_core_bundle_1.getQosEndSchemaFromError)(ex);
            if (endSchema) {
                endSchema.resultType = this._endResult;
                // Make sure the extraData from `endSchema` takes priority so that
                // extra data like correlation id from the error object is logged
                // instead of the generic extra data from `_endExtraData`.
                endSchema.extraData = (0, sp_lodash_subset_1.merge)({}, this._endExtraData, endSchema.extraData);
            }
            QosLogger_1.default.instance.writeQosEndResult(this._id, endSchema);
        }
        else {
            var endData = {
    ...
    QosMonitor.prototype._isDebugSession = function () {
        try {
            if (!sp_core_library_1._SPKillSwitch.isActivated('761ccb86-4fa0-4f56-a05b-477bcc09bd9f' /* '08/01/2024', 'finnnavin: Let QoSMonitor run in debug, uploading is blocked at SDK-level.' */)) {
                return false;
            }
            else {
            ...
            // KillSwitches throw exceptions when not initialized.
            // Currently this only happens when spfx-debug is present, so telemetry isn't being logged by the handler, and we can return false.
            return false;
        }
    };
