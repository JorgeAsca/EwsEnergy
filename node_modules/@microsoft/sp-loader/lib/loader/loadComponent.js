            ...
            if (resourceManifest) {
                ...
                if (moduleConfiguration_1.failoverPath) {
                    var dep = moduleLoader
                        ...
                        .catch(function (e) {
                        return _processLoadErrors(manifest, name_1, __spreadArray([ResourceUrlChecker.checkResourceUrl], moduleLoader.loadComponentDependencyErrorProcessors, true), function () {
                            return ErrorBuilder.buildLoadComponentDependencyFailoverPathError(manifest, name_1, moduleConfiguration_1.failoverPath, e);
                        });
                    });
                    depPromises.push(dep);
                }
                    ...
                    var dep = ManifestStore.instance
    ...
    for (var name_2 in resources) {
        if ((resources[name_2].type === 'path' || resources[name_2].type === 'localizedPath') &&
            ...
            if (name_2 !== manifest.loaderConfig.entryModuleId) {
                void _loadPathDependency(manifest, name_2, loadedPathDependencies, moduleLoader);
            }
        }
    }
...
function _moduleLoaderLoadPathDependency(manifest, name, moduleLoader) {
    ...
    return moduleLoader.load(manifest, name, globalName).catch(function (e) {
        return _processLoadErrors(manifest, name, __spreadArray([ResourceUrlChecker.checkResourceUrl], moduleLoader.loadPathDependencyErrorProcessors, true), function () { return ErrorBuilder.buildLoadPathDependencyError(manifest, name, e); });
    });
}
/**
...
function _loadEntryPoint(manifest, moduleLoader) {
    return moduleLoader.load(manifest).catch(function (e) {
        return _processLoadErrors(manifest, manifest.loaderConfig.entryModuleId, __spreadArray([ResourceUrlChecker.checkResourceUrl], moduleLoader.loadEntryPointErrorProcessors, true), function () { return ErrorBuilder.buildLoadEntryPointError(manifest, e); });
    });
}
