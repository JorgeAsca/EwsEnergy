...
import { _QosMonitor } from '@msinternal/sp-telemetry';
import ComponentStore from '../../stores/ComponentStore';
import { reactComponentId, reactDebugComponentId, reactDomComponentId, reactDomDebugComponentId, reactDomProfilingComponentId, reactQueryComponentId, swcHelpersComponentId, tslibComponentId } from '../../utilities/componentConstants';
import { isVersionMismatchThrowKSActivated, isVivaHomeReloadKSActive, isVivaHomeReloadWithBypassKSActive } from '../common/killSwitches';
...
var ManifestStore = /** @class */ (function () {
    ...
    ManifestStore.prototype.addManifest = function (manifest, shouldOverwrite, shouldPin, isDebug) {
        ...
        if (shouldPin ||
            (!_SPFlight.isDebugFlightEnabled &&
                isInternal &&
                id !== reactComponentId &&
                id !== reactDomComponentId &&
                id !== reactQueryComponentId &&
                id !== tslibComponentId &&
                id !== swcHelpersComponentId)) {
    ...
    ManifestStore.prototype.clearManifests = function () {
        ...
        this._store.forEach(function (sEntry, sKey) {
            if (!sEntry.isPinned &&
                sKey !== reactComponentId &&
                sKey !== reactDomComponentId &&
                sKey !== reactQueryComponentId &&
                sKey !== tslibComponentId &&
                sKey !== swcHelpersComponentId) {
                sEntry.manifests.forEach(function (mEntry, mKey) {
                    if (!mEntry.isDebug) {
                        // If there is only one manifest delete the whole entry
                        if (sEntry.manifests.size === 1) {
    ...
    ManifestStore.prototype.getManifest = function (id, version) {
        ...
        if (storeEntry) {
            ...
            if (!version) {
                ...
                var originalReqVersion_1 = Version.parse(version);
                ...
                if (!manifest) {
                    ...
                    var stats = window.__swstats;
                    // TODO: demyren: move service worker version to createLoggerContext
                    var error = new Error("Cannot find manifest for ".concat(id, "_").concat(version, ". Store had [").concat(storeVersions_1.join(', '), "]. Service worker version: ").concat(stats ? stats.version : window.__spfxMark ? '(unknown)' : '(none)').concat("; App version: ".concat(stats ? stats.appVersion : '(unknown)', "; Next app version: ").concat(stats ? stats.nextAppVersion : '(unknown)')));
                    monitor.writeUnexpectedFailure(undefined, error, { addedManifestsAuditLog: this._auditLog });
                    /**
                     * Hacky (and temporary) workaround for a vivahome issue where manifests are not getting updated with the
                     * latest manifest versions likely due to service worker cache.
                     */
                    if (!isVivaHomeReloadKSActive() &&
                        storeVersions_1.length &&
                        location.pathname.indexOf('/_layouts/15/vivahome.aspx') !== -1 &&
                        location.search.indexOf('sw=bypass') === -1) {
                        var versionMismatchDetectionMonitor = new _QosMonitor('VivaHome.ManifestVersionMismatch');
                        versionMismatchDetectionMonitor.writeUnexpectedFailure('mismatch detected', error);
                        if (isVivaHomeReloadWithBypassKSActive()) {
                            var bypassUrl = new URL(location.href);
                            bypassUrl.searchParams.set('sw', 'bypass');
                            location.href = bypassUrl.href;
                        }
                    }
                    if (storeEntry.isPinned && !isVersionMismatchThrowKSActivated()) {
        ...
        for (var resourceId in manifest.loaderConfig.scriptResources) {
            if (manifest.loaderConfig.scriptResources.hasOwnProperty(resourceId)) {
                ...
                if (resource.type === 'component') {
                    ...
                    if (componentResource.id === reactDomComponentId) {
        ...
        for (var resourceId in manifest.loaderConfig.scriptResources) {
            if (manifest.loaderConfig.scriptResources.hasOwnProperty(resourceId)) {
                ...
                if (resource.type === 'component') {
                    ...
                    var componentResource = resource;
                    if (componentResource.id === reactDomComponentId) {
                        componentResource.id = reactDomDebugComponentId;
                    }
                    else if (componentResource.id === reactComponentId) {
                        componentResource.id = reactDebugComponentId;
                    }
                }
            }
        }
...
var OldManifestStoreShim = /** @class */ (function () {
    function OldManifestStoreShim(serviceScope, webAbsoluteUrl, buildNumber) {
        ...
        this._impl = new ManifestStore(new ManifestProvider(serviceScope, webAbsoluteUrl, buildNumber));
    }
    ...
    OldManifestStoreShim.prototype.getManifest = function (id, version) {
        ...
        if (m) {
            ...
            throw new Error("Manifest not found ".concat(normalizeComponentId(id, version || '')));
        }
    };
