...
import { _PerformanceLogger, _QosMonitor } from '@msinternal/sp-telemetry';
import { isBotACEManifestLoadLogicRemovedKSActivated, traceRequireLoaderErrorKSActivated } from '../common/killSwitches';
...
import { isAPEXSPLoaderRemoveSystemJSEnabled } from '../common/flights';
import ManifestStore from '../../stores/ManifestStore';
import { isLoadTrackTimesKSActivated } from '../../utilities/KillSwitches';
/**
 * RequireJS Loader
 */
var RequireLoader = /** @class */ (function () {
        ...
        if (!isRequireLoaded) {
            ...
            DEPRECATED_UNIT_TEST
                ...
                : // eslint-disable-next-line @typescript-eslint/no-require-imports, no-eval
                    eval.call(window, require('!!raw-loader!../../../blobs/requirejs/2.3.7/require.min'));
        }
    ...
    RequireLoader.prototype.loadEntryPoint = function (manifest, name, globalName) {
        ...
        if (entryModuleId) {
            ...
            if (entryModule && entryModule.type === 'component') {
                var realManifest = ManifestStore.instance.tryGetManifest(entryModule.id, entryModule.version);
                if (!realManifest) {
        ...
        return this.load(normalizedName, globalName).then(function (module) {
            ...
            if (!isLoadTrackTimesKSActivated()) {
                var loadTime = _this._moduleLoads.get(manifest.id);
                if (loadTime) {
                    _PerformanceLogger.addLoadTime({
                        alias: manifest.alias,
                        isInternal: manifest.isInternal === true,
                        duration: loadTime
                    });
                    _PerformanceLogger.trackModuleLoadTimes = true;
                }
            }
            return module;
        });
    ...
    RequireLoader.prototype._loadComponent = function (manifest, retryCount) {
        ...
        var loadAllDeps = function () {
            return Promise.all(loadComponentDependencies(_this, manifest, depNames));
        };
        return this._configurator
            ...
            if (!isRecursiveCatch) {
                if (!traceRequireLoaderErrorKSActivated()) {
            ...
            if (manifest.isInternal || isAPEXSPLoaderRemoveSystemJSEnabled() || isScriptError) {
                throw e;
            }
    ...
    RequireLoader.prototype._loadWithDifferentModuleId = function (moduleName) {
        ...
        else if (isBotACEManifestLoadLogicRemovedKSActivated() && path.indexOf('/bot-driven-ace') !== -1) {
            //
            // 12/11/23, aterentiev, The special load logic for Bot-Driven ACEs is removed. We have corrected a server-side logic to load Bot ACE as component.
            //
    ...
    RequireLoader.prototype._require = function (moduleName) {
        ...
        return new Promise(function (resolve, reject) {
            _this._requireJs([moduleName], function (module) {
                resolve(module);
                // Measure the time it took to load/handle this code.
                if (_this._configurator.trackLoadTimes) {
                    var loadStartTime = _this._configurator.getLoadStartTime(moduleName);
                    if (loadStartTime) {
                        var endTime = performance.now();
                        performance.measure('requireTime_' + moduleName, { start: loadStartTime, end: endTime });
                        var componentId = moduleName.substring(0, moduleName.indexOf('_'));
                        _this._moduleLoads.set(componentId, endTime - loadStartTime);
                    }
                }
