...
exports._evaluateFlightOrManifestOperator = exports.loadPathDependencies = exports.loadComponentDependencies = void 0;
...
function loadComponentDependency(loader, name, resource) {
    var effectiveVersion = !(0, killSwitches_1.isFlightedSPFxDependenciesKillSwitchEnabled)() &&
        resource.fallbackVersion &&
        _evaluateFlightOrManifestOperator(resource.fallbackVersion.condition)
        ? resource.fallbackVersion.version
        : resource.version;
    var dep;
    var depManifest = ManifestStore_1.default.instance.tryGetManifest(resource.id, effectiveVersion);
    if (depManifest) {
        ...
        if (resource.failoverPath) {
            ...
            dep = ManifestStore_1.default.instance
                .requestManifest(resource.id, effectiveVersion)
...
function _evaluateFlightOrManifestOperator(operator) {
    if ('and' in operator) {
        return operator.and.every(function (op) { return _evaluateFlightOrManifestOperator(op); });
    }
    else if ('or' in operator) {
        return operator.or.some(function (op) { return _evaluateFlightOrManifestOperator(op); });
    }
    else if ('xor' in operator) {
        var _a = operator.xor, left = _a[0], right = _a[1];
        return _evaluateFlightOrManifestOperator(left) !== _evaluateFlightOrManifestOperator(right);
    }
    else if ('not' in operator) {
        return !_evaluateFlightOrManifestOperator(operator.not);
    }
    else {
        var component = operator;
        if (component.switchType === 'flight') {
            return sp_core_library_1._SPFlight.isEnabled(component.id);
        }
        else {
            return sp_core_library_1._SPKillSwitch.isActivated(component.id);
        }
    }
}
exports._evaluateFlightOrManifestOperator = _evaluateFlightOrManifestOperator;
