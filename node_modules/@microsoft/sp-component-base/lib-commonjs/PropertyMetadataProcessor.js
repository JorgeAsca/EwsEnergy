...
var sp_safehtml_1 = require("@msinternal/sp-safehtml");
/**
...
var PropertyMetadataProcessor = /** @class */ (function () {
    ...
    PropertyMetadataProcessor.reInstateServerProcessedData = function (propertiesMetadata, properties, serverProcessedContent, manifest) {
        var _this = this;
        var propertyPathsNotInServerProcessedContent = new Set();
        if (manifest) {
            ...
            this._forEachPropertyWithMetaData(propertiesMetadata, properties, function (propPath, metadata) {
                if (!(0, Flights_1.isServerPropertiesSanitizedEnabled)()) {
                    ...
                    // We should always sanitize the deserialized property bag for any property
                    // that is marked with propertiesMetadata. It is possible for the content to have been
                    // modified at rest and therefore work around the assumption of the serverProcessedContent
                    // properties matching the propertiesMetadata. Since the propertiesMetadata is generated at
                    // runtime we should enforce an expectation of the serverProcessedContent to match.
                    //
                    // This implicitly means a web part will receive a property bag that could not have properties it is expecting
                    // if the data has been incorrectly modified at rest. The web part is expected to handle that.
                    //
                    // If the path is already undefined then we can skip setting to undefined and the enforcement
                    if (manifest.id === 'b7dd04e1-19ce-4b24-9132-b60a1c2b910d' &&
                        sp_core_library_1._SPKillSwitch.isActivated('71b68bd4-eb7a-4330-93bd-dc6d382123c1' /* '9/28/2023', 'Do not serverProcessContent sanitization for DocumentEmbedWebPart' */)) {
                        return;
                    }
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    var propValue = lodash.get(properties, propPath);
                    if (propValue !== undefined) {
                        var qosMonitor = new sp_diagnostics_1._QosMonitor('PropertyMetadataProcessor.clearMetadataValue');
                        if (metadata.isLink) {
                            propertyPathsNotInServerProcessedContent.add(propPath);
                            if (!_this._isSafeLink(propValue)) {
                                qosMonitor.writeUnexpectedFailure('ClearingLinkProperty', new Error(propPath));
                                lodash.set(properties, propPath, undefined);
                            }
                        }
                        if (metadata.isHtmlString) {
                            propertyPathsNotInServerProcessedContent.add(propPath);
                            var safeHtml = _this._getSafeHtml(propValue);
                            if (safeHtml !== propValue) {
                                qosMonitor.writeUnexpectedFailure('SettingSafeHtml', new Error(propPath));
                                lodash.set(properties, propPath, safeHtml);
                            }
                        }
        ...
        if (serverProcessedContent) {
            ...
            // CustomMetadata is only processed on the server so we do not round trip the data back
            // into the web part properties.  This is by design of the feature
            var htmlMaps = [
                ...
                if (htmlMap) {
                    for (var _a = 0, _b = Object.entries(htmlMap); _a < _b.length; _a++) {
                        ...
                        if (typeof value === 'string') {
                            ...
                            if ((0, Flights_1.isServerPropertiesSanitizedEnabled)()) {
                                propertyPathsNotInServerProcessedContent.delete(propPath);
                            }
                        }
                    }
                }
        ...
        if ((0, Flights_1.isServerPropertiesSanitizedEnabled)() &&
            manifest &&
            propertyPathsNotInServerProcessedContent.size > 0) {
            var logServerPropertiesMissing = new sp_diagnostics_1._QosMonitor('PropertyMetadataProcessor.serverProcessedContentMissing');
            logServerPropertiesMissing.writeUnexpectedFailure('Missing Content', undefined, {
                manifest: manifest.id,
                properties: Array.from(propertyPathsNotInServerProcessedContent).join(',')
            });
        }
    };
    PropertyMetadataProcessor._isSafeLink = function (link) {
        try {
            var safeLinkUrl = new URL(link);
            return sp_safehtml_1.SafeHtml.isSafeLinkProtocol(safeLinkUrl.protocol);
        }
        catch (_a) {
            try {
                var safeLinkUrl = new URL(link, window.location.origin);
                return sp_safehtml_1.SafeHtml.isSafeLinkProtocol(safeLinkUrl.protocol);
            }
            catch (_b) {
                return false;
    ...
    PropertyMetadataProcessor._getSafeHtml = function (html) {
        return sp_safehtml_1.SafeHtml.clean(html);
    };
    /**
